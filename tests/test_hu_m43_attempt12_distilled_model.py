...
from ofc_regular.hu_m43_attempt12_teacher import ATTEMPT12_FROZEN_MODEL_ID
...
def _model() -> HuM43Attempt12DistilledModel:
    ...
    return HuM43Attempt12DistilledModel(
...
def test_attempt12_candidate_mapping_is_all_legal_unique_and_excludes_baseline() -> None:
    sample = _sample()
...
def test_attempt12_zero_candidate_state_falls_back_to_explicit_baseline(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    sample = _sample()
    baseline = sample["baseline_action_row_index"]
    keys = tuple(action_key_from_payload(item) for item in sample["actions"][:2])
    built = Attempt12DistilledFeatures(
        runtime_sample=sample,
        features=np.zeros(
            (2, HU_M43_ATTEMPT12_DISTILLED_FEATURE_DIM), dtype=np.float32
        ),
        action_keys=keys,
        candidate_indices=(),
        baseline_index=0,
...
def test_attempt12_action_key_candidates_ignore_input_enumeration_order() -> None:
    sample = _sample()
