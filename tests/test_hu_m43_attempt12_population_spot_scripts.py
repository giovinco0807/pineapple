...
PLAN = ROOT / "configs" / "hu_joint_policy_m43_attempt12_population.json"
START = ROOT / "scripts" / "Start-GcpHuM43Attempt12PopulationRun.ps1"
STATUS = ROOT / "scripts" / "Get-GcpHuM43Attempt12PopulationRunStatus.ps1"
RECEIVE = ROOT / "scripts" / "Receive-GcpHuM43Attempt12PopulationRun.ps1"
...
POPULATION_PLAN_SHA256 = "66d9c3dc50bcd6ae2ce72e2a558dcfae5720d3cb2387a16ef810d1498f4db462"
SEED_REGISTRY_SHA256 = "de63af1ded1bfe6ff7c92dc6bcc8aedec9c408956e687c856198143bffca8255"
...
def test_attempt12_population_scripts_parse(path: Path) -> None:
    shell = _powershell()
    if shell is None:
...
def test_attempt12_population_plan_is_fixed_before_labels() -> None:
    ...
    assert hashlib.sha256(PLAN.read_bytes()).hexdigest() == POPULATION_PLAN_SHA256
    assert runtime == {
    ...
    assert (
        plan["freshness"]["excluded_schedule_registry_sha256"]
        == SEED_REGISTRY_SHA256
    )


def test_attempt12_population_seed_schedule_is_disjoint_from_declared_history() -> None:
    plan = json.loads(_text(PLAN))
    ...
    for base in plan["freshness"]["excluded_attempt12_namespace_bases"]:
        attempt12_search = {int(base) + stride * index for index in range(250)}
        assert population.isdisjoint(attempt12_search)
...
def test_attempt12_population_plan_is_accepted_by_existing_strict_merger() -> None:
    plan = load_population_plan(PLAN)
    assert plan["fixed_baseline_profile"] == "stage19_p0"
    assert plan["shards"] * plan["paired_seeds_per_shard"] == 1000


def test_dry_run_cannot_create_attempt12_instances() -> None:
    shell = _powershell()
    if shell is None:
        pytest.skip("PowerShell is unavailable")
    completed = subprocess.run(
        [
            ...
            "unused",
            "-DevelopmentDecisionPath",
            "unused",
            "-DevelopmentSelectorReceiptPath",
            "unused",
            "-DevelopmentPassFreezePath",
            "unused",
            "-AuditDecisionPath",
            "unused",
            "-AuditSelectorReceiptPath",
            "unused",
            "-DryRun",
            "-CreateInstances",
        ],
...
def test_fresh_mutation_without_package_only_has_no_side_effects() -> None:
    ...
    if shell is None:
        pytest.skip("PowerShell is unavailable")
    run_name = f"attempt12-no-mode-{uuid.uuid4().hex}"
    ...
    assert completed.returncode != 0
    assert (
        "Fresh Attempt12 population mutation requires -PackageOnly"
        in completed.stdout + completed.stderr
    )
    assert not run_dir.exists()


def test_package_only_requires_complete_frozen_binding_artifacts() -> None:
    shell = _powershell()
    if shell is None:
        pytest.skip("PowerShell is unavailable")
    run_name = f"attempt12-missing-binding-{uuid.uuid4().hex}"
    run_dir = ROOT / "outputs" / "gcp_runs" / run_name
    completed = subprocess.run(
        [
            shell,
            ...
            "-RuntimeFreezePath",
            "missing-runtime-freeze",
            "-RuntimeSourceArchivePath",
            "missing-runtime-source",
            "-RuntimeSourceManifestPath",
            "missing-runtime-source-manifest",
            "-DevelopmentDecisionPath",
            "missing-development-decision",
            "-DevelopmentSelectorReceiptPath",
            "missing-development-selector-receipt",
            "-DevelopmentPassFreezePath",
            "missing-development-pass-freeze",
            "-AuditDecisionPath",
            "missing-audit-decision",
            "-AuditSelectorReceiptPath",
            "missing-audit-selector-receipt",
            "-PackageOnly",
        ],
        text=True,
        encoding="utf-8-sig",
...
def test_start_freezes_runtime_only_then_requires_canary_before_fanout() -> None:
    ...
    for token in (
        ...
        'artifacts/development_decision.json',
        'artifacts/development_selector_receipt.json',
        'artifacts/development_pass_freeze.json',
        'artifacts/audit50_decision.json',
        'artifacts/audit50_selector_receipt.json',
        ...
        "provenance_json_evidence_packaged = $true",
        ...
        'heartbeat = [ordered]@{ schema = $StatusSchema',
        "The recurring status object is the heartbeat",
        'HEARTBEAT_PID=""',
...
def test_embedded_attempt12_startup_has_valid_bash_syntax() -> None:
...
def test_status_and_receive_bind_transport_merge_and_final_acceptance() -> None:
    ...
    for token in (
        "hu_m43_attempt12_population_spot_manifest_v1",
        "hu_m43_attempt12_population_spot_done_v1",
        "hu_m43_attempt12_population_spot_status_v1",
        MODEL_SCHEMA,
        ACTION_SCORE_MODE,
        ...
        "active_instances",
        "Try-GetGcsJson",
        "Invalid or stale Attempt12 population status object",
    ...
    for token in (
        "outputs/hu_joint_policy/m43_attempt12_population",
        "refusing profile/current overwrite",
        "hu_m43_attempt12_population_spot_done_v1",
        "hu_m43_attempt12_population_spot_receipt_v1",
        ...
        'development_selector_receipt.json',
        'development_pass_freeze.json',
        'audit50_decision.json',
        'audit50_selector_receipt.json',
    ):
