...
from ofc_regular.hu_m43_attempt13_teacher import ATTEMPT13_FROZEN_MODEL_ID
...
def _model() -> HuM43Attempt13DistilledModel:
    ...
    return HuM43Attempt13DistilledModel(
...
def test_attempt13_candidate_mapping_is_all_legal_unique_and_excludes_baseline() -> None:
    sample = _sample()
    built = _model().build_features(sample)
    candidate_keys = [built.action_keys[index] for index in built.candidate_indices]
    expected_keys = {
        key
        for index, key in enumerate(built.action_keys)
        if index != built.baseline_index
    }
    assert len(candidate_keys) == 26
    assert set(candidate_keys) == expected_keys
    assert len(set(candidate_keys)) == len(candidate_keys)
    assert candidate_keys == sorted(candidate_keys, key=lambda key: key.sort_key())
    ...
    assert _model().runtime_contract["candidate_set"] == (
        "every_complete_legal_nonbaseline_action"
    )
    assert _model().runtime_contract["candidate_cutoff"] is False
    assert _model().runtime_contract["candidate_padding"] is False
    assert _model().runtime_contract["candidate_duplicates"] is False


def test_attempt13_zero_candidate_state_falls_back_to_explicit_baseline(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    sample = _sample()
    baseline = sample["baseline_action_row_index"]
    keys = tuple(action_key_from_payload(item) for item in sample["actions"][:2])
    built = Attempt13DistilledFeatures(
        runtime_sample=sample,
        features=np.zeros(
            (2, HU_M43_ATTEMPT13_DISTILLED_FEATURE_DIM), dtype=np.float32
        ),
        action_keys=keys,
        candidate_indices=(),
        baseline_index=0,
...
def test_attempt13_action_key_candidates_ignore_input_enumeration_order() -> None:
    sample = _sample()
...
@pytest.mark.parametrize(
    "hidden_key", ("opponent_private_discards", "opponent_discards")
)
def test_attempt13_features_reject_opponent_hidden_discards(hidden_key: str) -> None:
    sample = _sample()
    sample[hidden_key] = ["Ks"]
    with pytest.raises(ValueError, match="hidden|private|discard"):
        _model().build_features(sample)
