...
from ofc_regular import (
    hu_m31_t3_step6d_performance_lock_rearm2_open as rearm_open,
)
from ofc_regular import hu_m31_t3_step6d_performance_lock_spot_v1 as spot_v1
from ofc_regular import (
    verify_hu_m31_t3_feature_encoder_platform_parity_rearm2 as subject,
)
...
def _chain() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    smoke = rearm_open._expected_smoke_requirement()
    ...
    claim.update(
        {
            ...
            "seed_contract": (
                rearm_open.runner.candidate02_performance_lock_recovery_v3_seed_contract()
            ),
            ...
            "rearm_guards": {
                ...
                **{field: False for field in reuse_fields},
            },
            "restrictions": {
    ...
    hashes = [
        hashlib.sha256(f"rearm2-root-{index}".encode("ascii")).hexdigest()
    ...
    materialization.update(
        {
            ...
            "plan_sha256": rearm_open.PRECONTENT_PLAN_SHA256,
            "run_contract_digest": rearm_open.LOCK_RUN_CONTRACT_DIGEST,
    ...
    seal.update(
        {
            ...
            "prior_lock_comparison": {
                ...
                "old_v1_root_hash_overlap_count": 0,
                "old_v1_fingerprint_overlap_count": 0,
                "old_v1_seed_overlap_count": 0,
                ...
                **{field: False for field in reuse_fields[1:]},
            },
            "visibility": {
...
def _fixture(tmp_path: Path) -> dict[str, Path]:
    ...
    paths = {
        ...
        "global_claim": tmp_path / "GLOBAL_PERFORMANCE_LOCK_REARM2_CLAIM.json",
...
def test_rearm2_wrapper_writes_base_compatible_receipt_and_restores_v1(
    tmp_path: Path,
) -> None:
    ...
    assert receipt["lock_chain"]["contract"] == "performance_lock_rearm2_v1"
    assert receipt["lock_chain"]["validator_sha256"] == probe_v1.sha256_file(
        Path(subject.__file__)
    )
    assert receipt["lock_chain"]["v1_reused"] is False
    assert receipt["lock_chain"]["rearm1_reused"] is False
    assert receipt["lock_chain"]["fresh_recovery_v3_seed_schedule"] is True


def test_rearm2_wrapper_rejects_rearm1_overlap_and_restores_v1(
    tmp_path: Path,
) -> None:
    paths = _fixture(tmp_path)
    seal = probe_v1._read_canonical(paths["seal"], "seal")
    seal["prior_lock_comparison"]["rearm1_root_hash_overlap_count"] = 1
