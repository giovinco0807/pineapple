...
from ofc_regular import hu_m7_t2_2048_plan_v1 as subject
...
def test_reviewed_m7v5_identity_is_pinned() -> None:
    assert subject.DEFAULT_PACKAGE_IDENTITY == subject.ExpectedPackageIdentity(
        ledger_sha256="d124e98617a34d7767f0da9e04b5ceea489c97a52009b7779ffd150b20ee5dcf",
        runtime_sha256="ae8be6cd2aa333654141368c1ad713a1c6ab3ff1dedea752edcdeab2551d1988",
        wheelhouse_sha256="338ca072984775dc886e7c9c88d1d7e10b5363ae4aff5a7ba936eebf57d25053",
    )


def test_builds_paired_25k_2048_plans_with_expected_asymmetry(
    ...
    assert len(audit.declared_hashes) == 22
    assert len(audit.member_hashes) == 20
    for seat, plan in plans.items():
        assert plan["street"] == "T2"
        assert plan["seat"] == seat
        assert plan["samples"] == 2048
        assert plan["seeds_per_position"] == 1
        assert plan["hand_seed_base"] == 947_000_000
        assert plan["eval_seed_base"] == 10_000_000
        assert plan["fl_ev_value"] == 9.6
        assert len(plan["shards"]) == 174
    ...
    assert first["t2_second_model"] == "weights/t2_model_v1.bin"
    assert "t2_second_model" not in second
    assert "t2_second_model_sha256" not in second
...
def test_shards_are_contiguous_unique_and_larger_shards_are_last() -> None:
    ...
    assert [row["shard_id"] for row in shards] == [
        f"{index:03d}" for index in range(174)
    ]
    assert [row["count"] for row in shards[:56]] == [143] * 56
    assert [row["count"] for row in shards[56:]] == [144] * 118
    start = 0
    for row in shards:
...
def test_audit_rejects_member_hash_drift_even_with_reissued_identity(
    ...
    ledger["t4_model_v6_sha256"] = "0" * 64
    ...
    with pytest.raises(subject.PlanValidationError, match="t4_model_v6.bin digest"):
...
def test_pair_validator_rejects_second_seat_t2_reply_pin(
    tmp_path: pathlib.Path,
) -> None:
    package, identity = _fixture_package(tmp_path)
    plans, _ = subject.build_plan_pair(package, expected_identity=identity)
    plans["second"]["t2_second_model"] = "weights/t2_model_v1.bin"
    plans["second"]["t2_second_model_sha256"] = plans["first"][
