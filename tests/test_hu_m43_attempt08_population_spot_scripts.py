...
START = ROOT / "scripts" / "Start-GcpHuM43Attempt08PopulationRun.ps1"
STATUS = ROOT / "scripts" / "Get-GcpHuM43Attempt08PopulationRunStatus.ps1"
RECEIVE = ROOT / "scripts" / "Receive-GcpHuM43Attempt08PopulationRun.ps1"
...
def test_attempt08_population_scripts_parse(path: Path) -> None:
    shell = _powershell()
    if shell is None:
...
def test_attempt08_population_plan_is_fixed_before_labels() -> None:
...
def test_attempt08_population_seed_schedule_is_disjoint_from_declared_history() -> None:
    plan = json.loads(_text(PLAN))
    ...
    for base in plan["freshness"]["excluded_attempt08_namespace_bases"]:
        attempt08_search = {int(base) + stride * index for index in range(250)}
        assert population.isdisjoint(attempt08_search)
...
def test_attempt08_population_plan_is_accepted_by_existing_strict_merger() -> None:
    plan = load_population_plan(PLAN)
    assert plan["fixed_baseline_profile"] == "stage19_p0"
    assert plan["shards"] * plan["paired_seeds_per_shard"] == 1000


def test_dry_run_cannot_create_attempt08_instances() -> None:
    shell = _powershell()
    if shell is None:
        pytest.skip("PowerShell is unavailable")
    completed = subprocess.run(
...
def test_fresh_mutation_without_package_only_has_no_side_effects() -> None:
    ...
    if shell is None:
        pytest.skip("PowerShell is unavailable")
    run_name = f"attempt08-no-mode-{uuid.uuid4().hex}"
    ...
    assert completed.returncode != 0
    assert (
        "Fresh Attempt08 population mutation requires -PackageOnly"
...
def test_embedded_attempt08_startup_has_valid_bash_syntax() -> None:
...
def test_status_and_receive_bind_transport_merge_and_final_acceptance() -> None:
    ...
    for token in (
        "hu_m43_attempt08_population_spot_manifest_v1",
        "hu_m43_attempt08_population_spot_done_v1",
        "hu_m43_attempt08_population_spot_status_v1",
        MODEL_SCHEMA,
        ACTION_SCORE_MODE,
        ...
        "active_instances",
        "Try-GetGcsJson",
        "Invalid or stale Attempt08 population status object",
        "fixed 20x50 schedule",
    ):
        assert token in status
    for token in (
        "outputs/hu_joint_policy/m43_attempt08_population",
        "refusing profile/current overwrite",
        "hu_m43_attempt08_population_spot_done_v1",
        "hu_m43_attempt08_population_spot_receipt_v1",
