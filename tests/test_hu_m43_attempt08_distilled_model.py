...
def _test_attestation() -> FrozenExecutionModulesAttestation:
    ...
    object.__setattr__(
        attestation,
        "module_paths",
        tuple((name, f"unit-test/{name}.py") for name in ATTEMPT08_BOUND_EXECUTION_MODULES),
    )
    return attestation
...
def _test_bound(
    ...
    bound = object.__new__(BoundHuM43Attempt08DistilledModel)
    object.__setattr__(
        bound, "_BoundHuM43Attempt08DistilledModel__model", model
    )
    object.__setattr__(
        bound,
...
def _model(*, unsafe_folds: int = 0):
    raw = HuM43Attempt08DistilledModel(
        ...
        safety_enabled=True,
        winner_frozen=True,
        model_id="attempt08-distilled-test",
    )
    return _test_bound(raw)


def test_distilled_model_rebuilds_full_legal_set_and_selects_only_top8() -> None:
    sample = _sample()
    model = _model()
...
def test_distilled_model_artifact_round_trip_and_sha_binding(tmp_path) -> None:
    ...
    loaded = HuM43Attempt08DistilledModel.load(path, expected_sha256=digest)
    ...
    with pytest.raises(TypeError, match="loader-issued"):
        BoundHuM43Attempt08DistilledModel(loaded, object())
    with pytest.raises(TypeError, match="loader-issued"):
        BoundHuM43Attempt08DistilledModel(loaded, _test_attestation())
    forged = object.__new__(BoundHuM43Attempt08DistilledModel)
    assert is_bound_attempt08_distilled_model(forged) is False
    with pytest.raises(ValueError, match="SHA-256 mismatch"):
        HuM43Attempt08DistilledModel.load(path, expected_sha256="0" * 64)
...
def test_shared_matrix_fast_candidate_path_matches_legacy_fold_contract() -> None:
    ...
    built = HuM43Attempt08DistilledModel(
    ...
    actions = generate_turn_actions(observation.hero_board, observation.dealt_cards)
    legacy = FrozenAttempt08LambdaRanker(
        model=candidate, artifact_sha256=ATTEMPT08_FROZEN_MODEL_SHA256
