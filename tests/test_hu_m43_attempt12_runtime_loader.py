...
class _RawAttempt12Model:
    schema = HU_M43_ATTEMPT12_DISTILLED_MODEL_SCHEMA
    safety_enabled = True

    def predict_sample_with_baseline(self, *_args, **_kwargs):
        raise AssertionError("raw Attempt12 inference must not execute")

    def predict_safety_probability(self, *_args, **_kwargs):
        raise AssertionError("raw Attempt12 safety inference must not execute")


def test_attempt12_population_runtime_is_always_binding_required() -> None:
    assert requires_bound_population_runtime(_RawAttempt12Model())
...
def test_attempt12_loader_rejects_raw_and_partial_runtime_bindings(
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(ValueError, match="Attempt12 distilled runtime requires"):
...
def test_attempt12_loader_forwards_only_complete_binding_to_acceptance_loader(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    path, digest = _raw_artifact(tmp_path)
    module_name = "ofc_regular.validate_hu_m43_attempt12_acceptance"
    ...
    fake_module.load_bound_attempt12_distilled_model = _load_bound
...
def test_policy_direct_raw_attempt12_model_is_nonfire_and_counterfactual_baseline(
) -> None:
    observation = _observation()
    baseline = _Baseline()
    raw = _RawAttempt12Model()
    decisions: list[dict] = []
    ...
    selected = policy.choose_action_observation(
        observation, hand_id=11, game_id="attempt12", decision_seed=17
    )
    expected = min(
