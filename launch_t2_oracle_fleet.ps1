...
for ($i = 0; $i -lt $Count; $i++) {
    ...
    gcloud compute instances create $name `
        ...
        --metadata="worker-id=$workerId,max-seconds=$MaxSeconds,n-samples=$NSamples,chunk-size=$ChunkSize,states=$States" `
        --metadata-from-file="startup-script=gcp_t2_oracle_startup.sh" `
        --quiet
}

Write-Host ""
Write-Host "Monitor:"
Write-Host "  gcloud compute instances list --filter='name~ofc-t2oracle' --project=$Project"
Write-Host "  gcloud storage ls -r gs://ofc-solver-485418/t2_oracle/results/run_20260509/"
