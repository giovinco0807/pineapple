#!/bin/bash
# GCP Worker Startup Script V3 - Phase C Data Generation
# Supports configurable GCS destination via metadata.
set -e
...
TOP_K=$(curl -s "http://metadata.google.internal/computeMetadata/v1/instance/attributes/top-k" -H "Metadata-Flavor: Google" 2>/dev/null || echo "100")
GCS_DEST=$(curl -s "http://metadata.google.internal/computeMetadata/v1/instance/attributes/gcs-dest" -H "Metadata-Flavor: Google" 2>/dev/null || echo "gs://ofc-solver-485418/t0_phase_c")
...
echo "Seed=$SEED Hands=$HANDS Samples=$SAMPLES Nesting=$NESTING Top-K=$TOP_K"
...
echo "Command: $EXE t0-batch --hands $HANDS --samples $SAMPLES --nesting $NESTING --top-k $TOP_K --output $OUTFILE --seed $SEED"
