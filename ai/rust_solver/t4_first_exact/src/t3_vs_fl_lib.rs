...
/// Mean canonical score of one completed hero board over the matched boards.
fn score_mean(
    ...
    for &index in matched {
        ...
        let fl_busted = library.busted[index];
        let fl_royalty = library.royalty[index];
        let base = if terminal.busted {
            ...
            let fl_values = &library.values[index];
            let mut lines = 0i32;
            for row in 0..3 {
        ...
        let stay_term = if library.stay[index] && !fl_busted {
            stay_cost
        } else {
            0.0
        };
    ...
    let results: Result<Vec<LibActionValue>> = actions
        ...
        .map(|action| {
            ...
            for draw in &draw_sets {
                ...
                let matched: Vec<u32> = shortlist
                    .iter()
                    .copied()
                    .filter(|&index| library.masks[index as usize] & draw_mask == 0)
                    .collect();
                if matched.is_empty() {
                    continue;
                }
                ...
                for pattern in &patterns {
                    ...
                    let terminal = terminal_of(&final_board);
                    let value =
                        score_mean(&terminal, library, &matched, request.opp_count, fl_ev);
                    if value > best {
                        best = value;
                    }
                }
            ...
            let _categories =
                evaluator::opponent_rowwise_block(&after.rows, &unseen, fl_table, &mut rowwise);
