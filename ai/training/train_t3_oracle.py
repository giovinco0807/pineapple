...
Usage:
  python ai/training/train_t3_oracle.py --data-dir D:/ofc_data/t3_round2 --epochs 80
"""
import sys
import argparse
import json
...
class T3Dataset:
    """Load all NPZ files from t3_round2 sub-dirs into GPU tensors."""
...
def train(args):
    ...
    ds = T3Dataset([str(d) for d in sub_dirs], device=device)
    N = len(ds)
    ...
    print(f"  Model: T3PolicyValueNet ({n_params:,} params)")
    ...
    if args.resume:
        ...
        hist_path = save_dir / 't3_training_history.json'
        ...
        if epoch % args.eval_every == 0 or epoch == 1 or epoch == args.epochs:
            ...
            if val_regret < best_val_regret:
                ...
                torch.save({
                    ...
                    'hidden':     args.hidden,
                    'n_blocks':   args.n_blocks,
                }, save_dir / 't3_policyvalue_model.pt')
    ...
    # ── Final save ──
    torch.save(model.state_dict(), save_dir / 't3_policyvalue_final.pt')

    with open(save_dir / 't3_training_history.json', 'w') as f:
    ...
    print(f"  Model saved: {save_dir / 't3_policyvalue_model.pt'}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train T3 Oracle PolicyValueNet')
    parser.add_argument('--data-dir',     default='D:/ofc_data/t3_round2',
                        help='Root dir containing gen-2…gen-b1 subdirs')
    parser.add_argument('--save-dir',     default='ai/data/t3_oracle',
