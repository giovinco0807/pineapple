"""
T4 Value Network Training — PolicyValueNet on generated T4 NPZ dataset

Data format (from generate_t4_data.py):
...
Model: T4PolicyValueNet (identical structure to T3)
  Input: 490-dim state
  Policy head: 27 logits (ListNet ranking loss vs action_evs)
  Value head:  1 scalar (MSE vs best_evs)

Usage:
  python ai/training/train_t4_value.py --data-dir ai/data/t4_dataset_v1 --epochs 80
"""
import sys
import argparse
import json
...
class T4Dataset:
    """Load all NPZ files from data directories into GPU tensors."""
    ...
    def __init__(self, data_dirs, device='cpu', verbose=True):
        ...
        for d in data_dirs:
            ...
            for f in files:
                try:
                    npz = np.load(f)
                    all_states.append(npz['states'])
                    all_evs.append(npz['action_evs'])
                    all_masks.append(npz['action_masks'])
                    all_best.append(npz['best_evs'])
                except Exception as e:
                    if verbose:
                        print(f"  Warning: skip {f.name}: {e}")

        if not all_states:
            raise ValueError(f"No valid NPZ files found in {data_dirs}")
...
def train(args):
    ...
    sub_dirs = [d for d in data_root.iterdir()
                if d.is_dir() and list(d.glob('*.npz'))]
    if not sub_dirs:
        # Try flat structure (all NPZ in data_dir itself)
        sub_dirs = [data_root]
    print(f"  Data dirs: {[d.name for d in sub_dirs]}")

    ds = T4Dataset([str(d) for d in sub_dirs], device=device)
    N = len(ds)
    ...
    print(f"  Model: T4PolicyValueNet ({n_params:,} params)")
    ...
    if args.resume:
        model_path = save_dir / 't4_policyvalue_model.pt'
        hist_path = save_dir / 't4_training_history.json'
        ...
        if epoch % args.eval_every == 0 or epoch == 1 or epoch == args.epochs:
            ...
            if val_regret < best_val_regret:
                ...
                torch.save({
                    ...
                    'hidden':     args.hidden,
                    'n_blocks':   args.n_blocks,
                }, save_dir / 't4_policyvalue_model.pt')
    ...
    # ── Final save ──
    torch.save(model.state_dict(), save_dir / 't4_policyvalue_final.pt')
    ...
    print(f"  Model saved: {save_dir / 't4_policyvalue_model.pt'}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train T4 Oracle PolicyValueNet')
    parser.add_argument('--data-dir',     default='ai/data/t4_dataset_v1',
                        help='Root dir containing NPZ files')
    parser.add_argument('--save-dir',     default='ai/data/t4_oracle',
