"""Show mismatched samples with full details."""
import json
import subprocess
    ...
    for i, line in enumerate(f):
        ...
        for l in result.stdout.split('\n'):
            if l.startswith('{'):
                ...
                if abs(rust_score - expected) < 0.1:
                ...
                print()
                print(f"--- Ground Truth (score={expected}) ---")
                print(f"Top: {fmt(sample['solution']['top'])}")
                print(f"Mid: {fmt(sample['solution']['middle'])}")
                print(f"Bot: {fmt(sample['solution']['bottom'])}")
                print(f"Royalties: {sample['royalties']}, Stay: {sample.get('can_stay', False)}")
                print()
                print(f"--- Rust (score={rust_score:.0f}) ---")
                print(f"Top: {fmt_rust(p['top'])}")
                print(f"Mid: {fmt_rust(p['middle'])}")
                print(f"Bot: {fmt_rust(p['bottom'])}")
                print(f"TopR={p['top_royalty']} MidR={p['middle_royalty']} BotR={p['bottom_royalty']}")
                print(f"Stay: {p['can_stay']}, Bust: {p['is_bust']}")
                print()
                break
