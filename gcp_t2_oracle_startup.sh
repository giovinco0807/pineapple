...
LOG=/tmp/t2_oracle_startup.log
exec > >(tee -a "$LOG") 2>&1

PROJECT="ofc-solver-485418"
BUCKET="gs://ofc-solver-485418"
CODE_ARCHIVE="${BUCKET}/t2_oracle/code/ofc_t2_oracle_code.tar.gz"
MODEL_OBJECT="${BUCKET}/t2_oracle/models/t3_policyvalue_v2_best.pt"
RESULT_ROOT="${BUCKET}/t2_oracle/results/run_20260509"
...
MAX_SECONDS="${MAX_SECONDS:-13500}"
N_SAMPLES="${N_SAMPLES:-30}"
CHUNK_SIZE="${CHUNK_SIZE:-1000}"
STATES="${STATES:-100000000}"

echo "worker_id=${WORKER_ID} max_seconds=${MAX_SECONDS} n_samples=${N_SAMPLES} chunk_size=${CHUNK_SIZE}"
...
gcloud storage cp "$CODE_ARCHIVE" /tmp/ofc_t2_oracle_code.tar.gz
tar xzf /tmp/ofc_t2_oracle_code.tar.gz -C "$REMOTE_ROOT"
rm -f /tmp/ofc_t2_oracle_code.tar.gz
...
OUT_DIR="ai/data/t2_oracle_rule_worker_${WORKER_ID}"
GCS_OUT="${RESULT_ROOT}/worker_${WORKER_ID}"
mkdir -p "$OUT_DIR"

python -u ai/training/generate_t2_from_t0_jsonl_oracle.py \
  --source rule-random \
  ...
  2>&1 | tee "t2_worker_${WORKER_ID}.log"

gcloud storage cp "t2_worker_${WORKER_ID}.log" "${GCS_OUT}/t2_worker_${WORKER_ID}.log" || true
