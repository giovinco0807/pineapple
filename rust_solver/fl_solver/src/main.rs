...
fn compare_3_hands(a: &[Card], b: &[Card]) -> i32 {
    let (rank_a, str_a) = evaluate_3_card(a);
    let (rank_b, str_b) = evaluate_3_card(b);
    if (rank_a as u8) != (rank_b as u8) {
        return if (rank_a as u8) > (rank_b as u8) { 1 } else { -1 };
    }
    if str_a > str_b { 1 } else if str_a < str_b { -1 } else { 0 }
...
fn score_vs_opponent(
    ...
    // Line comparison
    let top_win = compare_3_hands(fl_top, &opp.top);
    let mid_win = compare_5_hands(fl_mid, &opp.middle);
    let bot_win = compare_5_hands(fl_bot, &opp.bottom);

    let line_score = top_win + mid_win + bot_win;
    let scoop = if line_score == 3 { 3 } else if line_score == -3 { -3 } else { 0 };

    // Opponent royalties
    let opp_top_roy = get_top_royalty(&opp.top);
    let opp_mid_roy = get_middle_royalty(&opp.middle);
    let opp_bot_roy = get_bottom_royalty(&opp.bottom);

    let royalty_diff = (fl_top_roy + fl_mid_roy + fl_bot_roy) - (opp_top_roy + opp_mid_roy + opp_bot_roy);

    let mut score = (line_score + scoop + royalty_diff) as f64;
    ...
    for bot_idx in &bot_combos {
        ...
        for mid_idx in remaining.iter().copied().combinations(5) {
            ...
            for top_idx in after_mid.iter().copied().combinations(3) {
                ...
                if !is_valid_placement(&top_c, &mid_c, &bot_c) { continue; }

                let r = get_top_royalty(&top_c) + get_middle_royalty(&mid_c) + get_bottom_royalty(&bot_c);
    ...
    for bot_idx in &bot_combos {
        ...
        for mid_idx in remaining.iter().copied().combinations(5) {
            ...
            for top_idx in after_mid.iter().copied().combinations(3) {
                ...
                let top_roy = get_top_royalty(&top_c);
                let mid_roy = get_middle_royalty(&mid_c);
                let bot_roy = get_bottom_royalty(&bot_c);
                let total_roy = top_roy + mid_roy + bot_roy;

                if total_roy < threshold { continue; }

                let can_stay = check_fl_stay(&top_c, &mid_c, &bot_c);
                let score = score_vs_opponent(
                    &top_c, &mid_c, &bot_c, opp,
