...
fn evaluate_terminal(board: &Board) -> f64 {
    ...
    let fl_bonus = if fl_q {
        match fl_c { 14 => 11.0, 15 => 12.0, 16 => 13.0, 17 => 14.0, _ => 0.0 }
    } else { 0.0 };
    total + fl_bonus
}
...
fn generate_one_hand(
    ...
    if t0_actions.is_empty() { return vec![]; }
    let t0_idx = rng.gen_range(0..t0_actions.len());
    let t0_action = &t0_actions[t0_idx];
    ...
    for s in 0..n1 {
        let mut deal_rng = StdRng::seed_from_u64(seed.wrapping_add((s * 9999 + 7) as u64));
        let deal = sample_3(&remaining_after_t0, &mut deal_rng);
        let rest = remove_dealt(&remaining_after_t0, &deal);
...
struct Cli {
    /// Number of T0 hands to generate
...
fn main() {
    ...
    let total_records = cli.n_hands * cli.n1;
    ...
    eprintln!("  Total T1 records: ~{}", total_records);
    ...
    let all_records: Vec<T1Record> = (0..cli.n_hands)
        .into_par_iter()
        .flat_map(|i| {
