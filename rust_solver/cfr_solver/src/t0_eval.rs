...
/// Evaluate all actions at the current turn using imperfect information (nested MC).
pub fn evaluate_turn_imperfect(
    board: &CompactBoard,
    hand: &[Card; 3],
    known_cards: &[Card],
    turn_index: usize, // 1=T1, 2=T2, 3=T3, 4=T4
    n_samples: usize,
    seed: u64,
    nesting: &[usize; 3],
) -> Vec<TurnActionResult> {
    ...
    if turn_index >= 4 {
        let results: Vec<TurnActionResult> = actions.iter().map(|&(discard_idx, row0, row1)| {
            ...
            let ev = score_final_board(&cb);
            let discard_str = card_to_string(&hand[discard_idx]);
            let desc = format!("{}→{:?}, {}→{:?}", card_to_string(&hand[kept0]), row0, card_to_string(&hand[kept1]), row1);
            TurnActionResult { discard: discard_str, placement_desc: desc, ev }
        }).collect();
        return sort_results(results);
    }
    ...
    let scores: Vec<f64> = (0..total_tasks)
        ...
        .map(|task_id| {
            ...
            // `turn` in estimate_future_ev is 0-indexed after T0 (0=T1, 1=T2, 2=T3, 3=T4)
            // Here `turn_index` is 1-indexed (1=T1, 2=T2, 3=T3, 4=T4)
            // The next turn to evaluate is `turn_index` (since 1-indexed turn_index is equivalent to 0-indexed next turn)
            estimate_future_ev(&cb, &remaining, turn_index, nesting, &mut rng)
        })
        .collect();
...
pub fn run_turn_batch_imperfect(input_path: &str, n_samples: usize, output_path: &str, seed: u64, nesting: &[usize; 3]) {
    ...
    let remaining_hands = n_hands - completed;
    println!("=== Turn Batch Imperfect Evaluation ===");
    println!("Input: {} | Hands: {} | Samples/hand: {} | Nesting: {:?}", input_path, n_hands, n_samples, nesting);
    println!("Output: {}", output_path);
    if completed > 0 {
        println!("Resuming from hand #{} ({} already done)", completed + 1, completed);
    ...
    let file = OpenOptions::new().create(true).append(true).open(output_path).unwrap();
    let file_mutex = Mutex::new(file);
    let start = std::time::Instant::now();
    let processed_count = AtomicUsize::new(0);

    (completed..n_hands).into_par_iter().for_each(|hand_idx| {
        let entry = &entries[hand_idx];
        let turn = entry["turn"].as_u64().unwrap_or(1) as usize;
        ...
        // Evaluate the turn!
        let results = evaluate_turn_imperfect(&board, &hand, &known, turn, n_samples, eval_seed, nesting);
        ...
        {
            let mut out_file = file_mutex.lock().unwrap();
            writeln!(out_file, "{}", json).expect("Failed to write");
