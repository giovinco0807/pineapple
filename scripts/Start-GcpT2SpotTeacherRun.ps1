param(
    ...
    [string]$RunName = ("regular-t2-" + (Get-Date -Format "yyyyMMdd-HHmmss")),
...
$turn3ModelPath = Join-Path $repoRoot $Turn3Model
if (-not (Test-Path $turn3ModelPath)) {
    throw "Turn3Model not found: $Turn3Model"
}
...
$packagePath = Join-Path $runDir "ofc_regular_t2_source.zip"
$startupPath = Join-Path $runDir "startup_t2_spot.sh"
$manifestPath = Join-Path $runDir "manifest.json"
$sourceUri = "gs://$Bucket/runs/$RunName/source/ofc_regular_t2_source.zip"
$startupUri = "gs://$Bucket/runs/$RunName/source/startup_t2_spot.sh"
...
Copy-Item -LiteralPath $turn3ModelPath -Destination (Join-Path $modelPackageDir "turn3_stage6.pkl")
...
WORK_DIR="/opt/ofc-regular-t2"
sudo rm -rf "$WORK_DIR"
sudo mkdir -p "$WORK_DIR"
sudo chown "$CURRENT_UID:$CURRENT_GID" "$WORK_DIR"
log "download source package $SOURCE_URI"
gcloud storage cp "$SOURCE_URI" /tmp/ofc_regular_t2_source.zip
unzip -q /tmp/ofc_regular_t2_source.zip -d "$WORK_DIR"
cd "$WORK_DIR"
...
STATUS_DIR="/tmp/ofc-t2-status"
SHARD_DIR="/tmp/ofc-t2-shards"
...
for (( shard=START_SHARD; shard<TOTAL_SHARDS; shard+=VM_COUNT )); do
  shard_name="$(printf "t2_%06d.jsonl" "$shard")"
  ...
  set +e
  python -B -m ofc_regular.t2_teacher_data \
  ...
  if [[ "$exit_code" -eq 0 && "$lines" -eq "$SHARD_SAMPLES" ]]; then
    ...
    cat > "$status_path" <<EOF
{"run_name":"$RUN_NAME","phase":"t2","shard":$shard,"status":"complete","instance":"$INSTANCE_NAME","zone":"$ZONE","seed":$seed,"samples":$SHARD_SAMPLES,"future_samples":$FUTURE_SAMPLES,"action_batch_size":$ACTION_BATCH_SIZE,"lines":$lines,"elapsed_seconds":$elapsed,"started_at":"$started_at","finished_at":"$finished_at","output":"$remote"}
EOF
    gcloud storage cp "$status_path" "$status_remote" || true
  else
    log "failed shard=$shard exit=$exit_code lines=$lines expected=$SHARD_SAMPLES"
    cat > "$status_path" <<EOF
{"run_name":"$RUN_NAME","phase":"t2","shard":$shard,"status":"failed","instance":"$INSTANCE_NAME","zone":"$ZONE","seed":$seed,"samples":$SHARD_SAMPLES,"future_samples":$FUTURE_SAMPLES,"action_batch_size":$ACTION_BATCH_SIZE,"lines":$lines,"exit_code":$exit_code,"elapsed_seconds":$elapsed,"started_at":"$started_at","finished_at":"$finished_at"}
EOF
    gcloud storage cp "$status_path" "$status_remote" || true
    exit "$exit_code"
  fi
