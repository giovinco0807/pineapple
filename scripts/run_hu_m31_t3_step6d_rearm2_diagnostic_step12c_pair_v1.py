...
This is a new authorization/identity boundary, not a retry of the terminal
Step12b execution.  The old runner and audit tree remain unchanged.
"""
...
EXECUTION_CONFIRMATION = "EXECUTE_STEP12C_DIRECT_V2_EXACT_PAIR_ATTEMPT0"
RUNNER_MODULE_NAME = "_hu_m31_t3_step12b_protocol_v2_runner"
FINAL_SCHEMA = "hu_m31_t3_step6d_step12c_pair_v1_final"
FAILURE_SCHEMA = "hu_m31_t3_step6d_step12c_pair_v1_failure"
...
def dry_run_receipt(
    ...
    return fresh_identity.seal(
        {
            "schema": "hu_m31_t3_step6d_step12c_pair_v1_dry_run",
...
def _terminal_tree_unchanged(before: dict[str, Any]) -> dict[str, Any]:
    after = fresh_identity.terminal_tree_snapshot()
    if after != before:
        raise RuntimeError("terminal Step12b artifact tree changed")
    return after
...
def _write_wrapper_failure(
    *,
    ...
    failure_stage: str,
    exception_type: str,
    terminal_tree: dict[str, Any],
) -> None:
    if not output_root.is_dir() or output_root.is_symlink():
        return
    path = output_root / "STEP12C_FAILURE.json"
    if path.exists() or path.is_symlink():
        return
    body = fresh_identity.seal(
        {
            "schema": FAILURE_SCHEMA,
            "status": "step12c_stopped_without_retry",
...
def execute_once(
    *, prepared: Any, output_root: str | Path
    ...
    step12b._IMMUTABLE_ROOTS.add(
        fresh_identity.TERMINAL_OUTPUT_ROOT.resolve()
    )
    ...
    try:
        ...
        terminal_after = _terminal_tree_unchanged(terminal_before)
        final = fresh_identity.seal(
            {
                "schema": FINAL_SCHEMA,
                "status": "step12c_pair_received_cleaned_and_disjoint",
                "fresh_identity_receipt_sha256": fresh["receipt_sha256"],
                ...
                "terminal_tree_before": terminal_before,
                "terminal_tree_after": terminal_after,
                "terminal_tree_unchanged": True,
                "attempt_index": 0,
                "attempt1_authorized": False,
                "third_vm_authorized": False,
                "automatic_retry_performed": False,
        ...
        step12b._exclusive_write_json(root / "STEP12C_FINAL.json", final)
        return final
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Step12c fresh-identity exact pair canary"
    )
    parser.add_argument(
        "--execute",
        action="store_true",
    ...
    parser.add_argument(
        "--confirm",
        default="",
        help="exact Step12c execution confirmation",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=fresh_identity.EXPECTED_OUTPUT_ROOT,
        help="must be the exact fresh Step12c output root",
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.execute and args.confirm != EXECUTION_CONFIRMATION:
        raise PermissionError(
            "exact Step12c execution confirmation is missing"
        )
