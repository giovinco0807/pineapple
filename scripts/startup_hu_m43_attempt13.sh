...
WORK=/opt/attempt13
RESULT=/var/lib/attempt13-result
VENV=/opt/attempt13-venv
LOG=/var/log/attempt13-worker.log
...
if [[ "$BATCH" != 1 && "$MODE" != preflight ]]; then
  echo 'scalar child selectors are allowed only in Attempt13 preflight' >&2
  exit 64
fi
...
RUN_ARGS=(
  ...
  --run-id "$RUN_ID" --plan configs/hu_joint_policy_m43_attempt13.json
...
/usr/bin/time -v -o "$RESULT/time.txt" \
  python -m ofc_regular.run_hu_m43_attempt13 "${RUN_ARGS[@]}" \
...
d={'schema':'hu_m43_attempt13_done_v1','status':'complete','run_name':m['run_name'],
