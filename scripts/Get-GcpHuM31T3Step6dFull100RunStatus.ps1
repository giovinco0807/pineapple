...
if (
    ...
    throw "Full100 status target differs from the frozen authorization"
}
...
try {
    & python -m ofc_regular.hu_m31_t3_step6d_full100_spot_v1 status `
        --run-dir ([IO.Path]::GetFullPath($RunDir)) `
        --project $ProjectId --bucket $Bucket
    if ($LASTEXITCODE -ne 0) {
        throw "Full100 status command failed ($LASTEXITCODE)"
    }
}
finally {
