"""T1 gate phase B: price each position's full 27-action fan, champion-centric.

Differences from the T0 version, all forced by the engine: restrict_action_keys
is plumbed only into evaluate_t0, so every batch here scores the whole fan --
narrowing would not save anything.  And the elimination module's noise constant
was measured at T0, so the stop rule uses each position's own batch-to-batch
scatter instead: after three batches, stop when the champion-vs-leader paired
gap clears 4 empirical standard errors either way, else run to the budget.

Worktree discipline and asserts identical to gate2_phase_b.
"""
...
def observation_dict(record: dict[str, Any], fl_ev: float = 9.6) -> dict[str, Any]:
    return {
        ...
        "street": "T1", "seat": record["seat"], "to_act_order": record["seat"],
...
def champ_key_of(record: dict, placements: dict[str, list]) -> str | None:
...
def fate(batch_scores: list[dict[str, float]], champ: str) -> dict[str, Any]:
    """Champion-vs-field verdict from per-batch shared-world scores."""
...
def main() -> int:
    ...
    parser.add_argument("--dir", required=True,
                        help="seat directory holding positions.jsonl; batches go under it")
    parser.add_argument("--worker", type=int, default=0)
    parser.add_argument("--workers", type=int, default=1)
    parser.add_argument("--samples", type=int, default=256)
    parser.add_argument("--min-batches", type=int, default=3)
    parser.add_argument("--max-batches", type=int, default=8)
    parser.add_argument("--seed-base", type=int, default=400_000_000)
    ...
    print(f"T1-{seat} worker {args.worker}/{args.workers}: {len(mine)} positions, "
    ...
    for record in mine:
        ...
        while True:
            ...
            if number == 0 and not model_rank:
                scored = m3.evaluate_request({
                    ...
                    "config": {f"learned_t1_{seat}_model_path": weights[f"t1_{seat}"][0],
                               f"learned_t1_{seat}_model_sha256": weights[f"t1_{seat}"][1]},
                }, library=library)
            ...
            config = JointExactConfig(
                ...
                run_id=f"t1gate-{seat}-p{index}-b{number}",
                **pins)
            wall = time.time()
            result = m3.evaluate_t1(observation, config=config, library=library)
        ...
        done_mark.write_text(json.dumps(summary, indent=1), encoding="utf-8")
        if verdict:
            ...
            print(f"DONE p{index:03d}: {state}/{firm} gap {verdict['gap']:+.3f} "
                  f"se {verdict['se'] if verdict['se'] else 0:.3f} "
