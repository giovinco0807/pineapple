"""T3 gate phase A: on-policy T2 positions for both seats, plus champion picks.

Same replay as the T1 version, two streets deeper.  From T1 on, each seat
...
def main() -> int:
    ...
    for i in range(args.positions):
        ...
        # T0 through T2 for both seats, champion play, play_ai dealing order.
        ...
        p1, p1_d2, _ = act(p1, p0, deck[19:22], p1_dead, 2, "second")
        p1_dead.append(p1_d2)

        if args.seat == "first":
            hero_board, hero_dead = p0, p0_dead
            opp_board, opp_discards = p1, 2
            dealt = deck[22:25]
        else:
            p0, p0_d3, _ = act(p0, p1, deck[22:25], p0_dead, 3, "first")
            p0_dead.append(p0_d3)
            hero_board, hero_dead = p1, p1_dead
            opp_board, opp_discards = p0, 3
            dealt = deck[25:28]

        champ_rows, champ_discard, champ_action = act(
            dict(hero_board), opp_board, dealt, hero_dead, 3, args.seat)
    ...
    print(f"wrote {len(records)} T3-{args.seat} positions -> {args.out}")
    return 0
