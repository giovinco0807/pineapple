"""T3 gate phase B: the T1 version one street deeper.

Deltas from t1_gate_b.py: street T3, evaluate_t3, the t2 models for the model
...
def observation_dict(record: dict[str, Any], fl_ev: float = 9.6) -> dict[str, Any]:
    return {
        ...
        "street": "T3", "seat": record["seat"], "to_act_order": record["seat"],
...
def main() -> int:
    ...
    parser.add_argument("--seed-base", type=int, default=600_000_000)
    ...
    print(f"T3-{seat} worker {args.worker}/{args.workers}: {len(mine)} positions, "
    ...
    for record in mine:
        ...
        while True:
            ...
            if number == 0 and not model_rank:
                scored = m3.evaluate_request({
                    ...
                    "config": {f"learned_t3_{seat}_model_path": weights[f"t3_{seat}"][0],
                               f"learned_t3_{seat}_model_sha256": weights[f"t3_{seat}"][1]},
                }, library=library)
            ...
            config = JointExactConfig(
                ...
                run_id=f"t3gate-{seat}-p{index}-b{number}",
                **pins)
            wall = time.time()
            result = m3.evaluate_t3(observation, config=config, library=library)
        ...
        done_mark.write_text(json.dumps({
            "index": index, "seat": seat, "street": "T3",
