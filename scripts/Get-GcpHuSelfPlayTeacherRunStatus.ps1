...
$expectedLines = 0
if ($manifest -and $manifest.total_samples) {
...
[pscustomobject]@{
    ...
    completed_lines_estimate = if ($manifest -and $manifest.shard_samples) { $completedShardNumbers.Count * [int]$manifest.shard_samples } else { 0 }
