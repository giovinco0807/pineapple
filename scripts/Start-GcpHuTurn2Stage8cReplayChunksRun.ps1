param(
    ...
    [string]$RunName = ("regular-hu-t2-stage8c-replay-chunks-" + (Get-Date -Format "yyyyMMdd-HHmmss")),
    [string]$InputJsonl = "outputs\evals\hu_turn2_stage8c_c4_rank45_risk035_hard_negatives\topk_all_fired_deduped.jsonl",
    [int]$FutureSamples = 512,
    [int]$ChunkSize = 100,
    [int]$StartOffset = 0,
    [int]$TotalRows = 0,
    [int]$ReplaySeed = 2026061509,
    [int]$VmCount = 10,
...
if (-not (Test-Path (Join-Path $repoRoot $InputJsonl))) { throw "InputJsonl not found: $InputJsonl" }
$resolvedInput = (Resolve-Path -LiteralPath (Join-Path $repoRoot $InputJsonl)).Path
...
if ($totalShards -le 0) { throw "No replay rows selected" }
...
if ($DryRun) {
    [pscustomobject]@{
        ...
        bucket = $Bucket
        phase = "hu_t2_stage8c_replay_chunks"
...
$packagePath = Join-Path $runDir "ofc_regular_hu_t2_stage8c_replay_chunks_source.zip"
$startupPath = Join-Path $runDir "startup_hu_t2_stage8c_replay_chunks_spot.sh"
$manifestPath = Join-Path $runDir "manifest.json"
$shardManifestPath = Join-Path $runDir "shards_manifest.jsonl"
$sourceUri = "gs://$Bucket/runs/$RunName/source/ofc_regular_hu_t2_stage8c_replay_chunks_source.zip"
$startupUri = "gs://$Bucket/runs/$RunName/source/startup_hu_t2_stage8c_replay_chunks_spot.sh"
...
$replayInputDir = Join-Path $packageDir "replay_input"
New-Item -ItemType Directory -Force -Path $replayInputDir | Out-Null
Copy-Item -LiteralPath $resolvedInput -Destination (Join-Path $replayInputDir "input.jsonl")
...
WORK_DIR="/opt/ofc-regular-hu-t2-stage8c-replay"
sudo rm -rf "$WORK_DIR"
sudo mkdir -p "$WORK_DIR"
sudo chown "$CURRENT_UID:$CURRENT_GID" "$WORK_DIR"
gcloud storage cp "$SOURCE_URI" /tmp/ofc_regular_hu_t2_stage8c_replay_chunks_source.zip
unzip -q /tmp/ofc_regular_hu_t2_stage8c_replay_chunks_source.zip -d "$WORK_DIR"
cd "$WORK_DIR"
...
STATUS_DIR="/tmp/ofc-hu-t2-stage8c-replay-status"
RESULT_DIR="/tmp/ofc-hu-t2-stage8c-replay-results"
mkdir -p "$STATUS_DIR" "$RESULT_DIR"

log "start replay chunk loop run=$RUN_NAME start=$START_SHARD stride=$VM_COUNT total_shards=$TOTAL_SHARDS"
for (( shard=START_SHARD; shard<TOTAL_SHARDS; shard+=VM_COUNT )); do
  ...
  replay_offset="$(json_field "$spec" replay_offset)"
  replay_count="$(json_field "$spec" replay_count)"
  ...
  log "run shard=$shard replay_offset=$replay_offset replay_count=$replay_count future=$FUTURE_SAMPLES"

  set +e
  python -B -m ofc_regular.replay_hu_turn2_stage8b_topk_hard_negatives \
    --input-jsonl replay_input/input.jsonl \
  ...
  if [[ "$exit_code" -eq 0 ]]; then
    ...
    cat > "$status_path" <<EOF
{"run_name":"$RUN_NAME","phase":"hu_t2_stage8c_replay_chunks","shard":$shard,"replay_offset":$replay_offset,"replay_count":$replay_count,"future_samples":$FUTURE_SAMPLES,"t3_continuation":"$T3_CONTINUATION","status":"complete","instance":"$INSTANCE_NAME","zone":"$ZONE","elapsed_seconds":$elapsed,"started_at":"$started_at","finished_at":"$finished_at","output":"gs://${BUCKET}/runs/${RUN_NAME}/results/${output_prefix}"}
EOF
    ...
    cat > "$status_path" <<EOF
{"run_name":"$RUN_NAME","phase":"hu_t2_stage8c_replay_chunks","shard":$shard,"replay_offset":$replay_offset,"replay_count":$replay_count,"future_samples":$FUTURE_SAMPLES,"t3_continuation":"$T3_CONTINUATION","status":"failed","instance":"$INSTANCE_NAME","zone":"$ZONE","exit_code":$exit_code,"elapsed_seconds":$elapsed,"started_at":"$started_at","finished_at":"$finished_at","log":"gs://${BUCKET}/runs/${RUN_NAME}/logs/${output_prefix}.log"}
EOF
    gcloud storage cp "$status_path" "$status_remote" >/dev/null || true
    exit "$exit_code"
  fi
