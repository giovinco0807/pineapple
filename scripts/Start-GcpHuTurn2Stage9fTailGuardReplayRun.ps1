param(
    ...
    [string]$RunName = ("regular-hu-t2-stage9f-tail-guard-replay-" + (Get-Date -Format "yyyyMMdd-HHmmss")),
    [string]$Targets = "outputs\training\hu_turn2_stage9f_cse2_csemax2_tail_guard_targets_10000\stage9f_tail_guard_targets.jsonl",
    [int]$McSamples = 512,
    [int]$ChunkSize = 8,
    [int]$StartOffset = 0,
    [int]$TotalRows = 0,
...
if ($McSamples -le 0) { throw "McSamples must be positive" }
...
if (-not (Test-Path (Join-Path $repoRoot $Targets))) { throw "Targets not found: $Targets" }
$resolvedTargets = (Resolve-Path -LiteralPath (Join-Path $repoRoot $Targets)).Path
$sourceRows = Get-LineCount $resolvedTargets
...
$totalShards = if ($rowsToProcess -gt 0) { [int][Math]::Ceiling($rowsToProcess / [double]$ChunkSize) } else { 0 }
if ($totalShards -le 0) { throw "No targets selected" }
...
for ($shardIndex = 0; $shardIndex -lt $totalShards; $shardIndex += 1) {
    $localOffset = $StartOffset + $shardIndex * $ChunkSize
    $remaining = $rowsToProcess - $shardIndex * $ChunkSize
    $count = [Math]::Min($ChunkSize, $remaining)
    $shardSpecs.Add([ordered]@{
        shard = $shardIndex
        target_offset = $localOffset
        target_count = $count
        output_prefix = ("shard_{0:D3}_offset{1:D4}_count{2:D3}" -f $shardIndex, $localOffset, $count)
    })
}
...
if ($DryRun) {
    [pscustomobject]@{
        ...
        bucket = $Bucket
        phase = "hu_t2_stage9f_tail_guard_replay"
        targets = $Targets
...
$packagePath = Join-Path $runDir "ofc_regular_hu_t2_stage9f_tail_guard_replay_source.zip"
$startupPath = Join-Path $runDir "startup_hu_t2_stage9f_tail_guard_replay_spot.sh"
$manifestPath = Join-Path $runDir "manifest.json"
$shardManifestPath = Join-Path $runDir "shards_manifest.jsonl"
$sourceUri = "gs://$Bucket/runs/$RunName/source/ofc_regular_hu_t2_stage9f_tail_guard_replay_source.zip"
$startupUri = "gs://$Bucket/runs/$RunName/source/startup_hu_t2_stage9f_tail_guard_replay_spot.sh"
...
$targetInputDir = Join-Path $packageDir "replay_input"
New-Item -ItemType Directory -Force -Path $targetInputDir | Out-Null
Copy-Item -LiteralPath $resolvedTargets -Destination (Join-Path $targetInputDir "targets.jsonl")
...
STAGE3_FEATURE_ENCODER_MODE="$(meta STAGE3_FEATURE_ENCODER_MODE)"
...
WORK_DIR="/opt/ofc-regular-hu-t2-stage9f-tail-guard-replay"
sudo rm -rf "$WORK_DIR"
sudo mkdir -p "$WORK_DIR"
sudo chown "$CURRENT_UID:$CURRENT_GID" "$WORK_DIR"
gcloud storage cp "$SOURCE_URI" /tmp/ofc_regular_hu_t2_stage9f_tail_guard_replay_source.zip
unzip -q /tmp/ofc_regular_hu_t2_stage9f_tail_guard_replay_source.zip -d "$WORK_DIR"
cd "$WORK_DIR"
...
STATUS_DIR="/tmp/ofc-hu-t2-stage9f-tail-guard-replay-status"
RESULT_DIR="/tmp/ofc-hu-t2-stage9f-tail-guard-replay-results"
mkdir -p "$STATUS_DIR" "$RESULT_DIR"

log "start Stage9f tail-guard replay loop run=$RUN_NAME start=$START_SHARD stride=$VM_COUNT total_shards=$TOTAL_SHARDS"
for (( shard=START_SHARD; shard<TOTAL_SHARDS; shard+=VM_COUNT )); do
  ...
  target_offset="$(json_field "$spec" target_offset)"
  target_count="$(json_field "$spec" target_count)"
  ...
  log "run shard=$shard target_offset=$target_offset target_count=$target_count mc=$MC_SAMPLES"

  set +e
  python -B -m ofc_regular.replay_hu_turn2_stage9f_tail_guard_targets \
    --targets replay_input/targets.jsonl \
    ...
    --stage3-feature-encoder-mode "$STAGE3_FEATURE_ENCODER_MODE" \
    --resume \
    --progress-every 1 > "$log_path" 2>&1
  exit_code=$?
  set -e
  ...
  if [[ "$exit_code" -eq 0 ]]; then
    ...
    cat > "$status_path" <<EOF
{"run_name":"$RUN_NAME","phase":"hu_t2_stage9f_tail_guard_replay","shard":$shard,"target_offset":$target_offset,"target_count":$target_count,"mc_samples":$MC_SAMPLES,"status":"complete","instance":"$INSTANCE_NAME","zone":"$ZONE","elapsed_seconds":$elapsed,"started_at":"$started_at","finished_at":"$finished_at","output":"gs://${BUCKET}/runs/${RUN_NAME}/results/${output_prefix}"}
EOF
    ...
    cat > "$status_path" <<EOF
{"run_name":"$RUN_NAME","phase":"hu_t2_stage9f_tail_guard_replay","shard":$shard,"target_offset":$target_offset,"target_count":$target_count,"mc_samples":$MC_SAMPLES,"status":"failed","instance":"$INSTANCE_NAME","zone":"$ZONE","exit_code":$exit_code,"elapsed_seconds":$elapsed,"started_at":"$started_at","finished_at":"$finished_at","log":"gs://${BUCKET}/runs/${RUN_NAME}/logs/${output_prefix}.log"}
EOF
    gcloud storage cp "$status_path" "$status_remote" >/dev/null || true
    exit "$exit_code"
  fi
