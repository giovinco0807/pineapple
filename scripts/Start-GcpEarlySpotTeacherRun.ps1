param(
    ...
    [string]$RunName = ("regular-early-" + (Get-Date -Format "yyyyMMdd-HHmmss")),
    [ValidateSet("turn1", "opening")]
    [string]$Phase = "turn1",
    [int]$TotalSamples = 10000,
    [int]$ShardSamples = 20,
    [int]$FutureSamples = 64,
    [int]$ActionBatchSize = 0,
    [int]$VmCount = 20,
    [string]$MachineType = "e2-highcpu-2",
    [string[]]$Zones = @(
        "asia-northeast1-a",
        "asia-northeast1-b",
        "asia-northeast1-c",
    ...
    [int]$BootDiskGb = 30,
    [string]$DownstreamModel = "models/turn2_stage8.pkl",
    [string[]]$StartShards = @(),
...
if ($ActionBatchSize -lt -1) { throw "ActionBatchSize must be -1 or greater" }
...
$downstreamModelPath = Join-Path $repoRoot $DownstreamModel
if (-not (Test-Path $downstreamModelPath)) {
    throw "DownstreamModel not found: $DownstreamModel"
}

$shardPrefix = if ($Phase -eq "turn1") { "t1" } else { "opening" }
$downstreamModelName = Split-Path -Leaf $DownstreamModel
...
$packagePath = Join-Path $runDir "ofc_regular_early_source.zip"
$startupPath = Join-Path $runDir "startup_early_spot.sh"
$manifestPath = Join-Path $runDir "manifest.json"
$sourceUri = "gs://$Bucket/runs/$RunName/source/ofc_regular_early_source.zip"
$startupUri = "gs://$Bucket/runs/$RunName/source/startup_early_spot.sh"
...
Copy-Item -LiteralPath $downstreamModelPath -Destination (Join-Path $modelPackageDir $downstreamModelName)
...
WORK_DIR="/opt/ofc-regular-early"
sudo rm -rf "$WORK_DIR"
sudo mkdir -p "$WORK_DIR"
sudo chown "$CURRENT_UID:$CURRENT_GID" "$WORK_DIR"
log "download source package $SOURCE_URI"
gcloud storage cp "$SOURCE_URI" /tmp/ofc_regular_early_source.zip
unzip -q /tmp/ofc_regular_early_source.zip -d "$WORK_DIR"
cd "$WORK_DIR"
...
if [[ "${DOWNSTREAM_MODEL##*.}" == "pt" ]]; then
  log "install cpu torch for downstream model $DOWNSTREAM_MODEL"
...
STATUS_DIR="/tmp/ofc-early-status"
SHARD_DIR="/tmp/ofc-early-shards"
mkdir -p "$STATUS_DIR" "$SHARD_DIR"

log "start shard loop run=$RUN_NAME phase=$PHASE start=$START_SHARD stride=$VM_COUNT total_shards=$TOTAL_SHARDS shard_samples=$SHARD_SAMPLES future=$FUTURE_SAMPLES"
for (( shard=START_SHARD; shard<TOTAL_SHARDS; shard+=VM_COUNT )); do
  shard_name="$(printf "%s_%06d.jsonl" "$SHARD_PREFIX" "$shard")"
  ...
  set +e
  python -B -m ofc_regular.early_teacher_data \
    --phase "$PHASE" \
    --downstream-model "models/${DOWNSTREAM_MODEL}" \
    --samples "$SHARD_SAMPLES" \
    --future-samples "$FUTURE_SAMPLES" \
    --action-batch-size "$ACTION_BATCH_SIZE" \
    --seed "$seed" \
    --output "$local_path"
  exit_code=$?
  set -e
  ...
  if [[ "$exit_code" -eq 0 && "$lines" -eq "$SHARD_SAMPLES" ]]; then
    ...
    cat > "$status_path" <<EOF
{"run_name":"$RUN_NAME","phase":"$PHASE","shard":$shard,"status":"complete","instance":"$INSTANCE_NAME","zone":"$ZONE","seed":$seed,"samples":$SHARD_SAMPLES,"future_samples":$FUTURE_SAMPLES,"action_batch_size":$ACTION_BATCH_SIZE,"lines":$lines,"elapsed_seconds":$elapsed,"started_at":"$started_at","finished_at":"$finished_at","output":"$remote"}
EOF
    gcloud storage cp "$status_path" "$status_remote" || true
  else
    log "failed shard=$shard exit=$exit_code lines=$lines expected=$SHARD_SAMPLES"
    cat > "$status_path" <<EOF
{"run_name":"$RUN_NAME","phase":"$PHASE","shard":$shard,"status":"failed","instance":"$INSTANCE_NAME","zone":"$ZONE","seed":$seed,"samples":$SHARD_SAMPLES,"future_samples":$FUTURE_SAMPLES,"action_batch_size":$ACTION_BATCH_SIZE,"lines":$lines,"exit_code":$exit_code,"elapsed_seconds":$elapsed,"started_at":"$started_at","finished_at":"$finished_at"}
EOF
    gcloud storage cp "$status_path" "$status_remote" || true
    exit "$exit_code"
  fi
...
foreach ($i in $workerIndices) {
    ...
    $metadata = @(
        ...
        "START_SHARD=$i",
        "SOURCE_URI=$sourceUri",
        "DOWNSTREAM_MODEL=$downstreamModelName",
        "PREDICTION_THREADS=$PredictionThreads",
