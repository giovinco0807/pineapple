...
Step12m is a new authorization/identity boundary, not a retry of the
terminal Step12b through Step12l executions.  All old runners and audit
trees remain unchanged.  Step12m runs after the external preflight token-barrier field fix was
instrumented to persist the underlying step11 error (code/status/operation),
so a set/readback failure now writes a diagnosable phase2 failure receipt.
It carries the deadline read retry and the SA create-readback poll.
Mutations are never retried.  Cloud execution additionally requires fresh
explicit user authorization outside this program.
"""
...
EXECUTION_CONFIRMATION = "EXECUTE_STEP12M_DIRECT_V2_EXACT_PAIR_ATTEMPT0"
RUNNER_MODULE_NAME = "_hu_m31_t3_step12b_protocol_v2_runner"
FINAL_SCHEMA = "hu_m31_t3_step6d_step12m_pair_v1_final"
FAILURE_SCHEMA = "hu_m31_t3_step6d_step12m_pair_v1_failure"
...
def dry_run_receipt(
    ...
    return fresh_identity.seal(
        {
            "schema": "hu_m31_t3_step6d_step12m_pair_v1_dry_run",
            "status": "step12m_offline_fresh_identity_no_cloud_adapter",
            ...
            "readonly_get_retry_contract": fresh[
                "readonly_get_retry_contract"
            ],
            "sa_create_readback_poll_contract": fresh[
...
def _terminal_trees_unchanged(before: dict[str, Any]) -> dict[str, Any]:
    ...
    if after != before:
        raise RuntimeError(
            "terminal Step12b through Step12l artifact tree changed"
        )
    return after
...
def _write_wrapper_failure(
    *,
    ...
    readonly_retry_events: list[dict[str, Any]],
    sa_readback_poll_events: list[dict[str, Any]],
) -> None:
    if not output_root.is_dir() or output_root.is_symlink():
        return
    path = output_root / "STEP12M_FAILURE.json"
    if path.exists() or path.is_symlink():
        return
    body = fresh_identity.seal(
        {
            "schema": FAILURE_SCHEMA,
            "status": "step12m_stopped_without_retry",
            "failure_stage": failure_stage,
            "exception_type": exception_type,
            "fresh_identity_receipt_sha256": fresh["receipt_sha256"],
            "terminal_trees": terminal_trees,
            "readonly_retry_event_count": len(readonly_retry_events),
            "readonly_retry_events": readonly_retry_events,
            "sa_readback_poll_event_count": len(sa_readback_poll_events),
            "sa_readback_poll_events": sa_readback_poll_events,
            "attempt_index": 0,
            "attempt1_authorized": False,
            "third_vm_authorized": False,
...
def execute_once(
    *, prepared: Any, output_root: str | Path
    ...
    for terminal_root in (
        fresh_identity.TERMINAL_STEP12B_OUTPUT_ROOT,
        fresh_identity.TERMINAL_STEP12C_OUTPUT_ROOT,
        fresh_identity.TERMINAL_STEP12D_OUTPUT_ROOT,
        fresh_identity.TERMINAL_STEP12E_OUTPUT_ROOT,
        fresh_identity.TERMINAL_STEP12F_OUTPUT_ROOT,
        fresh_identity.TERMINAL_STEP12G_OUTPUT_ROOT,
        fresh_identity.TERMINAL_STEP12H_OUTPUT_ROOT,
        fresh_identity.TERMINAL_STEP12I_OUTPUT_ROOT,
        fresh_identity.TERMINAL_STEP12J_OUTPUT_ROOT,
        fresh_identity.TERMINAL_STEP12K_OUTPUT_ROOT,
        fresh_identity.TERMINAL_STEP12L_OUTPUT_ROOT,
    ):
    ...
    try:
        ...
        retry_events = retry_admin.retry_events
        ...
        poll_events = polling_sa_admin.create_readback_poll_events
        ...
        final = fresh_identity.seal(
            {
                "schema": FINAL_SCHEMA,
                "status": "step12m_pair_received_cleaned_and_disjoint",
                "fresh_identity_receipt_sha256": fresh["receipt_sha256"],
                ...
                "readonly_retry_event_count": len(retry_events),
                "readonly_retry_events": list(retry_events),
                "sa_readback_poll_event_count": len(poll_events),
                "sa_readback_poll_events": list(poll_events),
        ...
        step12b._exclusive_write_json(root / "STEP12M_FINAL.json", final)
        return final
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Step12m fresh-identity exact pair canary"
    )
    parser.add_argument(
        "--execute",
        action="store_true",
    ...
    parser.add_argument(
        "--confirm",
        default="",
        help="exact Step12m execution confirmation",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=fresh_identity.EXPECTED_OUTPUT_ROOT,
        help="must be the exact fresh Step12m output root",
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.execute and args.confirm != EXECUTION_CONFIRMATION:
        raise PermissionError(
            "exact Step12m execution confirmation is missing"
        )
