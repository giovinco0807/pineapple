...
if ($manifestExists -contains $manifestUri) {
    ...
    if ($manifest.shard_prefix) {
        $shardPrefix = [string]$manifest.shard_prefix
    }
}

if (-not $MergedOutput) {
    $MergedOutput = "outputs/gcp_runs/$RunName/${shardPrefix}_teacher_merged.jsonl"
}

New-Item -ItemType Directory -Force -Path $DownloadDir | Out-Null
gcloud storage cp "$prefix/shards/*.jsonl" $DownloadDir --project $ProjectId | Out-Null

$fileFilter = "{0}_*.jsonl" -f $shardPrefix
$files = Get-ChildItem -LiteralPath $DownloadDir -Filter $fileFilter |
    Sort-Object Name

$shards = @()
$shardRegex = '^' + [regex]::Escape($shardPrefix) + '_(\d+)\.jsonl$'
foreach ($file in $files) {
    if ($file.Name -match $shardRegex) {
