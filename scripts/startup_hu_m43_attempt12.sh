...
WORK=/opt/attempt12
RESULT=/var/lib/attempt12-result
VENV=/opt/attempt12-venv
LOG=/var/log/attempt12-worker.log
...
if [[ "$BATCH" != 1 && "$MODE" != preflight ]]; then
  echo 'scalar child selectors are allowed only in Attempt12 preflight' >&2
  exit 64
fi
...
RUN_ARGS=(
  ...
  --run-id "$RUN_ID" --plan configs/hu_joint_policy_m43_attempt12.json
...
/usr/bin/time -v -o "$RESULT/time.txt" \
  python -m ofc_regular.run_hu_m43_attempt12 "${RUN_ARGS[@]}" \
...
d={'schema':'hu_m43_attempt12_done_v1','status':'complete','run_name':m['run_name'],
