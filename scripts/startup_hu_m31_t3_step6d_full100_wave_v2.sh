...
# One VM executes one source-isolated full100 wave-v2 job. The legacy archive
# below is frozen scientific content only; its launcher/authorization are never
# reused and this guest never creates another VM or job.
...
cleanup() {
  ...
  # Exact VM cleanup is controller-owned. Guest shutdown is deliberately
  # omitted because it can strand a TERMINATED instance and boot disk.
  exit "$code"
}
trap cleanup EXIT
...
# Commit sequence is always: temp -> fsync(file) -> rename -> fsync(parent).
atomic_commit_file() {
...
if destination.exists():
    ...
    if digest(temporary) != digest(destination):
...
# One custom metadata attribute carries the complete canonical bootstrap.
...
ACTUAL_ZONE="$(instance_meta zone | awk -F/ '{print $NF}')"
PROJECT_ID="$(project_meta project-id)"
...
if (
 ...
 or not re.fullmatch(r"regular-hu-m31-c02-f100wv2-[a-z0-9-]{8,48}",
                     value["run_name"])
 or not sha.fullmatch(value["execution_identity_sha256"])
 or any(not sha.fullmatch(value[field]) for field in (
   "wave_plan_sha256","attempt_ledger_sha256","resume_sha256",
   "observed_transition_digest","outer_manifest_sha256",
...
suffixes = {
    ...
    "startup": "/content/startup/startup_hu_m31_t3_step6d_full100_wave_v2.sh",
...
# The pinned Debian image guarantees the guest environment, not python3-venv.
# Use the same dated Debian snapshot that passed the tail-v2 cloud canary so
# runtime packages cannot drift between candidate and reference workers.
printf '%s\n' \
...
# Rehash and atomically extract every member of both closed-world archives,
# then bind the one selected job to the frozen plan and accepted native binary.
mapfile -t JOB < <(
python3 - "$JOB_ROOT_REAL" "$BOOTSTRAP" "$SCIENTIFIC_SOURCE_SHA256" \
  "$SCIENTIFIC_SOURCE_BYTES" "$SCIENTIFIC_MANIFEST_SHA256" \
  "$WAVE_PLAN_SHA256" "$WAVE_PLAN_OBJECT_SHA256" "$JOB_MANIFEST_SHA256" \
  "$WHEELHOUSE_SHA256" "$WHEELHOUSE_MANIFEST_SHA256" <<'PY'
import hashlib
import json
import os
import pathlib
...
(source_sha, source_bytes, science_manifest_sha, schedule_sha,
 wave_object_sha, job_sha, wheel_sha, wheel_manifest_sha) = sys.argv[3:11]
...
def load(path, label, allow_newline):
    raw = path.read_bytes()
    value = json.loads(raw.decode("ascii"))
    expected = {canonical(value)}
    if allow_newline:
        expected.add(canonical(value, True))
    if not isinstance(value, dict) or raw not in expected:
...
def atomic_write(path, raw):
    ...
    if os.path.commonpath((str(root), str(parent))) != str(root):
        raise SystemExit("archive extraction escaped owned root")
    cursor = path.parent
    while cursor != root:
        if cursor.is_symlink():
            raise SystemExit("archive extraction parent is a symlink")
...
if (digest(source) != source_sha or source.stat().st_size != int(source_bytes)
    or digest(science_manifest_path) != science_manifest_sha
    or digest(wave_path) != wave_object_sha or digest(job_path) != job_sha
    or digest(wheel_archive) != wheel_sha
    or digest(wheel_manifest_path) != wheel_manifest_sha):
    raise SystemExit("downloaded object hash/size binding changed")

manifest = load(science_manifest_path, "scientific manifest", True)
# wave_plan_v2.canonical_bytes writes one trailing LF.  The downloaded object
# is also SHA-bound by the outer manifest, so accepting the canonical LF form
# here preserves the exact producer/consumer contract.
wave = load(wave_path, "wave plan", True)
job = load(job_path, "job manifest", True)
wheel_manifest = load(wheel_manifest_path, "wheelhouse manifest", True)
if (manifest.get("schema") != "hu_m31_t3_step6d_full100_spot_package_v1"
    ...
    or manifest.get("source_entry_count") != len(manifest["source_entries"])
    or wave.get("schema") != "hu_m31_t3_step6d_full100_wave_plan_v2"
    or wave.get("schedule_sha256") != schedule_sha
    or wave.get("run_name") != bootstrap["run_name"]
    or wave.get("execution_identity_sha256") != bootstrap["execution_identity_sha256"]
    or wave.get("current_profile_changed") is not False
    or wave.get("cloud_launch_authorized") is not False):
    raise SystemExit("scientific/wave schema binding changed")
...
with zipfile.ZipFile(source) as archive:
    ...
    for info in infos:
        ...
        if (info.is_dir() or stat.S_ISLNK(mode)
            ...
            or type(record["bytes"]) is not int or record["bytes"] != info.file_size):
            raise SystemExit("scientific archive member contract changed")
        raw = archive.read(info)
        if hashlib.sha256(raw).hexdigest() != record["sha256"]:
            raise SystemExit("scientific archive member hash changed")
        atomic_write(science.joinpath(*relative.parts), raw)

required_roots = {f"frozen/full100_roots/hand_{i:03d}.json" for i in range(100)}
if not required_roots.issubset(entries):
    raise SystemExit("scientific archive lacks frozen full100 roots")
full_plan_path = science / "frozen/full100_plan.json"
full_plan = load(full_plan_path, "frozen full100 plan", True)
if (full_plan != wave.get("full100_plan")
    or digest(full_plan_path) != wave.get("full100_plan_sha256")
    or manifest.get("plan_sha256") != wave.get("full100_plan_sha256")
    or manifest.get("run_contract_digest") != wave.get("run_contract_digest")):
    raise SystemExit("inner frozen scientific plan changed")

job_keys = {"schema","run_contract","run_contract_digest",
            "source_role","work_hand_indices"}
if (set(job) != job_keys
    or job.get("schema") != "hu_m31_t3_step6d_performance_shard_manifest_v2"
    or job.get("source_role") != bootstrap["source_role"]
    or hashlib.sha256(canonical(job["run_contract"], True)).hexdigest()
       != job.get("run_contract_digest")
    or job.get("run_contract_digest") != wave.get("run_contract_digest")
    or not isinstance(job.get("work_hand_indices"), list)
    or len(job["work_hand_indices"]) != 10
    or len(set(job["work_hand_indices"])) != 10
    or any(type(i) is not int or i not in range(100)
           for i in job["work_hand_indices"])):
    raise SystemExit("selected runner job changed")
planned = {row["job_id"]: row for row in wave["full100_plan"]["jobs"]}
selected = planned.get(bootstrap["job_id"])
if (selected is None or selected["source_role"] != bootstrap["source_role"]
    or selected["work_hand_indices"] != job["work_hand_indices"]
    or selected["shard_manifest_sha256"] != job_sha):
    raise SystemExit("bootstrap selected an extra/unplanned job")

role = bootstrap["source_role"]
binding = manifest[f"accepted_{role}"]
library_relative = binding["package_path"]
library = science.joinpath(*safe_relative(library_relative).parts)
allocation_bytes = canonical(job["run_contract"]["allocation"], True)
if (digest(library) != binding["sha256"]
    or wave["runtime_binding"]["binary_sha256_by_role"][role] != binding["sha256"]
    or wave["runtime_binding"]["package_sha256"] != source_sha
    or wave["runtime_binding"]["allocation_digest"]
       != hashlib.sha256(allocation_bytes).hexdigest()
    or job["run_contract"]["allocation"]
       != {"workers":1,"rayon_threads_per_worker":16}):
    raise SystemExit("native/allocation binding changed")
...
if (wheel_manifest.get("schema") != "hu_m31_t3_step6d_perfdev_v2_wheelhouse_v1"
    ...
    or wheel_manifest.get("entries_sha256")
       != hashlib.sha256(canonical(wheel_entries, True)).hexdigest()):
...
with zipfile.ZipFile(wheel_archive) as archive:
    ...
    if names != sorted(expected_wheels) or len(names) != len(set(names)):
        raise SystemExit("wheelhouse archive missing/extra member")
    for name in names:
        relative = safe_relative(name)
        record = expected_wheels[name]
        ...
        atomic_write(wheels.joinpath(*relative.parts), raw)
requirements = science / "configs/hu_m43_attempt08_runtime_requirements.txt"
if digest(requirements) != wheel_manifest.get("requirements_sha256"):
    raise SystemExit("offline requirements hash changed")
for raw in requirements.read_text(encoding="utf-8").splitlines():
    line = raw.strip()
    if line and not line.startswith("--extra-index-url ") and "==" not in line:
        raise SystemExit("unpinned offline requirement")

print(role)
print(",".join(str(i) for i in job["work_hand_indices"]))
print(library_relative)
print(binding["sha256"])
print(job["run_contract_digest"])
print(wave["runtime_binding"]["package_sha256"])
...
python -m pip install --disable-pip-version-check --no-input --no-index \
  --find-links "$WHEELS" \
  -r "$SCIENCE/configs/hu_m43_attempt08_runtime_requirements.txt"
python -m pip check
...
for hand in "${WORK_HANDS[@]}"; do
  ...
  cp --no-preserve=mode,ownership,timestamps \
    "$SCIENCE/frozen/full100_roots/hand_$pad.json" "$tmp"
...
def exposes_hidden(value):
    forbidden = {
        "opponent_private_discards", "opponent_hidden_discards",
...
for hand in work:
    ...
    hand_value = json.loads(source_hand.read_text(encoding="utf-8"))
    if exposes_hidden(hand_value):
...
# The wave-v2 control contract hashes canonical JSON records with one trailing
# LF.  Keep the transport identity byte-for-byte aligned with
# wave_plan_v2.canonical_bytes; the DONE document itself remains newline-free.
...
value = {
    ...
    "observed_transition_digest": transition_digest,
    ...
    "worker_principal": principal, "work_hand_indices": work,
    "artifact_count": len(artifacts), "artifacts": artifacts,
    "runner_done_sha256": file_sha(runner_done_path),
...
# Read back every checkpoint before the sole commit marker. No metadata,
# hidden truth, log, or additional job object is published.
for hand in "${WORK_HANDS[@]}"; do
  printf -v pad '%03d' "$hand"
