...
$ReceiptSchema = "hu_m43_attempt13_population_spot_receipt_v1"
$AcceptanceStatusSchema = "hu_m43_attempt13_population_acceptance_status_v1"
...
if (-not (Test-Path -LiteralPath $manifestPath -PathType Leaf)) { throw "Frozen Attempt13 population manifest is missing: $manifestPath" }
...
if ($manifest.schema -ne $ManifestSchema -or $manifest.run_name -ne $RunName -or
    ...
    [long]$manifest.population_plan.seed -ne 250108071901 -or
    [long]$manifest.population_plan.seed_stride -ne 1000003 -or
    [int]$manifest.population_plan.paired_seeds_per_shard -ne 50 -or
    $manifest.runtime.model_schema -ne "hu_m43_attempt13_t1_second_distilled_selector_v1" -or
    $manifest.runtime.artifact_schema -ne "hu_m43_attempt13_t1_second_distilled_pickle_v1" -or
    $manifest.runtime.feature_schema -ne "hu_m43_attempt13_lambda_all_legal_public_infoset_features_v1" -or
    $manifest.runtime.head_schema -ne "hu_m43_attempt13_policy_delta_safe_tail_heads_v1" -or
    $manifest.runtime.action_score_mode -ne "attempt13_lambda_all_legal_distilled_safe_selector_v1" -or
    ...
    [int]$manifest.launch_preflight.all_reserved_registry_overlap_count -ne 0 -or
    $manifest.launch_preflight.profile_id -ne "stage20_m4_attempt13" -or
    (@($manifest.launch_preflight.population_namespace_bases) -join ",") -cne "250108071901,251108071901,252108071901,253108071901,254108071901,255108071901,256108071901" -or
    ...
    $manifest.heartbeat.schema -ne "hu_m43_attempt13_population_spot_status_v1" -or
    ...
    throw "Frozen Attempt13 population manifest identity/boundary mismatch"
}

$root = [System.IO.Path]::GetFullPath((Join-Path $repoRoot "outputs/hu_joint_policy/m43_attempt13_population"))
if ($OutputDir) {
...
if (-not $OutputDir.StartsWith($rootPrefix, [System.StringComparison]::OrdinalIgnoreCase)) {
    throw "OutputDir must stay under outputs/hu_joint_policy/m43_attempt13_population; refusing profile/current overwrite"
}
if (Test-Path -LiteralPath $OutputDir) {
    ...
    if (-not (Test-Path -LiteralPath $receiptPath -PathType Leaf) -or
        ...
        throw "Attempt13 population OutputDir exists without complete receipts; refusing overwrite"
    }
    ...
    if ($receipt.schema -ne $ReceiptSchema -or $receipt.status -ne "verified_and_merged" -or
        ...
        throw "Existing Attempt13 population receipt/hash chain is invalid"
    }
...
if ((Get-Sha256 $planPath) -ne $PopulationPlanSha256 -or
    [string]$manifest.population_plan.sha256 -ne $PopulationPlanSha256) { throw "Downloaded Attempt13 population plan hash mismatch; staging preserved at $stage" }
...
if ((Get-Sha256 $populationSourceArchivePath) -ne [string]$manifest.source.sha256) {
    throw "Downloaded Attempt13 frozen population source hash mismatch; staging preserved at $stage"
}
...
if ((Get-Sha256 (Join-Path $frozenPackageRoot "artifacts/population_plan.json")) -ne [string]$manifest.population_plan.sha256) {
    throw "Frozen Attempt13 package plan hash mismatch; staging preserved at $stage"
}
...
foreach ($artifact in @(
    ...
    if ((Get-Sha256 ([string]$artifact[1])) -ne [string]$artifact[2]) {
        throw "Downloaded Attempt13 acceptance artifact hash mismatch; staging preserved at $stage"
    }
}
foreach ($artifact in @(
    ...
    if ((Get-Sha256 ([string]$artifact[0])) -ne [string]$artifact[2]) {
        throw "Attempt13 frozen package/acceptance artifact mismatch; staging preserved at $stage"
    }
    if (-not (Test-Path -LiteralPath ([string]$artifact[1]))) {
        Copy-Item -LiteralPath ([string]$artifact[0]) -Destination ([string]$artifact[1])
    }
    if ((Get-Sha256 ([string]$artifact[1])) -ne [string]$artifact[2]) {
        throw "Attempt13 downloaded artifact differs from frozen package; staging preserved at $stage"
    }
}
...
for ($shard = 0; $shard -lt [int]$manifest.shards.count; $shard++) {
    ...
    if ($done.schema -ne $DoneSchema -or $done.status -ne "complete" -or
        ...
        throw "Attempt13 population shard DONE/hash chain mismatch: $shard; staging preserved at $stage"
    }
...
if ($evaluation.schema -ne "hu_m4_t1_population_evaluation_v1" -or
    ...
    $runtime.model_schema -ne "hu_m43_attempt13_t1_second_distilled_selector_v1" -or
    $runtime.artifact_schema -ne "hu_m43_attempt13_t1_second_distilled_pickle_v1" -or
    $runtime.feature_schema -ne "hu_m43_attempt13_lambda_all_legal_public_infoset_features_v1" -or
    $runtime.head_schema -ne "hu_m43_attempt13_policy_delta_safe_tail_heads_v1" -or
    $runtime.model_id -ne $manifest.runtime.model_id -or
    $runtime.action_score_mode -ne "attempt13_lambda_all_legal_distilled_safe_selector_v1" -or
    $runtime.profile_id -ne "stage20_m4_attempt13" -or
    (@($runtime.opponents) -join ",") -cne "stage19_p0,stage9f_p2,stage7_m5_r10,random_exact_final" -or
    (@($runtime.population_namespace_bases) -join ",") -cne "250108071901,251108071901,252108071901,253108071901,254108071901,255108071901,256108071901" -or
    ...
    throw "Merged Attempt13 population violates the frozen distilled runtime/plan contract; staging preserved at $stage"
}
...
$acceptArgs = @(
    "-m", "ofc_regular.validate_hu_m43_attempt13_acceptance", "finalize",
...
if ($acceptCode -notin @(0, 2)) {
    throw "Attempt13 final acceptance validator failed ($acceptCode):`n$(@($acceptRaw) -join [Environment]::NewLine); staging preserved at $stage"
}
if (-not (Test-Path -LiteralPath $statusPath -PathType Leaf)) {
    throw "Attempt13 acceptance validator did not write its immutable status; staging preserved at $stage"
}
$acceptance = Get-Content -LiteralPath $statusPath -Raw | ConvertFrom-Json
...
if ($acceptance.schema -ne $AcceptanceStatusSchema -or $acceptance.status -ne $expectedDecision -or
    $acceptance.profile_id -ne "stage20_m4_attempt13" -or
    (@($acceptance.opponents) -join ",") -cne "stage19_p0,stage9f_p2,stage7_m5_r10,random_exact_final" -or
    $acceptance.current_profile_mutated -ne $false -or
    $acceptance.runtime_policy_activated -ne $false -or
    ...
    throw "Attempt13 acceptance status/exit code mismatch; staging preserved at $stage"
}
$receipt.acceptance_status_sha256 = Get-Sha256 $statusPath
$receipt.acceptance_decision = $expectedDecision
Write-Utf8NoBom $receiptPath (($receipt | ConvertTo-Json -Depth 12) + "`n")
if (Test-Path -LiteralPath $OutputDir) { throw "Attempt13 OutputDir appeared during receive; refusing overwrite. Staging preserved at $stage" }
