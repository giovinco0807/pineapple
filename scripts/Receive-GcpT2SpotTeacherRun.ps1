...
if (-not $MergedOutput) {
    $MergedOutput = "outputs/gcp_runs/$RunName/t2_teacher_merged.jsonl"
}
...
$files = Get-ChildItem -LiteralPath $DownloadDir -Filter "t2_*.jsonl" |
    Sort-Object Name

$shards = @()
foreach ($file in $files) {
    if ($file.Name -match '^t2_(\d+)\.jsonl$') {
