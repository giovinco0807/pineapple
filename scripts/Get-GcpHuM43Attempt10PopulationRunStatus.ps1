...
function Try-GetGcsJson([string]$Uri) {
    for ($attempt = 1; $attempt -le 2; $attempt++) {
        ...
        if ($message -notmatch '(?i)not found|does not exist|No URLs matched|404') {
            throw "Unable to read Attempt10 population status object: $Uri`n$message"
        }
...
if ($manifest.schema -ne $ManifestSchema -or $manifest.run_name -ne $RunName -or
    ...
    throw "Frozen Attempt10 population manifest identity mismatch"
}
...
if ($shardCount -ne 20 -or [int]$manifest.population_plan.paired_seeds -ne 1000 -or
    ...
    throw "Frozen Attempt10 population manifest is not the fixed 20x50 schedule"
}
...
else { throw "Unable to list Attempt10 population DONE objects: $(@($rawDone) -join [Environment]::NewLine)" }

$doneRows = @()
$seen = [System.Collections.Generic.HashSet[int]]::new()
foreach ($uri in $doneUris) {
    if ($uri -notmatch '/results/shard-(\d{4})/DONE$') { throw "Attempt10 DONE URI does not encode a canonical shard: $uri" }
    $uriShard = [int]$Matches[1]
    $expectedUri = "$prefix/results/shard-{0:D4}/DONE" -f $uriShard
    if ([string]$uri -cne $expectedUri) { throw "Attempt10 DONE URI is outside the frozen run prefix: $uri" }
    if ($uriShard -lt 0 -or $uriShard -ge $shardCount) { throw "Attempt10 DONE shard is outside frozen range: $uriShard" }
    if (-not $seen.Add($uriShard)) { throw "Duplicate Attempt10 DONE shard: $uriShard" }
    ...
    if ($row.schema -ne $DoneSchema -or $row.status -ne "complete" -or
        ...
        throw "Invalid Attempt10 population DONE object: $uri"
    }
    $doneRows += $row
}
...
if ($doneRows.Count -lt $shardCount) {
    ...
    if ($statusCode -eq 0) {
        ...
        foreach ($uri in @($rawStatus | Where-Object { $_ -match '\.json$' })) {
            if ($uri -notmatch '/status/shard-(\d+)\.json$') { throw "Noncanonical Attempt10 population status URI: $uri" }
            $uriShard = [int]$Matches[1]
            $expectedUri = "$prefix/status/shard-$uriShard.json"
            if ([string]$uri -cne $expectedUri -or $uriShard -lt 0 -or $uriShard -ge $shardCount) {
                throw "Attempt10 population status URI is outside the frozen run: $uri"
            }
            if (-not $seenStatus.Add($uriShard)) { throw "Duplicate Attempt10 population status shard: $uriShard" }
            ...
            if ($row.schema -ne $StatusSchema -or $row.run_name -ne $RunName -or
                ...
                throw "Invalid or stale Attempt10 population status object: $uri"
            }
            $statusRows += $row
        }
    }
    elseif ((@($rawStatus) -join "`n") -notmatch '(?i)not found|does not exist|No URLs matched|404') {
        throw "Unable to list Attempt10 population status objects: $(@($rawStatus) -join [Environment]::NewLine)"
    }
}
...
[ordered]@{
    schema = "hu_m43_attempt10_population_spot_status_report_v1"
    run_name = $RunName
