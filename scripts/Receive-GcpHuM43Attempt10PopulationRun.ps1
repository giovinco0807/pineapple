...
$ReceiptSchema = "hu_m43_attempt10_population_spot_receipt_v1"
$AcceptanceStatusSchema = "hu_m43_attempt10_population_acceptance_status_v1"
...
if (-not (Test-Path -LiteralPath $manifestPath -PathType Leaf)) { throw "Frozen Attempt10 population manifest is missing: $manifestPath" }
...
if ($manifest.schema -ne $ManifestSchema -or $manifest.run_name -ne $RunName -or
    ...
    $manifest.runtime.model_schema -ne "hu_m43_attempt10_t1_second_distilled_selector_v1" -or
    $manifest.runtime.artifact_schema -ne "hu_m43_attempt10_t1_second_distilled_pickle_v1" -or
    $manifest.runtime.feature_schema -ne "hu_m43_attempt10_lambda_top12_public_infoset_features_v1" -or
    $manifest.runtime.head_schema -ne "hu_m43_attempt10_policy_delta_safe_tail_heads_v1" -or
    $manifest.runtime.action_score_mode -ne "attempt10_lambda_top12_distilled_safe_selector_v1" -or
    ...
    throw "Frozen Attempt10 population manifest identity/boundary mismatch"
}

$root = [System.IO.Path]::GetFullPath((Join-Path $repoRoot "outputs/hu_joint_policy/m43_attempt10_population"))
if ($OutputDir) {
...
if (-not $OutputDir.StartsWith($rootPrefix, [System.StringComparison]::OrdinalIgnoreCase)) {
    throw "OutputDir must stay under outputs/hu_joint_policy/m43_attempt10_population; refusing profile/current overwrite"
}
if (Test-Path -LiteralPath $OutputDir) {
    ...
    if (-not (Test-Path -LiteralPath $receiptPath -PathType Leaf) -or
        ...
        throw "Attempt10 population OutputDir exists without complete receipts; refusing overwrite"
    }
    ...
    if ($receipt.schema -ne $ReceiptSchema -or $receipt.status -ne "verified_and_merged" -or
        ...
        throw "Existing Attempt10 population receipt/hash chain is invalid"
    }
...
if ((Get-Sha256 $planPath) -ne [string]$manifest.population_plan.sha256) { throw "Downloaded Attempt10 population plan hash mismatch; staging preserved at $stage" }
...
if ((Get-Sha256 $populationSourceArchivePath) -ne [string]$manifest.source.sha256) {
    throw "Downloaded Attempt10 frozen population source hash mismatch; staging preserved at $stage"
}
...
if ((Get-Sha256 (Join-Path $frozenPackageRoot "artifacts/population_plan.json")) -ne [string]$manifest.population_plan.sha256) {
    throw "Frozen Attempt10 package plan hash mismatch; staging preserved at $stage"
}
...
foreach ($artifact in @(
    ...
    if ((Get-Sha256 ([string]$artifact[1])) -ne [string]$artifact[2]) {
        throw "Downloaded Attempt10 acceptance artifact hash mismatch; staging preserved at $stage"
    }
}
foreach ($artifact in @(
    ...
    if ((Get-Sha256 ([string]$artifact[0])) -ne [string]$artifact[2]) {
        throw "Attempt10 frozen package/acceptance artifact mismatch; staging preserved at $stage"
    }
    if (-not (Test-Path -LiteralPath ([string]$artifact[1]))) {
        Copy-Item -LiteralPath ([string]$artifact[0]) -Destination ([string]$artifact[1])
    }
    if ((Get-Sha256 ([string]$artifact[1])) -ne [string]$artifact[2]) {
        throw "Attempt10 downloaded artifact differs from frozen package; staging preserved at $stage"
    }
}
...
for ($shard = 0; $shard -lt [int]$manifest.shards.count; $shard++) {
    ...
    if ($done.schema -ne $DoneSchema -or $done.status -ne "complete" -or
        ...
        throw "Attempt10 population shard DONE/hash chain mismatch: $shard; staging preserved at $stage"
    }
...
try {
    & python -c "import pathlib,sys; from ofc_regular.hu_m43_attempt10_distilled_runtime import validate_distilled_runtime_dependencies,validate_frozen_execution_modules; root=pathlib.Path(sys.argv[1]); validate_frozen_execution_modules(extracted_root=root/'runtime_source',manifest=root/'artifacts/runtime_source_manifest.json',module_names=['ofc_regular.hu_m43_attempt10_distilled_runtime','ofc_regular.evaluate_hu_m4_population','ofc_regular.merge_hu_m4_population_shards']); validate_distilled_runtime_dependencies(root)" $frozenPackageRoot
    if ($LASTEXITCODE -ne 0) { throw "Attempt10 frozen merge execution verification failed; staging preserved at $stage" }
    & python @mergeArgs
    if ($LASTEXITCODE -ne 0) { throw "Attempt10 population shard merger failed ($LASTEXITCODE); staging preserved at $stage" }
}
finally {
...
if ($evaluation.schema -ne "hu_m4_t1_population_evaluation_v1" -or
    ...
    $runtime.model_schema -ne "hu_m43_attempt10_t1_second_distilled_selector_v1" -or
    $runtime.artifact_schema -ne "hu_m43_attempt10_t1_second_distilled_pickle_v1" -or
    $runtime.feature_schema -ne "hu_m43_attempt10_lambda_top12_public_infoset_features_v1" -or
    $runtime.head_schema -ne "hu_m43_attempt10_policy_delta_safe_tail_heads_v1" -or
    $runtime.model_id -ne $manifest.runtime.model_id -or
    $runtime.action_score_mode -ne "attempt10_lambda_top12_distilled_safe_selector_v1" -or
    ...
    throw "Merged Attempt10 population violates the frozen distilled runtime/plan contract; staging preserved at $stage"
}
...
$acceptArgs = @(
    "-m", "ofc_regular.validate_hu_m43_attempt10_acceptance", "finalize",
...
if ($acceptCode -notin @(0, 2)) {
    throw "Attempt10 final acceptance validator failed ($acceptCode):`n$(@($acceptRaw) -join [Environment]::NewLine); staging preserved at $stage"
}
if (-not (Test-Path -LiteralPath $statusPath -PathType Leaf)) {
    throw "Attempt10 acceptance validator did not write its immutable status; staging preserved at $stage"
}
$acceptance = Get-Content -LiteralPath $statusPath -Raw | ConvertFrom-Json
...
if ($acceptance.schema -ne $AcceptanceStatusSchema -or $acceptance.status -ne $expectedDecision -or
    ...
    throw "Attempt10 acceptance status/exit code mismatch; staging preserved at $stage"
}
$receipt.acceptance_status_sha256 = Get-Sha256 $statusPath
$receipt.acceptance_decision = $expectedDecision
Write-Utf8NoBom $receiptPath (($receipt | ConvertTo-Json -Depth 12) + "`n")
if (Test-Path -LiteralPath $OutputDir) { throw "Attempt10 OutputDir appeared during receive; refusing overwrite. Staging preserved at $stage" }
