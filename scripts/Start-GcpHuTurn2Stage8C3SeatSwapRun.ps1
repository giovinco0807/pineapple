param(
    ...
    [string]$RunName = ("regular-hu-t2-c3-seat-swap-" + (Get-Date -Format "yyyyMMdd-HHmmss")),
...
$calibrationValues = "outputs/hu_turn2_stage8_20k_mc512_calibration/state_calibration_values.csv"
if (-not (Test-Path (Join-Path $repoRoot $calibrationValues))) {
    throw "Required calibration file not found: $calibrationValues"
}
...
$packagePath = Join-Path $runDir "ofc_regular_hu_t2_c3_seat_swap_source.zip"
$startupPath = Join-Path $runDir "startup_hu_t2_c3_seat_swap_spot.sh"
$manifestPath = Join-Path $runDir "manifest.json"
$shardManifestPath = Join-Path $runDir "shards_manifest.jsonl"
$sourceUri = "gs://$Bucket/runs/$RunName/source/ofc_regular_hu_t2_c3_seat_swap_source.zip"
$startupUri = "gs://$Bucket/runs/$RunName/source/startup_hu_t2_c3_seat_swap_spot.sh"
...
$calDest = Join-Path $packageDir $calibrationValues
New-Item -ItemType Directory -Force -Path (Split-Path $calDest -Parent) | Out-Null
Copy-Item -LiteralPath (Join-Path $repoRoot $calibrationValues) -Destination $calDest
...
WORK_DIR="/opt/ofc-regular-hu-t2-c3-seat-swap"
sudo rm -rf "$WORK_DIR"
sudo mkdir -p "$WORK_DIR"
sudo chown "$CURRENT_UID:$CURRENT_GID" "$WORK_DIR"
log "download source package $SOURCE_URI"
gcloud storage cp "$SOURCE_URI" /tmp/ofc_regular_hu_t2_c3_source.zip
unzip -q /tmp/ofc_regular_hu_t2_c3_source.zip -d "$WORK_DIR"
cd "$WORK_DIR"
...
STATUS_DIR="/tmp/ofc-hu-t2-c3-status"
RESULT_DIR="/tmp/ofc-hu-t2-c3-results"
mkdir -p "$STATUS_DIR" "$RESULT_DIR"

log "start C3 shard loop run=$RUN_NAME start=$START_SHARD stride=$VM_COUNT total_shards=$TOTAL_SHARDS"
for (( shard=START_SHARD; shard<TOTAL_SHARDS; shard+=VM_COUNT )); do
  ...
  set +e
  python -B -m ofc_regular.evaluate_hu_turn2_stage8_seat_swap \
...
log "all assigned C3 shards finished"
