param(
    ...
    [Parameter(Mandatory = $true)][string]$DevelopmentDecisionPath,
    [Parameter(Mandatory = $true)][string]$DevelopmentSelectorReceiptPath,
    [Parameter(Mandatory = $true)][string]$DevelopmentPassFreezePath,
    [Parameter(Mandatory = $true)][string]$AuditDecisionPath,
    [Parameter(Mandatory = $true)][string]$AuditSelectorReceiptPath,
    [string]$FrozenRuntimeArtifactRootPath = "",
    [string]$PlanPath = "configs/hu_joint_policy_m43_attempt13_population.json",
    [string]$ProjectId = "ofc-solver-485418",
    [string]$Bucket = "pokerhu-ofc-solver-485418-training",
    [string]$RunName = ("regular-hu-m43-attempt13-population-" + (Get-Date -Format "yyyyMMdd-HHmmss")),
...
$ManifestSchema = "hu_m43_attempt13_population_spot_manifest_v1"
$PreflightSchema = "hu_m43_attempt13_population_launch_preflight_v1"
$DoneSchema = "hu_m43_attempt13_population_spot_done_v1"
$StatusSchema = "hu_m43_attempt13_population_spot_status_v1"
$ActionScoreMode = "attempt13_lambda_all_legal_distilled_safe_selector_v1"
$ModelSchema = "hu_m43_attempt13_t1_second_distilled_selector_v1"
$ArtifactSchema = "hu_m43_attempt13_t1_second_distilled_pickle_v1"
$FeatureSchema = "hu_m43_attempt13_lambda_all_legal_public_infoset_features_v1"
$HeadSchema = "hu_m43_attempt13_policy_delta_safe_tail_heads_v1"
$BaselineProfile = "stage19_p0"
$PopulationPlanSha256 = "bf05a9477e18049883926537b4fb15c7e3ecf350c212072aed7c5b981d593a17"
$SeedRegistrySha256 = "5872a79f5337d7d252ce1b5f60085939109dd08ff5cdb41737fcfc3acc98cdb0"
...
$PackageReadySchema = "hu_m43_attempt13_population_package_ready_v1"
...
function Publish-ImmutableObject([string]$LocalPath, [string]$Uri) {
    ...
    try {
        ...
        if ((Get-Sha256 $download) -ne (Get-Sha256 $LocalPath)) {
            throw "Immutable Attempt13 population object already exists with different bytes: $Uri"
        }
    }
...
if (-not $ResumeExisting -and -not $DryRun -and -not $PackageOnly) {
    throw "Fresh Attempt13 population mutation requires -PackageOnly; then use -ResumeExisting with its immutable PACKAGE_READY marker"
}
...
$sourcePath = Join-Path $runDir "ofc_regular_hu_m43_attempt13_population_source.zip"
$manifestPath = Join-Path $runDir "population_run_manifest.json"
$shardsPath = Join-Path $runDir "population_shards.jsonl"
$startupPath = Join-Path $runDir "startup_hu_m43_attempt13_population.sh"
...
$sourceUri = "$gcsPrefix/source/ofc_regular_hu_m43_attempt13_population_source.zip"
$manifestUri = "$gcsPrefix/source/population_run_manifest.json"
$shardsUri = "$gcsPrefix/source/population_shards.jsonl"
$startupUri = "$gcsPrefix/source/startup_hu_m43_attempt13_population.sh"
...
$developmentDecisionUri = "$gcsPrefix/source/acceptance/development_decision.json"
$developmentSelectorReceiptUri = "$gcsPrefix/source/acceptance/development_selector_receipt.json"
$developmentPassFreezeUri = "$gcsPrefix/source/acceptance/development_pass_freeze.json"
$auditDecisionUri = "$gcsPrefix/source/acceptance/audit50_decision.json"
$auditSelectorReceiptUri = "$gcsPrefix/source/acceptance/audit50_selector_receipt.json"
$vmPrefix = Convert-ToVmPrefix $RunName

if ($ResumeExisting) {
    foreach ($path in @($manifestPath, $sourcePath, $shardsPath, $startupPath, $packageReadyPath)) {
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Frozen Attempt13 population input is missing: $path" }
    }
    ...
    if ($manifest.schema -ne $ManifestSchema -or $manifest.run_name -ne $RunName -or
        ...
        [int]$manifest.launch_preflight.prior_population_overlap_count -ne 0 -or
        [int]$manifest.launch_preflight.all_reserved_registry_overlap_count -ne 0) {
        throw "Frozen Attempt13 population run identity mismatch"
    }
    if ($packageReady.schema -ne $PackageReadySchema -or
        ...
        $packageReady.population_plan_sha256 -ne $PopulationPlanSha256 -or
        $packageReady.seed_registry_sha256 -ne $SeedRegistrySha256 -or
        $packageReady.development_decision_sha256 -ne $manifest.launch_preflight.development_decision_sha256 -or
        $packageReady.development_selector_receipt_sha256 -ne $manifest.launch_preflight.development_selector_receipt_sha256 -or
        $packageReady.development_pass_freeze_sha256 -ne $manifest.launch_preflight.development_pass_freeze_sha256 -or
        $packageReady.audit50_decision_sha256 -ne $manifest.launch_preflight.audit50_decision_sha256 -or
        $packageReady.audit50_selector_receipt_sha256 -ne $manifest.launch_preflight.audit50_selector_receipt_sha256 -or
        $packageReady.current_profile_mutated -ne $false -or
        $packageReady.no_runtime_activation -ne $true) {
        throw "Attempt13 PACKAGE_READY authorization is missing, stale, or not bound to this immutable package"
    }
    foreach ($entry in @(
        ...
        @((Join-Path $packageDir "artifacts/development_decision.json"), $manifest.acceptance_artifacts.development_decision.sha256),
        @((Join-Path $packageDir "artifacts/development_selector_receipt.json"), $manifest.acceptance_artifacts.development_selector_receipt.sha256),
        @((Join-Path $packageDir "artifacts/development_pass_freeze.json"), $manifest.acceptance_artifacts.development_pass_freeze.sha256),
        @((Join-Path $packageDir "artifacts/audit50_decision.json"), $manifest.acceptance_artifacts.audit50_decision.sha256),
        @((Join-Path $packageDir "artifacts/audit50_selector_receipt.json"), $manifest.acceptance_artifacts.audit50_selector_receipt.sha256),
        ...
        if ((Get-Sha256 $entry[0]) -ne $entry[1]) { throw "Frozen Attempt13 local hash chain is broken" }
    }
    ...
    try {
        & python -c "import pathlib,sys; from ofc_regular.hu_m43_attempt13_distilled_runtime import validate_distilled_runtime_dependencies,validate_frozen_execution_modules; root=pathlib.Path(sys.argv[1]); validate_frozen_execution_modules(extracted_root=root/'runtime_source',manifest=root/'artifacts/runtime_source_manifest.json',module_names=['ofc_regular.hu_m43_attempt13_distilled_runtime']); validate_distilled_runtime_dependencies(root)" $packageDir
        if ($LASTEXITCODE -ne 0) { throw "Frozen Attempt13 resume dependency/source verification failed" }
    }
    finally {
    ...
    if (-not $DryRun -and (Test-GcsObject $manifestUri)) {
        ...
        try {
            ...
            if ((Get-Sha256 $remoteManifest) -ne (Get-Sha256 $manifestPath)) {
                throw "Frozen remote Attempt13 manifest disagrees with the local resume manifest"
            }
        }
        finally {
    ...
    if (Test-Path -LiteralPath $runDir) { throw "Attempt13 population run directory already exists and is immutable: $runDir" }
    $resolved = [ordered]@{
        ...
        development_decision = Resolve-RepoPath $DevelopmentDecisionPath
        development_selector_receipt = Resolve-RepoPath $DevelopmentSelectorReceiptPath
        development_pass_freeze = Resolve-RepoPath $DevelopmentPassFreezePath
        audit50_decision = Resolve-RepoPath $AuditDecisionPath
        audit50_selector_receipt = Resolve-RepoPath $AuditSelectorReceiptPath
    ...
    $frozenPopulationPlan = Join-Path $runtimeSourceRoot "configs/hu_joint_policy_m43_attempt13_population.json"
    if ((Get-Sha256 $resolved.population_plan) -ne $PopulationPlanSha256 -or
        (Get-Sha256 $frozenPopulationPlan) -ne $PopulationPlanSha256) {
        throw "Requested Attempt13 population plan differs from the frozen runtime registry"
    }
    $resolved.population_plan = $frozenPopulationPlan
    $preflightArgs = @(
        "-m", "ofc_regular.validate_hu_m43_attempt13_acceptance", "preflight",
        ...
        "--development-decision", $resolved.development_decision,
        "--development-selector-receipt", $resolved.development_selector_receipt,
        "--development-pass-freeze", $resolved.development_pass_freeze,
        "--audit-decision", $resolved.audit50_decision,
        "--audit-selector-receipt", $resolved.audit50_selector_receipt,
        "--output", $preflightOutputPath
    )
    $oldPythonPath = $env:PYTHONPATH
    $oldDontWriteBytecode = $env:PYTHONDONTWRITEBYTECODE
    ...
    if ($preflightCode -ne 0) { throw "Attempt13 population preflight failed ($preflightCode):`n$(@($preflightRaw) -join [Environment]::NewLine)" }
    ...
    if ($preflight.schema -ne $PreflightSchema -or $preflight.status -ne "pass" -or
        ...
        $preflight.population_plan_file_sha256 -ne $PopulationPlanSha256 -or
        $preflight.seed_registry_sha256 -ne $SeedRegistrySha256 -or
        $preflight.development200_full_fit_bound -ne $true -or
        $preflight.audit50_one_shot_go_bound -ne $true -or
        [int]$preflight.audit50_fit_rows -ne 0 -or
        $preflight.threshold_reselection_performed -ne $false -or
        [string]$preflight.development_decision_sha256 -cnotmatch '^[0-9a-f]{64}$' -or
        [string]$preflight.development_selector_receipt_sha256 -cnotmatch '^[0-9a-f]{64}$' -or
        [string]$preflight.development_pass_freeze_sha256 -cnotmatch '^[0-9a-f]{64}$' -or
        [string]$preflight.audit50_decision_sha256 -cnotmatch '^[0-9a-f]{64}$' -or
        [string]$preflight.audit50_selector_receipt_sha256 -cnotmatch '^[0-9a-f]{64}$' -or
        [int]$preflight.teacher_overlap_count -ne 0 -or
        [int]$preflight.prior_population_overlap_count -ne 0 -or
        [int]$preflight.all_reserved_registry_overlap_count -ne 0 -or
        $preflight.profile_id -ne "stage20_m4_attempt13" -or
        (@($preflight.opponents) -join ",") -cne "stage19_p0,stage9f_p2,stage7_m5_r10,random_exact_final" -or
        (@($preflight.population_namespace_bases) -join ",") -cne "250108071901,251108071901,252108071901,253108071901,254108071901,255108071901,256108071901") {
        throw "Attempt13 population preflight returned an unsafe receipt"
    }
    ...
    if ($plan.schema -ne "hu_m43_population_acceptance_plan_v1" -or
        ...
        [long]$plan.seed -ne 250108071901 -or [long]$plan.seed_stride -ne 1000003 -or
        [int]$plan.shards -ne 20 -or [int]$plan.paired_seeds_per_shard -ne 50 -or
        (@($plan.runtime_contract.candidate_nonbaseline_count_range) -join ',') -cne '0,26' -or
        (@($plan.runtime_contract.lightgbm_group_size_range) -join ',') -cne '1,27' -or
        $plan.runtime_contract.complete_legal_action_set_rebuilt -ne $true -or
        $plan.runtime_contract.candidate_padding_allowed -ne $false -or
        $plan.runtime_contract.candidate_duplication_allowed -ne $false -or
        $plan.runtime_contract.baseline_appended_exactly_once -ne $true -or
        ...
        throw "Attempt13 population plan is not the frozen 1000x4, 20x50, stage19_p0-baseline contract"
    }
    ...
    if (Get-ChildItem -LiteralPath $artifactDir -Recurse -File | Where-Object { $_.Extension -eq ".jsonl" }) {
        throw "Teacher/calibration/locked JSONL must not enter the Attempt13 population package"
    }
    $artifactHashes = [ordered]@{
        ...
        development_decision = Get-Sha256 (Join-Path $artifactDir "development_decision.json")
        development_selector_receipt = Get-Sha256 (Join-Path $artifactDir "development_selector_receipt.json")
        development_pass_freeze = Get-Sha256 (Join-Path $artifactDir "development_pass_freeze.json")
        audit50_decision = Get-Sha256 (Join-Path $artifactDir "audit50_decision.json")
        audit50_selector_receipt = Get-Sha256 (Join-Path $artifactDir "audit50_selector_receipt.json")
    ...
    foreach ($binding in @(
        ...
        @("development_selector_receipt", "development_selector_receipt_sha256"),
        @("development_pass_freeze", "development_pass_freeze_sha256"),
        @("audit50_decision", "audit50_decision_sha256"),
        @("audit50_selector_receipt", "audit50_selector_receipt_sha256"),
        @("population_plan", "population_plan_file_sha256")
    )) {
        if ($artifactHashes[$binding[0]] -ne $preflight.($binding[1])) { throw "Attempt13 lifecycle artifact changed during packaging: $($binding[0])" }
    }
    ...
    if (Test-Path -LiteralPath (Join-Path $packageDir "configs")) {
        throw "Live repository configs/current-profile metadata must not enter the Attempt13 population package"
    }
    $runtimeSourceContract = Get-Content -LiteralPath $resolvedRuntimeSourceManifest -Raw | ConvertFrom-Json
    $expectedRuntimeConfigs = @(
        $runtimeSourceContract.file_set.files |
            Where-Object { [string]$_.path -like "configs/*" } |
            ForEach-Object { [System.IO.Path]::GetFileName([string]$_.path) } |
            Sort-Object
    )
    ...
    if (Test-Path -LiteralPath (Join-Path $packageDir "rust")) {
        throw "Rust source/debug artifacts must not enter the Attempt13 population package"
    }
    ...
    try {
        & python -c "import pathlib,sys; from ofc_regular.hu_m43_attempt13_distilled_runtime import validate_distilled_runtime_dependencies,validate_frozen_execution_modules; root=pathlib.Path(sys.argv[1]); validate_frozen_execution_modules(extracted_root=root/'runtime_source',manifest=root/'artifacts/runtime_source_manifest.json',module_names=['ofc_regular.hu_m43_attempt13_distilled_runtime']); validate_distilled_runtime_dependencies(root)" $packageDir
        if ($LASTEXITCODE -ne 0) { throw "Packaged Attempt13 dependency/source verification failed" }
    }
    finally {
        $env:PYTHONPATH = $oldPythonPath
        $env:PYTHONDONTWRITEBYTECODE = $oldDontWriteBytecode
...
#!/usr/bin/env bash
set -Eeuo pipefail
LOG=/var/log/hu_m43_attempt13_population.log
exec > >(tee -a "$LOG") 2>&1
...
[[ "$SEED_REGISTRY_SHA" == "5872a79f5337d7d252ce1b5f60085939109dd08ff5cdb41737fcfc3acc98cdb0" ]]
...
json.dump({'schema':'hu_m43_attempt13_population_spot_status_v1','run_name':run,'shard':int(shard),'state':state,'exit_code':int(code),'instance':instance,'zone':zone,'manifest_sha256':manifest,'updated_at':datetime.datetime.now(datetime.timezone.utc).isoformat()},open(p,'w'),sort_keys=True)
PY
gcloud storage cp "$STATUS" "$PREFIX/status/shard-${SHARD}.json" >/dev/null || true; }
# The recurring status object is the heartbeat; immutable DONE is the
# completed-shard checkpoint and is committed only after every result object.
...
python - <<'PY'
from ofc_regular.hu_m43_attempt13_distilled_runtime import validate_distilled_runtime_dependencies, validate_distilled_runtime_extracted_tree, validate_distilled_runtime_source_archive, validate_frozen_execution_modules
...
validate_frozen_execution_modules(extracted_root='runtime_source', manifest='artifacts/runtime_source_manifest.json', module_names=['ofc_regular.hu_m43_attempt13_distilled_runtime'])
validate_distilled_runtime_dependencies('.')
...
python -m ofc_regular_promotion.attempt13_population --shard-index "$SHARD" --model artifacts/m43_model.pkl --expected-model-sha256 "$MODEL_SHA" --training-manifest artifacts/training_manifest.json --runtime-freeze artifacts/runtime_freeze.json --population-plan artifacts/population_plan.json --runtime-source-archive artifacts/runtime_source.zip --runtime-source-manifest artifacts/runtime_source_manifest.json --runtime-source-root runtime_source --runtime-dependency-root . --development-decision artifacts/development_decision.json --development-selector-receipt artifacts/development_selector_receipt.json --development-pass-freeze artifacts/development_pass_freeze.json --audit-decision artifacts/audit50_decision.json --audit-selector-receipt artifacts/audit50_selector_receipt.json --records-output "$OUT/records.jsonl" --output "$OUT/evaluation.json" --progress-every 10 > "$OUT/evaluation_stdout.log"
...
assert r['profile_id']=='stage20_m4_attempt13'
assert r['opponents']==['stage19_p0','stage9f_p2','stage7_m5_r10','random_exact_final']
assert r['population_namespace_bases']==[250108071901,251108071901,252108071901,253108071901,254108071901,255108071901,256108071901]
assert r['model_schema']=='hu_m43_attempt13_t1_second_distilled_selector_v1' and r['runtime_binding_verified'] is True
assert r['artifact_schema']=='hu_m43_attempt13_t1_second_distilled_pickle_v1'
assert r['feature_schema']=='hu_m43_attempt13_lambda_all_legal_public_infoset_features_v1'
assert r['head_schema']=='hu_m43_attempt13_policy_delta_safe_tail_heads_v1'
assert r['action_score_mode']=='attempt13_lambda_all_legal_distilled_safe_selector_v1' and r['safety_enabled'] is True
...
json.dump({'schema':'hu_m43_attempt13_population_spot_done_v1','status':'complete','run_name':run,'shard':int(shard),'manifest_sha256':manifest,'source_sha256':source,'model_sha256':model,'evaluation_sha256':evaluation,'records_sha256':records,'current_profile_mutated':False,'no_runtime_activation':True},open(p,'w'),sort_keys=True)
PY
...
'@
    ...
    $manifest = [ordered]@{
        ...
        acceptance_artifacts = [ordered]@{
            ...
            development_decision = [ordered]@{ uri = $developmentDecisionUri; sha256 = [string]$preflight.development_decision_sha256 }
            development_selector_receipt = [ordered]@{ uri = $developmentSelectorReceiptUri; sha256 = [string]$preflight.development_selector_receipt_sha256 }
            development_pass_freeze = [ordered]@{ uri = $developmentPassFreezeUri; sha256 = [string]$preflight.development_pass_freeze_sha256 }
            audit50_decision = [ordered]@{ uri = $auditDecisionUri; sha256 = [string]$preflight.audit50_decision_sha256 }
            audit50_selector_receipt = [ordered]@{ uri = $auditSelectorReceiptUri; sha256 = [string]$preflight.audit50_selector_receipt_sha256 }
        }
        ...
        source_boundary = [ordered]@{ teacher_jsonl_packaged = $false; audit_jsonl_packaged = $false; calibration_jsonl_packaged = $false; locked_jsonl_packaged = $false; provenance_json_evidence_packaged = $true; current_profile_artifact_packaged = $false; runtime_artifacts_only = $true }
        ...
        heartbeat = [ordered]@{ schema = $StatusSchema; object_prefix = "$gcsPrefix/status"; interval_seconds = $SyncIntervalSeconds; required = $true }
...
if ($PackageOnly) {
    if (Test-Path -LiteralPath $packageReadyPath) { throw "Attempt13 PACKAGE_READY marker already exists" }
    $packageReady = [ordered]@{
        ...
        development_decision_sha256 = [string]$manifest.launch_preflight.development_decision_sha256
        development_selector_receipt_sha256 = [string]$manifest.launch_preflight.development_selector_receipt_sha256
        development_pass_freeze_sha256 = [string]$manifest.launch_preflight.development_pass_freeze_sha256
        audit50_decision_sha256 = [string]$manifest.launch_preflight.audit50_decision_sha256
        audit50_selector_receipt_sha256 = [string]$manifest.launch_preflight.audit50_selector_receipt_sha256
        current_profile_mutated = $false
        no_runtime_activation = $true
    }
    Write-NewUtf8NoBom $packageReadyPath (($packageReady | ConvertTo-Json -Depth 6) + "`n")
    [ordered]@{ schema = "hu_m43_attempt13_population_package_result_v1"; run_name = $RunName; mode = "package_only"; run_dir = $runDir; manifest_sha256 = $manifestSha; source_sha256 = Get-Sha256 $sourcePath; package_ready_sha256 = Get-Sha256 $packageReadyPath; shards = [int]$manifest.shards.count; no_runtime_activation = $true; current_profile_mutated = $false } | ConvertTo-Json -Depth 6
    exit 0
}
if ($DryRun) {
    [ordered]@{ schema = "hu_m43_attempt13_population_package_result_v1"; run_name = $RunName; mode = "dry_run"; run_dir = $runDir; manifest_sha256 = $manifestSha; source_sha256 = Get-Sha256 $sourcePath; shards = [int]$manifest.shards.count; no_runtime_activation = $true; current_profile_mutated = $false } | ConvertTo-Json -Depth 6
    exit 0
}
...
foreach ($pair in @(
    ...
    @((Join-Path $packageDir "artifacts/development_decision.json"), $developmentDecisionUri),
    @((Join-Path $packageDir "artifacts/development_selector_receipt.json"), $developmentSelectorReceiptUri),
    @((Join-Path $packageDir "artifacts/development_pass_freeze.json"), $developmentPassFreezeUri),
    @((Join-Path $packageDir "artifacts/audit50_decision.json"), $auditDecisionUri),
    @((Join-Path $packageDir "artifacts/audit50_selector_receipt.json"), $auditSelectorReceiptUri)
)) {
...
if (@($selected | Select-Object -Unique).Count -ne $selected.Count) {
    throw "Attempt13 population shard selection contains duplicates"
}
if (@($selected | Where-Object { $_ -ne 0 }).Count) {
    $canaryUri = "$gcsPrefix/results/shard-0000/DONE"
    if (-not (Test-GcsObject $canaryUri)) {
        throw "Attempt13 population fanout requires a completed shard-0000 canary"
    }
    ...
    if ($canary.schema -ne $DoneSchema -or $canary.status -ne "complete" -or
        ...
        throw "Attempt13 population fanout canary is not bound to the frozen run"
    }
}
foreach ($shard in $selected) {
    if ($shard -lt 0 -or $shard -ge [int]$manifest.shards.count) { throw "Shard index outside frozen Attempt13 plan: $shard" }
    ...
    if (@($existing | Where-Object { $_ }).Count) {
        ...
        throw "Attempt13 population worker already exists: $vmName"
    }
    ...
    if (-not $created) { throw "Could not create Attempt13 population worker $vmName`n$($errors -join [Environment]::NewLine)" }
}
