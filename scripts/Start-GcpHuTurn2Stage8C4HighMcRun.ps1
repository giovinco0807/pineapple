param(
    ...
    [string]$RunName = ("regular-hu-t2-c4-highmc-" + (Get-Date -Format "yyyyMMdd-HHmmss")),
    [string]$SelectedStatesJsonl = "outputs\evals\hu_turn2_stage8_c4_selected_high_mc_audit_mc4096\selected_teacher_states_deduped.jsonl",
    [string]$CacheDir = "D:\ofc-pineapple-storage\regular-ofc-pineapple\feature_cache\hu_t2_stage8_20k_mc512",
    [string]$Configs = "2.75/0/0.85",
    [int]$ReplayOffset = 0,
    [int]$TargetReplayStates = 50,
    [int]$ShardStateCount = 2,
    [int]$McSamples = 4096,
    [int]$VmCount = 20,
...
if ($ReplayOffset -lt 0) { throw "ReplayOffset must be non-negative" }
if ($TargetReplayStates -le 0) { throw "TargetReplayStates must be positive" }
if ($ShardStateCount -le 0) { throw "ShardStateCount must be positive" }
...
if (-not (Test-Path (Join-Path $repoRoot $SelectedStatesJsonl))) { throw "Selected states not found: $SelectedStatesJsonl" }
if (-not (Test-Path $CacheDir)) { throw "CacheDir not found: $CacheDir" }
...
if ($configList.Count -ne 1) { throw "C4 high-MC runner currently expects one config per run" }

$totalShards = [int][Math]::Ceiling($TargetReplayStates / [double]$ShardStateCount)
...
$replayPackDir = Join-Path $runDir "replay_pack"
$packagePath = Join-Path $runDir "ofc_regular_hu_t2_c4_highmc_source.zip"
$startupPath = Join-Path $runDir "startup_hu_t2_c4_highmc_spot.sh"
$manifestPath = Join-Path $runDir "manifest.json"
$shardManifestPath = Join-Path $runDir "shards_manifest.jsonl"
$sourceUri = "gs://$Bucket/runs/$RunName/source/ofc_regular_hu_t2_c4_highmc_source.zip"
$startupUri = "gs://$Bucket/runs/$RunName/source/startup_hu_t2_c4_highmc_spot.sh"
...
if (Test-Path $replayPackDir) { Remove-Item -LiteralPath $replayPackDir -Recurse -Force }
New-Item -ItemType Directory -Force -Path $packageDir, $replayPackDir | Out-Null

python -m ofc_regular.prepare_hu_turn2_stage8_high_mc_replay_pack `
    --selected-states-jsonl $SelectedStatesJsonl `
    --cache-dir $CacheDir `
    --output-dir $replayPackDir `
    --replay-offset $ReplayOffset `
    --replay-count $TargetReplayStates | Out-Host
...
Copy-Item -LiteralPath $replayPackDir -Destination (Join-Path $packageDir "replay_pack") -Recurse

$shardLines = New-Object System.Collections.Generic.List[string]
for ($shardIndex = 0; $shardIndex -lt $totalShards; $shardIndex += 1) {
    $localOffset = $shardIndex * $ShardStateCount
    $remaining = $TargetReplayStates - $localOffset
    $count = [Math]::Min($ShardStateCount, $remaining)
    $spec = [ordered]@{
        shard = $shardIndex
        replay_offset = $localOffset
        replay_count = $count
        output_prefix = ("shard_{0:D3}_offset{1:D4}_count{2:D2}" -f $shardIndex, $localOffset, $count)
    }
...
WORK_DIR="/opt/ofc-regular-hu-t2-c4-highmc"
sudo rm -rf "$WORK_DIR"
sudo mkdir -p "$WORK_DIR"
sudo chown "$CURRENT_UID:$CURRENT_GID" "$WORK_DIR"
log "download source package $SOURCE_URI"
gcloud storage cp "$SOURCE_URI" /tmp/ofc_regular_hu_t2_c4_highmc_source.zip
unzip -q /tmp/ofc_regular_hu_t2_c4_highmc_source.zip -d "$WORK_DIR"
cd "$WORK_DIR"
...
STATUS_DIR="/tmp/ofc-hu-t2-c4-status"
RESULT_DIR="/tmp/ofc-hu-t2-c4-results"
mkdir -p "$STATUS_DIR" "$RESULT_DIR"

log "start C4 high-MC shard loop run=$RUN_NAME start=$START_SHARD stride=$VM_COUNT total_shards=$TOTAL_SHARDS"
for (( shard=START_SHARD; shard<TOTAL_SHARDS; shard+=VM_COUNT )); do
  ...
  log "run shard=$shard replay_offset=$replay_offset replay_count=$replay_count mc=$MC_SAMPLES"

  set +e
  python -B -m ofc_regular.audit_hu_turn2_stage8_high_mc \
    --selected-states-jsonl replay_pack/selected_states.jsonl \
    --teacher-samples-jsonl replay_pack/teacher_samples.jsonl \
    --configs "$CONFIGS" \
    --output-dir "$local_out" \
    --mc-samples "$MC_SAMPLES" \
    --max-replay-states "$replay_count" \
...
log "all assigned C4 high-MC shards finished"
...
$manifest = [ordered]@{
    ...
    target_replay_states = $TargetReplayStates
    shard_state_count = $ShardStateCount
