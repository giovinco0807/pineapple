param(
    ...
    [string]$PlanPath = "configs/hu_joint_policy_m43_attempt11_population.json",
    [string]$ProjectId = "ofc-solver-485418",
    [string]$Bucket = "pokerhu-ofc-solver-485418-training",
    [string]$RunName = ("regular-hu-m43-attempt11-population-" + (Get-Date -Format "yyyyMMdd-HHmmss")),
...
$ManifestSchema = "hu_m43_attempt11_population_spot_manifest_v1"
$PreflightSchema = "hu_m43_attempt11_population_launch_preflight_v1"
$DoneSchema = "hu_m43_attempt11_population_spot_done_v1"
$StatusSchema = "hu_m43_attempt11_population_spot_status_v1"
$ActionScoreMode = "attempt11_lambda_variable_top12_distilled_safe_selector_v1"
$ModelSchema = "hu_m43_attempt11_t1_second_distilled_selector_v1"
$ArtifactSchema = "hu_m43_attempt11_t1_second_distilled_pickle_v1"
$FeatureSchema = "hu_m43_attempt11_lambda_variable_top12_public_infoset_features_v1"
$HeadSchema = "hu_m43_attempt11_policy_delta_safe_tail_heads_v1"
...
$PackageReadySchema = "hu_m43_attempt11_population_package_ready_v1"
...
function Publish-ImmutableObject([string]$LocalPath, [string]$Uri) {
    ...
    try {
        ...
        if ((Get-Sha256 $download) -ne (Get-Sha256 $LocalPath)) {
            throw "Immutable Attempt11 population object already exists with different bytes: $Uri"
        }
    }
...
if (-not $ResumeExisting -and -not $DryRun -and -not $PackageOnly) {
    throw "Fresh Attempt11 population mutation requires -PackageOnly; then use -ResumeExisting with its immutable PACKAGE_READY marker"
}
...
if ($ResumeExisting) {
    foreach ($path in @($manifestPath, $sourcePath, $shardsPath, $startupPath, $packageReadyPath)) {
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Frozen Attempt11 population input is missing: $path" }
    }
    ...
    if ($manifest.schema -ne $ManifestSchema -or $manifest.run_name -ne $RunName -or
        ...
        throw "Frozen Attempt11 population run identity mismatch"
    }
    if ($packageReady.schema -ne $PackageReadySchema -or
        ...
        throw "Attempt11 PACKAGE_READY authorization is missing, stale, or not bound to this immutable package"
    }
    foreach ($entry in @(
        ...
        if ((Get-Sha256 $entry[0]) -ne $entry[1]) { throw "Frozen Attempt11 local hash chain is broken" }
    }
    ...
    try {
        & python -c "import pathlib,sys; from ofc_regular.hu_m43_attempt11_distilled_runtime import validate_distilled_runtime_dependencies,validate_frozen_execution_modules; root=pathlib.Path(sys.argv[1]); validate_frozen_execution_modules(extracted_root=root/'runtime_source',manifest=root/'artifacts/runtime_source_manifest.json',module_names=['ofc_regular.hu_m43_attempt11_distilled_runtime']); validate_distilled_runtime_dependencies(root)" $packageDir
        if ($LASTEXITCODE -ne 0) { throw "Frozen Attempt11 resume dependency/source verification failed" }
    }
    finally {
    ...
    if (-not $DryRun -and (Test-GcsObject $manifestUri)) {
        ...
        try {
            ...
            if ((Get-Sha256 $remoteManifest) -ne (Get-Sha256 $manifestPath)) {
                throw "Frozen remote Attempt11 manifest disagrees with the local resume manifest"
            }
        }
        finally {
    ...
    if (Test-Path -LiteralPath $runDir) { throw "Attempt11 population run directory already exists and is immutable: $runDir" }
    $resolved = [ordered]@{
    ...
    $frozenPopulationPlan = Join-Path $runtimeSourceRoot "configs/hu_joint_policy_m43_attempt11_population.json"
    if ((Get-Sha256 $resolved.population_plan) -ne (Get-Sha256 $frozenPopulationPlan)) {
        throw "Requested Attempt11 population plan differs from the frozen runtime registry"
    }
    $resolved.population_plan = $frozenPopulationPlan
    $preflightArgs = @(
        "-m", "ofc_regular.validate_hu_m43_attempt11_acceptance", "preflight",
    ...
    if ($preflightCode -ne 0) { throw "Attempt11 population preflight failed ($preflightCode):`n$(@($preflightRaw) -join [Environment]::NewLine)" }
    ...
    if ($preflight.schema -ne $PreflightSchema -or $preflight.status -ne "pass" -or
        ...
        throw "Attempt11 population preflight returned an unsafe receipt"
    }
    ...
    if ($plan.schema -ne "hu_m43_population_acceptance_plan_v1" -or
        ...
        throw "Attempt11 population plan is not the frozen 1000x4, 20x50, stage19_p0-baseline contract"
    }
    ...
    if (Get-ChildItem -LiteralPath $artifactDir -Recurse -File | Where-Object { $_.Extension -eq ".jsonl" }) {
        throw "Teacher/calibration/locked JSONL must not enter the Attempt11 population package"
    }
    ...
    foreach ($binding in @(
        ...
        if ($artifactHashes[$binding[0]] -ne $preflight.($binding[1])) { throw "Attempt11 lifecycle artifact changed during packaging: $($binding[0])" }
    }
    ...
    if (Test-Path -LiteralPath (Join-Path $packageDir "configs")) {
        throw "Live repository configs/current-profile metadata must not enter the Attempt11 population package"
    }
    ...
    if (Test-Path -LiteralPath (Join-Path $packageDir "rust")) {
        throw "Rust source/debug artifacts must not enter the Attempt11 population package"
    }
    ...
    try {
        & python -c "import pathlib,sys; from ofc_regular.hu_m43_attempt11_distilled_runtime import validate_distilled_runtime_dependencies,validate_frozen_execution_modules; root=pathlib.Path(sys.argv[1]); validate_frozen_execution_modules(extracted_root=root/'runtime_source',manifest=root/'artifacts/runtime_source_manifest.json',module_names=['ofc_regular.hu_m43_attempt11_distilled_runtime']); validate_distilled_runtime_dependencies(root)" $packageDir
        if ($LASTEXITCODE -ne 0) { throw "Packaged Attempt11 dependency/source verification failed" }
    }
    finally {
        $env:PYTHONPATH = $oldPythonPath
        $env:PYTHONDONTWRITEBYTECODE = $oldDontWriteBytecode
...
#!/usr/bin/env bash
set -Eeuo pipefail
LOG=/var/log/hu_m43_attempt11_population.log
exec > >(tee -a "$LOG") 2>&1
...
json.dump({'schema':'hu_m43_attempt11_population_spot_status_v1','run_name':run,'shard':int(shard),'state':state,'exit_code':int(code),'instance':instance,'zone':zone,'manifest_sha256':manifest,'updated_at':datetime.datetime.now(datetime.timezone.utc).isoformat()},open(p,'w'),sort_keys=True)
PY
...
python - <<'PY'
from ofc_regular.hu_m43_attempt11_distilled_runtime import validate_distilled_runtime_dependencies, validate_distilled_runtime_extracted_tree, validate_distilled_runtime_source_archive, validate_frozen_execution_modules
...
validate_frozen_execution_modules(extracted_root='runtime_source', manifest='artifacts/runtime_source_manifest.json', module_names=['ofc_regular.hu_m43_attempt11_distilled_runtime'])
...
assert r['model_schema']=='hu_m43_attempt11_t1_second_distilled_selector_v1' and r['runtime_binding_verified'] is True
assert r['artifact_schema']=='hu_m43_attempt11_t1_second_distilled_pickle_v1'
assert r['feature_schema']=='hu_m43_attempt11_lambda_variable_top12_public_infoset_features_v1'
assert r['head_schema']=='hu_m43_attempt11_policy_delta_safe_tail_heads_v1'
assert r['action_score_mode']=='attempt11_lambda_variable_top12_distilled_safe_selector_v1' and r['safety_enabled'] is True
...
json.dump({'schema':'hu_m43_attempt11_population_spot_done_v1','status':'complete','run_name':run,'shard':int(shard),'manifest_sha256':manifest,'source_sha256':source,'model_sha256':model,'evaluation_sha256':evaluation,'records_sha256':records,'current_profile_mutated':False,'no_runtime_activation':True},open(p,'w'),sort_keys=True)
PY
...
if ($PackageOnly) {
    if (Test-Path -LiteralPath $packageReadyPath) { throw "Attempt11 PACKAGE_READY marker already exists" }
    ...
    [ordered]@{ schema = "hu_m43_attempt11_population_package_result_v1"; run_name = $RunName; mode = "package_only"; run_dir = $runDir; manifest_sha256 = $manifestSha; source_sha256 = Get-Sha256 $sourcePath; package_ready_sha256 = Get-Sha256 $packageReadyPath; shards = [int]$manifest.shards.count; no_runtime_activation = $true; current_profile_mutated = $false } | ConvertTo-Json -Depth 6
    exit 0
}
if ($DryRun) {
    [ordered]@{ schema = "hu_m43_attempt11_population_package_result_v1"; run_name = $RunName; mode = "dry_run"; run_dir = $runDir; manifest_sha256 = $manifestSha; source_sha256 = Get-Sha256 $sourcePath; shards = [int]$manifest.shards.count; no_runtime_activation = $true; current_profile_mutated = $false } | ConvertTo-Json -Depth 6
    exit 0
}
...
if (@($selected | Select-Object -Unique).Count -ne $selected.Count) {
    throw "Attempt11 population shard selection contains duplicates"
}
if (@($selected | Where-Object { $_ -ne 0 }).Count) {
    $canaryUri = "$gcsPrefix/results/shard-0000/DONE"
    if (-not (Test-GcsObject $canaryUri)) {
        throw "Attempt11 population fanout requires a completed shard-0000 canary"
    }
    ...
    if ($canary.schema -ne $DoneSchema -or $canary.status -ne "complete" -or
        ...
        throw "Attempt11 population fanout canary is not bound to the frozen run"
    }
}
foreach ($shard in $selected) {
    if ($shard -lt 0 -or $shard -ge [int]$manifest.shards.count) { throw "Shard index outside frozen Attempt11 plan: $shard" }
    ...
    if (@($existing | Where-Object { $_ }).Count) {
        ...
        throw "Attempt11 population worker already exists: $vmName"
    }
    ...
    if (-not $created) { throw "Could not create Attempt11 population worker $vmName`n$($errors -join [Environment]::NewLine)" }
}
