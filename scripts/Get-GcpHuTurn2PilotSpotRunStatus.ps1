...
$tmpRoot = Join-Path $env:TEMP ("{0}-hu-t2-status" -f $RunName)
...
try {
    ...
    if ($shardList) { $shardLines = @($shardList | Where-Object { $_ -match '\.jsonl$' }) }
}
catch { $shardLines = @() }
try {
    $summaryList = gcloud storage ls "$prefix/summaries/*.json" --project $ProjectId 2>$null
    if ($summaryList) { $summaryLines = @($summaryList | Where-Object { $_ -match '\.json$' }) }
}
catch { $summaryLines = @() }
try {
    ...
    foreach ($file in Get-ChildItem -LiteralPath $statusDir -Filter "*.json") {
        try {
            ...
            if ($status.status -eq "complete") {
                $completed += [int]$status.shard
            }
            elseif ($status.status -eq "failed") {
                $failed += [int]$status.shard
            }
        }
        catch {
            $failed += -1
...
if ($vmPrefix.Length -gt 54) {
    $vmPrefix = $vmPrefix.Substring(0, 54).Trim('-')
}
...
[pscustomobject]@{
    ...
    shard_files = $shardLines.Count
    summary_files = $summaryLines.Count
