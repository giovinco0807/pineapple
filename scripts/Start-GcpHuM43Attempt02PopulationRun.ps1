param(
    ...
    [string]$RunName = ("regular-hu-m43-attempt02-population-" + (Get-Date -Format "yyyyMMdd-HHmmss")),
...
$PreflightSchema = "hu_m43_attempt02_population_launch_preflight_v1"
$DoneSchema = "hu_m43_attempt02_population_spot_done_v1"
$StatusSchema = "hu_m43_attempt02_population_spot_status_v1"
$ActionScoreMode = "centered_fold_mean_delta_then_safety_gate_v4"
$ModelSchema = "hu_m43_t1_joint_model_v4"
$RequiredModels = @(
...
function Convert-ToVmPrefix([string]$Value) {
    ...
    if ($name -notmatch '^[a-z]') { $name = "m43a2p-$name" }
...
$sourcePath = Join-Path $runDir "ofc_regular_hu_m43_attempt02_population_source.zip"
$manifestPath = Join-Path $runDir "population_run_manifest.json"
$shardsPath = Join-Path $runDir "population_shards.jsonl"
$startupPath = Join-Path $runDir "startup_hu_m43_attempt02_population.sh"
$gcsPrefix = "gs://$Bucket/runs/$RunName"
$sourceUri = "$gcsPrefix/source/ofc_regular_hu_m43_attempt02_population_source.zip"
$manifestUri = "$gcsPrefix/source/population_run_manifest.json"
$shardsUri = "$gcsPrefix/source/population_shards.jsonl"
$startupUri = "$gcsPrefix/source/startup_hu_m43_attempt02_population.sh"
...
if ($ResumeExisting) {
    foreach ($path in @($manifestPath, $sourcePath, $shardsPath, $startupPath)) {
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) { throw "Frozen Attempt02 population input is missing: $path" }
    }
    ...
    if ($manifest.schema -ne $ManifestSchema -or $manifest.run_name -ne $RunName -or
        ...
        throw "Frozen Attempt02 population run identity mismatch"
    }
    foreach ($entry in @(
        ...
        if ((Get-Sha256 $entry[0]) -ne $entry[1]) { throw "Frozen Attempt02 local hash chain is broken" }
    }
    if (-not $DryRun) {
        foreach ($uri in @($manifestUri, $sourceUri, $shardsUri, $startupUri, $planUri)) {
            if (-not (Test-GcsObject $uri)) { throw "Frozen remote Attempt02 object is missing: $uri" }
        }
        $remoteManifest = [System.IO.Path]::GetTempFileName()
        $remotePlan = [System.IO.Path]::GetTempFileName()
        try {
            Invoke-Gcloud @("storage", "cp", $manifestUri, $remoteManifest, "--project", $ProjectId) | Out-Null
            Invoke-Gcloud @("storage", "cp", $planUri, $remotePlan, "--project", $ProjectId) | Out-Null
            if ((Get-Sha256 $remoteManifest) -ne (Get-Sha256 $manifestPath)) {
                throw "Frozen remote Attempt02 manifest disagrees with the local resume manifest"
            }
            if ((Get-Sha256 $remotePlan) -ne $manifest.population_plan.sha256) {
                throw "Frozen remote Attempt02 plan disagrees with the resume manifest"
            }
        }
        finally {
            Remove-Item -LiteralPath $remoteManifest, $remotePlan -Force -ErrorAction SilentlyContinue
        }
    }
}
else {
    if (Test-Path -LiteralPath $runDir) { throw "Attempt02 population run directory already exists and is immutable: $runDir" }
    $resolved = [ordered]@{
    ...
    $preflightArgs = @(
        "-m", "ofc_regular.validate_hu_m43_attempt02_acceptance", "preflight",
        "--model", $resolved.model,
        "--training-manifest", $resolved.training_manifest,
        "--data-contract", $resolved.data_contract,
        "--freeze-manifest", $resolved.freeze_manifest,
    ...
    if ($preflightCode -ne 0) { throw "Attempt02 population preflight failed ($preflightCode):`n$(@($preflightRaw) -join [Environment]::NewLine)" }
    ...
    if ($preflight.schema -ne $PreflightSchema -or $preflight.status -ne "pass" -or
        ...
        throw "Attempt02 population preflight returned an unsafe receipt"
    }
    $plan = Get-Content -LiteralPath $resolved.population_plan -Raw | ConvertFrom-Json

    New-Item -ItemType Directory -Path $packageDir -Force | Out-Null
    # Runtime inputs are allowlisted.  In particular, do not copy the repository
    # configs tree: it contains historical/current-profile metadata and teacher
    # provenance that the fixed explicit population evaluator does not consume.
    foreach ($item in @("pyproject.toml", "src")) {
        Copy-Item -LiteralPath (Join-Path $repoRoot $item) -Destination $packageDir -Recurse
    }
    # Only the pinned Linux shared objects are runtime inputs.  Exclude local
    # pycache and Rust build/debug source artifacts (including Windows PDBs).
    Get-ChildItem -LiteralPath (Join-Path $packageDir "src") -Recurse -File -Filter "*.pyc" |
        Remove-Item -Force
    ...
    if (Get-ChildItem -LiteralPath $artifactDir -Recurse -File | Where-Object { $_.Extension -eq ".jsonl" }) {
        throw "Teacher/calibration/locked JSONL must not enter the Attempt02 population package"
    }
    ...
    foreach ($binding in @(
        ...
        @("data_contract", "data_contract_file_sha256"), @("freeze_manifest", "freeze_manifest_file_sha256"),
        @("locked_receipt", "locked_receipt_file_sha256"), @("consumption_marker", "consumption_marker_file_sha256"),
        @("population_plan", "population_plan_file_sha256")
    )) {
        if ($artifactHashes[$binding[0]] -ne $preflight.($binding[1])) { throw "Attempt02 lifecycle artifact changed during packaging: $($binding[0])" }
    }
    ...
    if (Test-Path -LiteralPath (Join-Path $packageDir "configs")) {
        throw "Repository configs/current-profile metadata must not enter the Attempt02 population package"
    }
    if (Test-Path -LiteralPath (Join-Path $packageDir "rust")) {
        throw "Rust source/debug artifacts must not enter the Attempt02 population package"
    }
...
#!/usr/bin/env bash
set -Eeuo pipefail
LOG=/var/log/hu_m43_attempt02_population.log
exec > >(tee -a "$LOG") 2>&1
...
json.dump({'schema':'hu_m43_attempt02_population_spot_status_v1','run_name':run,'shard':int(shard),'state':state,'exit_code':int(code),'instance':instance,'zone':zone,'manifest_sha256':manifest,'updated_at':datetime.datetime.now(datetime.timezone.utc).isoformat()},open(p,'w'),sort_keys=True)
PY
...
python -m ofc_regular.evaluate_hu_m4_population --model artifacts/m43_model.pkl --expected-model-sha256 "$MODEL_SHA" --freeze-manifest artifacts/freeze_manifest.json --training-manifest artifacts/training_manifest.json --paired-seeds "$COUNT" --seed "$SEED" --seed-stride "$STRIDE" --opponents stage19_p0 stage9f_p2 stage7_m5_r10 random_exact_final --records-output "$OUT/records.jsonl" --output "$OUT/evaluation.json" --progress-every 10 > "$OUT/evaluation_stdout.log"
...
assert r['model_schema']=='hu_m43_t1_joint_model_v4' and r['runtime_binding_verified'] is True
assert r['action_score_mode']=='centered_fold_mean_delta_then_safety_gate_v4' and r['safety_enabled'] is True
PY
...
json.dump({'schema':'hu_m43_attempt02_population_spot_done_v1','status':'complete','run_name':run,'shard':int(shard),'manifest_sha256':manifest,'source_sha256':source,'model_sha256':model,'evaluation_sha256':evaluation,'records_sha256':records,'current_profile_mutated':False,'no_runtime_activation':True},open(p,'w'),sort_keys=True)
PY
...
'@
    ...
    $manifest = [ordered]@{
        ...
        runtime = [ordered]@{ model_sha256 = [string]$preflight.model_sha256; model_schema = $ModelSchema; model_id = [string]$preflight.model_id; action_score_mode = $ActionScoreMode; frozen_threshold = [double]$preflight.frozen_threshold; training_manifest_sha256 = [string]$preflight.training_manifest_sha256; data_contract_sha256 = [string]$preflight.data_contract_file_sha256; freeze_manifest_sha256 = [string]$preflight.freeze_manifest_file_sha256; locked_receipt_sha256 = [string]$preflight.locked_receipt_file_sha256; consumption_marker_sha256 = [string]$preflight.consumption_marker_file_sha256; runtime_teacher_inputs = $false; current_profile_used = $false }
        launch_preflight = $preflight
        source_boundary = [ordered]@{ teacher_jsonl_packaged = $false; calibration_jsonl_packaged = $false; locked_jsonl_packaged = $false; current_profile_artifact_packaged = $false; lifecycle_hashes_and_receipts_only = $true }
...
if (-not $ResumeExisting) {
    foreach ($pair in @(
        ...
        if (Test-GcsObject $pair[1]) { throw "Immutable Attempt02 population object already exists: $($pair[1])" }
        Invoke-Gcloud @("storage", "cp", $pair[0], $pair[1], "--project", $ProjectId, "--if-generation-match=0") | Out-Null
    }
    if (Test-GcsObject $manifestUri) { throw "Immutable Attempt02 population manifest already exists: $manifestUri" }
...
if (@($selected | Select-Object -Unique).Count -ne $selected.Count) {
    throw "Attempt02 population shard selection contains duplicates"
}
if (@($selected | Where-Object { $_ -ne 0 }).Count) {
    $canaryUri = "$gcsPrefix/results/shard-0000/DONE"
    if (-not (Test-GcsObject $canaryUri)) {
        throw "Attempt02 population fanout requires a completed shard-0000 canary"
    }
    ...
    if ($canary.schema -ne $DoneSchema -or $canary.status -ne "complete" -or
        ...
        throw "Attempt02 population fanout canary is not bound to the frozen run"
    }
}
foreach ($shard in $selected) {
    if ($shard -lt 0 -or $shard -ge [int]$manifest.shards.count) { throw "Shard index outside frozen Attempt02 plan: $shard" }
    ...
    if (@($existing | Where-Object { $_ }).Count) {
        ...
        throw "Attempt02 population worker already exists: $vmName"
    }
    ...
    if (-not $created) { throw "Could not create Attempt02 population worker $vmName`n$($errors -join [Environment]::NewLine)" }
}
