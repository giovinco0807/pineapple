...
Step12e is a new authorization/identity boundary, not a retry of the
terminal Step12b/Step12c/Step12d executions.  All old runners and audit
trees remain unchanged.  New in Step12e: read-only IAM policy GETs are
wrapped with a bounded idempotent retry (3 attempts, 2s/8s backoff,
transport failures only); mutations are never retried.  Cloud execution
additionally requires fresh explicit user authorization outside this
program.
"""
...
EXECUTION_CONFIRMATION = "EXECUTE_STEP12E_DIRECT_V2_EXACT_PAIR_ATTEMPT0"
RUNNER_MODULE_NAME = "_hu_m31_t3_step12b_protocol_v2_runner"
FINAL_SCHEMA = "hu_m31_t3_step6d_step12e_pair_v1_final"
FAILURE_SCHEMA = "hu_m31_t3_step6d_step12e_pair_v1_failure"
...
def dry_run_receipt(
    ...
    return fresh_identity.seal(
        {
            "schema": "hu_m31_t3_step6d_step12e_pair_v1_dry_run",
...
def _terminal_trees_unchanged(before: dict[str, Any]) -> dict[str, Any]:
    ...
    if after != before:
        raise RuntimeError(
            "terminal Step12b/Step12c/Step12d artifact tree changed"
        )
    return after
...
def _write_wrapper_failure(
    *,
    ...
    if not output_root.is_dir() or output_root.is_symlink():
        return
    path = output_root / "STEP12E_FAILURE.json"
    if path.exists() or path.is_symlink():
        return
    body = fresh_identity.seal(
        {
            "schema": FAILURE_SCHEMA,
            "status": "step12e_stopped_without_retry",
    ...
    *, prepared: Any, output_root: str | Path
    ...
    try:
        ...
        final = fresh_identity.seal(
            {
                "schema": FINAL_SCHEMA,
                "status": "step12e_pair_received_cleaned_and_disjoint",
                "fresh_identity_receipt_sha256": fresh["receipt_sha256"],
        ...
        step12b._exclusive_write_json(root / "STEP12E_FINAL.json", final)
        return final
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Step12e fresh-identity exact pair canary"
    )
    parser.add_argument(
        "--execute",
        action="store_true",
    ...
    parser.add_argument(
        "--confirm",
        default="",
        help="exact Step12e execution confirmation",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=fresh_identity.EXPECTED_OUTPUT_ROOT,
        help="must be the exact fresh Step12e output root",
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.execute and args.confirm != EXECUTION_CONFIRMATION:
        raise PermissionError(
            "exact Step12e execution confirmation is missing"
        )
