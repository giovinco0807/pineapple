"""Fresh-identity boundary for the post-Step12h exact-pair canary.

Step12b through Step12h consumed cloud identities and audit roots are all
terminal.  This module proves that a newly prepared direct-v2 contract is
disjoint from all seven terminal executions before a live backend can be
constructed.  Step12i is the first canary launched after both live-only
defects (the missing project-IAM host in the live client allowlist and
the empty-policy abort in Phase2 initial-zero) were fixed, so it is the
first attempt that can reach Phase2 IAM install and VM insert.
"""
...
SCHEMA = "hu_m31_t3_step6d_step12i_fresh_identity_v1"
STATUS = (
    "fresh_step12i_identity_disjoint_from_terminal_step12b_through_step12h"
)
TERMINAL_STEP12H_CLOSEOUT_RECEIPT_SHA256 = (
    "a0aa25fe5a6622ab70ec2eda0530abf9a9c4576f6b7d99117814a01c5fcefd55"
)
TERMINAL_STEP12H_DEPLOYMENT_CONTRACT_SHA256 = (
    "156d4d476c8148df85320505ebb81828c1733751a9c3a864bdacd094c76b2472"
)
...
TERMINAL_STEP12H_OUTPUT_ROOT = ARTIFACT_PARENT / "step12h_pair_v1_actual"
EXPECTED_OUTPUT_ROOT = ARTIFACT_PARENT / "step12i_pair_v1_actual"
    ...
    for _, terminal_root in _TERMINAL_ROOTS:
        ...
        if root == terminal or terminal in root.parents:
            raise PermissionError(
                "only the exact fresh Step12i output root is allowed"
            )
    if root != expected:
        raise PermissionError(
            "only the exact fresh Step12i output root is allowed"
...
def exact_output_root(path: str | Path) -> Path:
    ...
    if root.exists() or root.is_symlink():
        raise FileExistsError(f"Step12i output root is not fresh: {root}")
    if not root.parent.is_dir() or root.parent.is_symlink():
        raise FileNotFoundError("Step12i output parent must be a real directory")
    return root
...
def _validate_terminal_closeouts() -> None:
    # Step12b through Step12g bindings are revalidated through the Step12h
    # boundary module so all seven stay pinned by one call.
    step12h_identity._validate_terminal_closeouts()
    step12h_closeout = _validated_sealed_receipt(
        _read_json(
            TERMINAL_STEP12H_OUTPUT_ROOT
            / "initial_zero_shape_failure_closeout_receipt.json"
        ),
        expected_sha256=TERMINAL_STEP12H_CLOSEOUT_RECEIPT_SHA256,
        label="terminal Step12h closeout",
    )
    if step12h_closeout.get("step12h_identity_terminal") is not True:
        raise ValueError("terminal Step12h closeout binding changed")
    step12h_deployment = _read_json(
        TERMINAL_STEP12H_OUTPUT_ROOT / "deployment_contract.json"
    )
    if (
        step12h_deployment.get("deployment_contract_sha256")
        != TERMINAL_STEP12H_DEPLOYMENT_CONTRACT_SHA256
    ):
        raise ValueError("terminal Step12h deployment binding changed")
...
def validate_fresh_identity(
    *,
    ...
    if (
        ...
        raise ValueError("Step12i exact-pair topology changed")
    ...
    for label, fresh in fresh_values.items():
        if not isinstance(fresh, str) or not fresh:
            raise ValueError(f"Step12i {label} is missing")
        for terminal in terminals:
            if fresh == terminal[label]:
                raise ValueError(
                    f"Step12i reused terminal {terminal['label']} {label}"
                )
    ...
    if len(set(new_instance_names)) != 2 or any(
        ...
        raise ValueError("Step12i reused a terminal VM/disk name")
    ...
    return seal(
        {
            ...
            "terminal_step12c_closeout_receipt_sha256": (
                step12d_identity.TERMINAL_STEP12C_CLOSEOUT_RECEIPT_SHA256
            ),
            "terminal_step12d_closeout_receipt_sha256": (
                step12e_identity.TERMINAL_STEP12D_CLOSEOUT_RECEIPT_SHA256
            ),
            "terminal_step12e_closeout_receipt_sha256": (
                step12f_identity.TERMINAL_STEP12E_CLOSEOUT_RECEIPT_SHA256
            ),
            "terminal_step12f_closeout_receipt_sha256": (
                step12g_identity.TERMINAL_STEP12F_CLOSEOUT_RECEIPT_SHA256
            ),
            "terminal_step12g_closeout_receipt_sha256": (
                step12h_identity.TERMINAL_STEP12G_CLOSEOUT_RECEIPT_SHA256
            ),
            "terminal_step12h_closeout_receipt_sha256": (
                TERMINAL_STEP12H_CLOSEOUT_RECEIPT_SHA256
            ),
