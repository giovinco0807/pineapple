"""Frozen pre-preflight contract for the M4.3 Attempt12 search arm.

Attempt12 is a development-only, profile-blind T1-second search contract.  It
...
ATTEMPT12_POOLED_SAMPLES = (
    ATTEMPT12_STRESS_SAMPLES + ATTEMPT12_CONFIRMATION_SAMPLES
)
ATTEMPT12_EVALUATION_SAMPLES = 512
ATTEMPT12_VETO_MIN_MEAN = 0.0
ATTEMPT12_VETO_MIN_P05 = -25.0
ATTEMPT12_VETO_MIN_P01 = -40.0
ATTEMPT12_STRICT_MIN_P05 = -22.0
ATTEMPT12_STRICT_MIN_P01 = -36.0

ATTEMPT11_PLAN_SHA256 = (
...
_SEED_BASES = {
    "hand": 200_108_071_901,
    "rerank": 201_108_071_901,
    "veto": 202_108_071_901,
    "stress": 203_108_071_901,
    "confirmation": 204_108_071_901,
    "evaluation": 205_108_071_901,
    "child": 206_108_071_901,
}
_PREFLIGHT_SEED_BASES = {
    "hand": 210_108_071_901,
    "rerank": 211_108_071_901,
    "veto": 212_108_071_901,
    "stress": 213_108_071_901,
    "confirmation": 214_108_071_901,
    "evaluation": 215_108_071_901,
    "child": 216_108_071_901,
}
_KNOWN_SEED_GROUPS: tuple[tuple[str, Mapping[str, int], range], ...] = (
    ...
    (
        "Attempt08 population",
        {"population": 160_108_071_901},
        range(1000),
    ),
    (
        "Attempt10 population",
        {"population": 180_108_071_901},
        range(1000),
    ),
    (
        "Attempt11 population",
        {"population": 190_108_071_901},
        range(1000),
    ),
)


def load_and_validate_attempt12_plan(path: str | Path) -> dict[str, Any]:
    """Load a byte-identical plan and fail closed on any drift."""

    target = Path(path)
    if hashlib.sha256(target.read_bytes()).hexdigest() != M43_ATTEMPT12_PLAN_SHA256:
        raise ValueError("Attempt12 plan SHA-256 changed")
    plan = json.loads(target.read_text(encoding="utf-8-sig"))
    if not isinstance(plan, dict):
        raise ValueError("Attempt12 plan must be a mapping")
    validate_attempt12_plan(plan)
    return plan


def validate_attempt12_plan(plan: Mapping[str, Any]) -> None:
    ...
    validate_attempt12_seed_freshness(plan)


def validate_attempt12_artifact_bindings(
    plan: Mapping[str, Any], *, repository_root: str | Path
) -> None:
    """Validate the preserved Attempt11 plan and unchanged frozen dependencies."""
    ...
    attempt11_payload = json.loads(
        resolved["preserved Attempt11 plan"].read_text(encoding="utf-8-sig")
    )
    if (
        attempt11_payload.get("schema") != "hu_m43_attempt11_plan_v1"
        or attempt11_payload.get("activation_guards", {}).get(
            "runtime_policy_activated"
        )
        is not False
        or attempt11_payload.get("scope", {}).get("current_profile_mutated")
        is not False
    ):
        raise ValueError("Attempt11 preserved boundary changed")


def enumerate_attempt12_seed_schedules(
    plan: Mapping[str, Any], *, population: str
) -> dict[str, tuple[int, ...]]:
    """Derive all seven schedules for development, audit, or preflight."""

    if population == "preflight":
        return enumerate_attempt12_preflight_seed_schedules(plan)
...
def enumerate_attempt12_preflight_seed_schedules(
...
def enumerate_known_seed_schedules() -> dict[str, tuple[int, ...]]:
    """Materialize the explicitly frozen search and population namespaces."""
    ...
    for group, bases, indices in _KNOWN_SEED_GROUPS:
        for domain, base in bases.items():
            schedules[f"{group}.{domain}"] = tuple(
                base + M43_ATTEMPT12_SEED_STRIDE * index for index in indices
            )
    return schedules


def validate_attempt12_seed_freshness(plan: Mapping[str, Any]) -> None:
    ...
    for population, expected_count in (("development", 200), ("future_audit", 50)):
        for domain, values in enumerate_attempt12_seed_schedules(
            plan, population=population
        ).items():
            if len(values) != expected_count or len(set(values)) != expected_count:
                raise ValueError(f"Attempt12 {population}.{domain} is not unique")
            declared[f"{population}.{domain}"] = values
    for domain, values in enumerate_attempt12_preflight_seed_schedules(plan).items():
        if len(values) != 5 or len(set(values)) != 5:
            raise ValueError(f"Attempt12 preflight.{domain} is not unique")
        declared[f"preflight.{domain}"] = values
    _reject_pairwise_overlap(declared, label="Attempt12 declared seed schedules")
    ...
    for new_name, new_values in declared.items():
        ...
        for old_name, old_values in known.items():
            if new_set.intersection(old_values):
                raise ValueError(f"Attempt12 {new_name} overlaps {old_name}")


def _load_canonical_plan() -> dict[str, Any]:
    if not _CANONICAL_PLAN_PATH.is_file():
        raise ValueError("canonical Attempt12 plan is missing")
    raw = _CANONICAL_PLAN_PATH.read_bytes()
    if hashlib.sha256(raw).hexdigest() != M43_ATTEMPT12_PLAN_SHA256:
        raise ValueError("canonical Attempt12 plan SHA-256 changed")
    value = json.loads(raw.decode("utf-8-sig"))
    if not isinstance(value, dict):
        raise ValueError("canonical Attempt12 plan must be a mapping")
    return value


def _validate_frozen_semantics(plan: Mapping[str, Any]) -> None:
    _require_exact(plan.get("schema"), M43_ATTEMPT12_PLAN_SCHEMA, "plan.schema")
    _require_exact(plan.get("milestone"), "M4.3-attempt12", "plan.milestone")
    ...
    _require_exact(plan.get("profiles"), list(M43_ATTEMPT12_PROFILES), "profiles")
    ...
    if (
        ...
        raise ValueError("Attempt12 seven-shard full-profile preflight changed")

    boundary = _mapping(plan.get("attempt11_boundary"), "attempt11_boundary")
    _require_exact(
        boundary.get("status"),
        "development_no_go_closed",
        "attempt11_boundary.status",
    )
    _require_exact(
        boundary.get("development_roots"), 200, "attempt11 boundary roots"
    )
    _require_exact(
        boundary.get("fires_total"), 18, "attempt11 boundary fires"
    )
    if (
        boundary.get("all_attempt11_files_and_artifacts_preserved") is not True
        or boundary.get("attempt11_defaults_changed") is not False
        or boundary.get("attempt11_seed_reuse_allowed") is not False
        or boundary.get("attempt11_runtime_promotion_allowed") is not False
    ):
        raise ValueError("Attempt11 artifacts and closed boundary must remain preserved")
    ...
    if scope.get("public_information_set_only") is not True:
        raise ValueError("Attempt12 must remain public-information-set only")
    ...
    if (
        ...
        raise ValueError("Attempt12 candidate uniqueness/baseline contract changed")
    ...
    _require_exact(
        ...
        "all_n_complete_unique_legal_nonbaseline_actions_plus_explicit_baseline_where_n_is_0_to_26",
        "rerank.action_scope",
    )
    ...
    _require_exact(
        ...
        "first_min(4,n)_in_R_order_plus_unique_lambda_raw_risk_fill_from_all_remaining_complete_legal_R_positions",
        "shortlist.selection",
    )
    ...
    _require_exact(
        ...
        "minimum_frozen_lambda_raw_risk_score_then_ActionKey_without_replacement_until_K_is_full",
    ...
    _require_exact(
        veto.get("eligibility"),
        {
            ...
            "paired_delta_p05_min": -25,
            "paired_delta_p01_min": -40,
        },
        "veto.eligibility",
    )
    _require_exact(
    ...
    _require_exact(stress.get("symbol"), "X1024", "stress.symbol")
    _require_exact(stress.get("samples"), 1024, "stress.samples")
    _require_exact(
        stress.get("action_scope"),
        "all_V256_retained_nonbaseline_actions_plus_explicit_baseline",
        "stress.scope",
    )
    _require_exact(
        stress.get("retention"),
        "no_filter_all_V256_survivors_forwarded_unchanged",
        "stress.retention",
    )
    if stress.get("may_rerank_candidates") is not False:
        raise ValueError("X1024 may not filter or rerank candidates")

    confirmation = _mapping(search.get("confirmation"), "confirmation")
    _require_exact(confirmation.get("symbol"), "C1024", "confirmation.symbol")
    _require_exact(confirmation.get("samples"), 1024, "confirmation.samples")
    _require_exact(
        confirmation.get("action_scope"),
        "the_exact_same_all_V256_retained_nonbaseline_actions_plus_explicit_baseline_as_X1024",
        "confirmation.scope",
    )
    _require_exact(
        confirmation.get("retention"),
        "no_independent_filter_pool_X1024_and_C1024_by_stable_ActionKey",
        "confirmation.retention",
    )
    pooled = _mapping(confirmation.get("pooled_decision"), "confirmation.pooled_decision")
    _require_exact(pooled.get("symbol"), "P2048", "pooled.symbol")
    _require_exact(pooled.get("samples_per_action"), 2048, "pooled.samples")
    _require_exact(
        pooled.get("eligibility"),
        {
            "paired_delta_mean_strictly_greater_than": 0,
            "paired_delta_p05_min": -22,
            "paired_delta_p01_min": -36,
        },
        "pooled.eligibility",
    )
    _require_exact(
        pooled.get("selection"),
        "minimum_normalized_p05_p01_tail_risk_then_pooled_mean_desc_then_original_R_position_then_ActionKey",
        "pooled.selection",
    )
    _require_exact(
        pooled.get("selection_risk_score"),
        "max(max(0,-pooled_p05)/22.0,max(0,-pooled_p01)/36.0)",
        "pooled.selection_risk_score",
    )
    if confirmation.get("may_rerank_candidates") is not True:
        raise ValueError("P2048 must apply the frozen final selection ordering")
    if (
        confirmation.get("selection_is_final_only_after_single_pooled_strict_eligibility")
        is not True
    ):
        raise ValueError("P2048 selection must follow its single pooled gate")
    ...
    if evaluation.get("diagnostics_only") is not True:
        raise ValueError("E512 must remain diagnostics-only")
    if evaluation.get("may_rerank_veto_confirm_promote_or_change_root_output") is not False:
        raise ValueError("E512 may not alter the frozen root decision")
    _require_exact(
    ...
    if search.get("candidate_selection_and_evaluation_rng_independent") is not True:
        raise ValueError("selection and E512 RNG must remain independent")
    ...
    if not guards or any(value is not False for value in guards.values()):
        raise ValueError("all Attempt12 activation guards must remain false")
...
def _validate_profile_balance(plan: Mapping[str, Any]) -> None:
    ...
    for section, expected in (("development_population", 40), ("future_audit_population", 10)):
        ...
        if any(assigned.count(profile) != expected for profile in profiles):
            raise ValueError(f"Attempt12 {section} is not profile-balanced")
...
def _validate_compute_accounting(plan: Mapping[str, Any]) -> None:
    ...
    _require_exact(
        ...
        {
            "rerank_action_futures_formula": "(n+1)*128_where_n_is_0_to_26",
            "rerank_action_futures_min": r,
            "rerank_action_futures_max": 27 * r,
            ...
            "fixed_prefilter_action_futures_max": 27 * r + 9 * v,
            ...
            "confirmation_action_futures_formula": "(V_retained_nonbaseline_count+1)*1024",
            ...
            "maximum_full_fire_action_futures_per_root": 27 * r
            + 9 * v
            + 9 * x
            + 9 * c
            + 2 * e,
...
__all__ = [
    ...
    "ATTEMPT12_VETO_MIN_MEAN",
    "ATTEMPT12_VETO_MIN_P01",
    "ATTEMPT12_VETO_MIN_P05",
    "ATTEMPT12_VETO_SAMPLES",
    "M43_ATTEMPT12_PLAN_SCHEMA",
    "M43_ATTEMPT12_PLAN_SHA256",
    "M43_ATTEMPT12_PROFILES",
    "M43_ATTEMPT12_SEED_STRIDE",
    "enumerate_attempt12_preflight_seed_schedules",
    ...
    "validate_attempt12_seed_freshness",
]
