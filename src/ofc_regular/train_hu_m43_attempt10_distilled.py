"""Train the fixed Attempt10 public-infoset distillation ensemble.
...
from .hu_m43_attempt10_contract import M43_ATTEMPT10_PLAN_SHA256
from .hu_m43_attempt10_distilled_runtime import (
    ATTEMPT10_EXPECTED_RUNTIME_FINGERPRINT_SHA256,
    ATTEMPT10_RUNTIME_REQUIREMENTS_SHA256,
...
ATTEMPT10_DISTILLATION_CONFIG_SCHEMA = "hu_m43_attempt10_distillation_config_v1"
ATTEMPT10_DISTILLATION_CONFIG_SHA256 = (
    "1bb97a15d335e11c471cf223326017592a2318dc0cdcfe95917f6d378c6e8461"
)
ATTEMPT10_DISTILLATION_FULL_ITERATIONS = 180
ATTEMPT10_DISTILLATION_FULL_STATES = 200
ATTEMPT10_DEVELOPMENT_ROW_SCHEMA = "hu_m43_attempt10_search_root_v1"
ATTEMPT10_DISTILLED_TRAINING_MANIFEST_SCHEMA = (
    "hu_m43_attempt10_distilled_training_manifest_v1"
)
ATTEMPT10_DEVELOPMENT_DECISION_SCHEMA = (
    "hu_m43_attempt10_development_go_no_go_v1"
)
ATTEMPT10_DEVELOPMENT_SELECTOR_RECEIPT_SCHEMA = (
    "hu_m43_attempt10_development_selector_receipt_v1"
)
ATTEMPT10_DEVELOPMENT_PASS_FREEZE_SCHEMA = (
    "hu_m43_attempt10_development_go_freeze_v1"
)
ATTEMPT10_PROFILES = (
    "stage19_p0",
    "stage9f_p2",
...
ATTEMPT10_FOLDS = 5
ATTEMPT10_PROPOSAL_ROWS = ATTEMPT10_TOP_K + 1


@dataclass(frozen=True)
class Attempt10DistillationState:
    root_index: int
    root_profile: str
...
def read_attempt10_jsonl(path: str | Path) -> list[dict[str, Any]]:
    raw = Path(path).read_bytes()
    if not raw or not raw.endswith(b"\n"):
        raise ValueError("Attempt10 distillation JSONL must be non-empty and newline terminated")
    rows: list[dict[str, Any]] = []
    for line_number, line in enumerate(raw.splitlines(), start=1):
        try:
            value = json.loads(line)
        except json.JSONDecodeError as exc:
            raise ValueError(f"Attempt10 distillation JSONL line {line_number} is invalid") from exc
        if not isinstance(value, dict):
            raise ValueError("Attempt10 distillation JSONL rows must be mappings")
        rows.append(value)
    return rows


def prepare_attempt10_distillation_states(
    rows: Sequence[Mapping[str, Any]],
    *,
    ranker: FrozenAttempt10LambdaRanker,
    expected_roots: int | None = 200,
) -> tuple[Attempt10DistillationState, ...]:
    ...
    if expected_roots is not None and len(rows) != expected_roots:
        raise ValueError(f"Attempt10 distillation requires exactly {expected_roots} rows")
    by_root: dict[int, Mapping[str, Any]] = {}
    for raw in rows:
        root_index = _strict_int(raw.get("root_index"), "root_index")
        if root_index in by_root:
            raise ValueError("Attempt10 distillation rejects duplicate root indices")
        by_root[root_index] = raw
    if expected_roots is not None and set(by_root) != set(range(expected_roots)):
        raise ValueError("Attempt10 distillation root indices are not contiguous")

    states: list[Attempt10DistillationState] = []
    seen_fingerprints: set[str] = set()
    for root_index in sorted(by_root):
        row = by_root[root_index]
        _reject_hidden_information(row)
        if row.get("schema") != ATTEMPT10_DEVELOPMENT_ROW_SCHEMA:
            raise ValueError("Attempt10 distillation requires canonical search rows")
        ...
        if (
            ...
            raise ValueError(
                "Attempt10 distillation accepts Development200 rows only"
            )
        profile = str(row.get("root_profile"))
        if profile not in ATTEMPT10_PROFILES:
            raise ValueError("Attempt10 distillation root profile is unknown")
        if expected_roots == 200 and profile != ATTEMPT10_PROFILES[root_index % 5]:
            raise ValueError("Attempt10 distillation profile schedule changed")
        ...
        if observation.to_dict() != observation_payload:
            raise ValueError("Attempt10 distillation observation is non-canonical")
        if observation.street != "T1" or observation.seat != "second":
            raise ValueError("Attempt10 distillation authorizes only T1-second")
        fingerprint = observation.fingerprint()
        if row.get("observation_fingerprint") not in (None, fingerprint):
            raise ValueError("Attempt10 distillation observation fingerprint changed")
        if fingerprint in seen_fingerprints:
            raise ValueError("Attempt10 distillation rejects duplicate information sets")
        ...
        if teacher.get("schema") != ATTEMPT10_TEACHER_SCHEMA:
            raise ValueError("Attempt10 distillation teacher schema changed")
        if teacher.get("policy_observation") != observation_payload:
            raise ValueError("Attempt10 distillation teacher observation changed")
        if teacher.get("baseline_action_key") != baseline_token:
            raise ValueError("Attempt10 distillation teacher baseline changed")
        if teacher.get("runtime_gate_allowed") is not False:
            raise ValueError("Attempt10 teacher data cannot be runtime authorization")
        _validate_attempt10_public_teacher(
            observation,
        ...
        teacher_top12 = tuple(str(value) for value in _sequence(
            teacher.get("learned_top12_action_keys"), "learned_top12_action_keys"
        ))
        built_top12 = tuple(built.action_keys[index].to_token() for index in built.top12_indices)
        if teacher_top12 != built_top12:
            raise ValueError("Attempt10 distillation top-twelve mapping changed")
        proposal_indices = (*built.top12_indices, baseline_index)

        rerank = _mapping(teacher.get("rerank"), "rerank")
        rerank_rows = _sequence(rerank.get("actions"), "rerank.actions")
        if len(rerank_rows) != ATTEMPT10_PROPOSAL_ROWS:
            raise ValueError("Attempt10 distillation requires top12 plus baseline R128 rows")
        ...
        if tuple(str(_mapping(item, "rerank action").get("action_key")) for item in rerank_rows) != expected_tokens:
            raise ValueError("Attempt10 distillation R128 action mapping changed")
        ...
        if type(fired) is not bool or type(fallback) is not bool:
            raise ValueError("Attempt10 distillation decision flags are invalid")
        if fired == fallback or (not fired and selected_token != baseline_token):
            raise ValueError("Attempt10 distillation baseline fallback changed")
        if selected_token not in expected_tokens:
            raise ValueError("Attempt10 distillation selected action is outside proposals")

        delta = np.zeros(ATTEMPT10_PROPOSAL_ROWS, dtype=np.float64)
        tails = np.zeros((ATTEMPT10_PROPOSAL_ROWS, 3), dtype=np.float64)
        for index, raw_rerank in enumerate(rerank_rows):
            ...
            if raw_delta.shape != (128,) or not np.isfinite(raw_delta).all():
                raise ValueError("Attempt10 distillation R128 vector changed")
            ...
            for name, expected in summary.items():
                if not math.isclose(
                    ...
                    raise ValueError("Attempt10 distillation R128 summary changed")
        ...
        if not np.allclose(delta[-1], 0.0, rtol=0.0, atol=1.0e-12) or not np.allclose(
            ...
            raise ValueError("Attempt10 distillation explicit baseline is not zero")

        relevance = np.zeros(ATTEMPT10_PROPOSAL_ROWS, dtype=np.int8)
        selected_position = expected_tokens.index(selected_token)
        relevance[selected_position] = 4
        safe = np.zeros(ATTEMPT10_PROPOSAL_ROWS, dtype=np.int8)
        ...
        if fired:
            if evaluation.get("opened") is not True or len(evaluation_rows) != 2:
                raise ValueError("Attempt10 distillation fired row lacks E256")
            ...
            if selected_evaluation.get("action_key") != selected_token:
                raise ValueError("Attempt10 distillation E256 action mapping changed")
            ...
            if evaluation_raw.shape != (256,) or not np.isfinite(evaluation_raw).all():
                raise ValueError("Attempt10 distillation fired row lacks E256")
            if float(np.mean(evaluation_raw)) > 0.0:
                safe[selected_position] = 1
        elif evaluation.get("opened") is not False or evaluation_rows:
            raise ValueError("Attempt10 distillation nonfire opened E256")
...
def _validate_attempt10_public_teacher(
    ...
    *,
    ...
    config = Attempt10TeacherConfig(
        frozen_model_sha256=ATTEMPT10_FROZEN_MODEL_SHA256,
    ...
    result = validate_attempt10_teacher_output(
        observation,
    ...
    if (
        ...
        raise ValueError("Attempt10 public teacher validation result changed")


def assign_attempt10_identity_folds(
    states: Sequence[Attempt10DistillationState], *, folds: int = ATTEMPT10_FOLDS
) -> np.ndarray:
    if folds != ATTEMPT10_FOLDS or not states:
        raise ValueError("Attempt10 distillation requires exactly five folds")
    if len({state.observation_fingerprint for state in states}) != len(states):
        raise ValueError("Attempt10 distillation fold identities repeat")
    ...
    if np.any(result < 0):
        raise ValueError("Attempt10 distillation fold assignment found unknown profile")
    return result


def fit_attempt10_distilled_model(
    states: Sequence[Attempt10DistillationState],
    *,
    candidate_ranker: FrozenAttempt10LambdaRanker,
    config: Mapping[str, Any],
    smoke: bool = False,
    model_id: str = "hu-m43-attempt10-distilled-development200-oof",
) -> tuple[HuM43Attempt10DistilledModel, dict[str, Any]]:
    """Fit five train-four/hold-one models and freeze OOF tail cushions."""

    _validate_config(config)
    if len(states) < ATTEMPT10_FOLDS:
        raise ValueError("Attempt10 distillation has too few states")
    folds = assign_attempt10_identity_folds(states)
    training = _mapping(config.get("training"), "training")
    seed = _strict_int(training.get("random_seed"), "random_seed")
    predictors: list[Attempt10DistilledFoldPredictor] = []
    for fold_index in range(ATTEMPT10_FOLDS):
        ...
        for column, values in enumerate(
            ...
            residuals[column].extend(
                ...
                for target, predicted in zip(
                    state.tails[top12, column], values[top12], strict=True
                )
            )
    ...
    model = HuM43Attempt10DistilledModel(
        ...
        model_id=model_id,
        manifest={
            "training_data": "attempt10_development200_only",
            "audit50_fit_rows": 0,
            "threshold_sweep_performed": False,
            "identity_grouped_folds": ATTEMPT10_FOLDS,
    ...
    diagnostics = {
        "states": len(states),
        "rows": len(states) * ATTEMPT10_PROPOSAL_ROWS,
        "fold_counts": {
            str(fold): int(np.sum(folds == fold)) for fold in range(ATTEMPT10_FOLDS)
        },
        "profile_counts": {
...
def train_attempt10_distilled_artifact(
    *,
    input_path: str | Path,
    ...
    _validate_config(config)
    if _file_sha256(config_path) != ATTEMPT10_DISTILLATION_CONFIG_SHA256:
        raise ValueError("Attempt10 full distillation config SHA changed")
    decision = _load_mapping(decision_path, "development decision")
    if (
        decision.get("schema") != ATTEMPT10_DEVELOPMENT_DECISION_SCHEMA
        or decision.get("decision") != "go"
        or decision.get("search_freeze_authorized") is not True
    ):
        raise ValueError("Attempt10 distillation requires development Go")
    ...
    if decision_source.get("input_jsonl_sha256") != input_sha:
        raise ValueError("Attempt10 development decision does not bind training JSONL")
    ...
    if (
        ...
        raise ValueError("Attempt10 development selector receipt is not fit authorization")
    if (
        pass_freeze.get("schema") != ATTEMPT10_DEVELOPMENT_PASS_FREEZE_SCHEMA
        or pass_freeze.get("status") != "go_freeze_attempt10_development"
        ...
        raise ValueError("Attempt10 immutable development-pass freeze is not bound")
    ...
    if (
        freeze_bindings["plan_sha256"] != M43_ATTEMPT10_PLAN_SHA256
        ...
        or decision_source.get("plan_sha256") != M43_ATTEMPT10_PLAN_SHA256
        ...
        raise ValueError("Attempt10 development source hash chain changed")
    ...
    if (
        ...
        or teacher_contract.get("plan_sha256") != M43_ATTEMPT10_PLAN_SHA256
        or teacher_contract.get("candidate_model_sha256")
        != ATTEMPT10_FROZEN_MODEL_SHA256
    ):
        raise ValueError("Attempt10 runtime source/teacher closure changed")
    ...
    if _mapping(
        ...
        raise ValueError("Attempt10 runtime dependency semantic binding changed")

    ranker = FrozenAttempt10LambdaRanker.load(
        candidate_model_path, expected_sha256=ATTEMPT10_FROZEN_MODEL_SHA256
    )
    rows = read_attempt10_jsonl(input_path)
    states = prepare_attempt10_distillation_states(rows, ranker=ranker, expected_roots=200)
    model, diagnostics = fit_attempt10_distilled_model(
        states, candidate_ranker=ranker, config=config, smoke=smoke
    )
    model_sha = model.save(output_model_path)
    manifest = {
        "schema": ATTEMPT10_DISTILLED_TRAINING_MANIFEST_SCHEMA,
        ...
        "source": {
            ...
            "candidate_model_sha256": ATTEMPT10_FROZEN_MODEL_SHA256,
        ...
        "runtime": {
            ...
            "runtime_requirements_sha256": ATTEMPT10_RUNTIME_REQUIREMENTS_SHA256,
            "runtime_fingerprint_sha256": (
                ATTEMPT10_EXPECTED_RUNTIME_FINGERPRINT_SHA256
            ),
        },
...
def _fit_fold(
    ...
    *,
    ...
    if not states:
        raise ValueError("Attempt10 distilled fold has no training states")
    ...
    groups = [ATTEMPT10_PROPOSAL_ROWS for _ in states]
    state_weight = _profile_state_weights(states)
    weight = np.concatenate(
        [
            np.full(ATTEMPT10_PROPOSAL_ROWS, value / ATTEMPT10_PROPOSAL_ROWS)
...
def _profile_state_weights(states: Sequence[Attempt10DistillationState]) -> np.ndarray:
    counts = {profile: sum(state.root_profile == profile for state in states) for profile in ATTEMPT10_PROFILES}
...
def _validate_config(config: Mapping[str, Any]) -> None:
    if config.get("schema") != ATTEMPT10_DISTILLATION_CONFIG_SCHEMA:
        raise ValueError("Attempt10 distillation config schema changed")
    schemas = _mapping(config.get("schemas"), "schemas")
    if schemas.get("model") != HU_M43_ATTEMPT10_DISTILLED_MODEL_SCHEMA:
        raise ValueError("Attempt10 distillation model schema changed")
    candidate = _mapping(config.get("candidate_generation"), "candidate_generation")
    if (
        candidate.get("source_model_sha256") != ATTEMPT10_FROZEN_MODEL_SHA256
        or candidate.get("nonbaseline_top_k") != ATTEMPT10_TOP_K
        or candidate.get("feature_dimension") != HU_M43_ATTEMPT10_DISTILLED_FEATURE_DIM
    ):
        raise ValueError("Attempt10 distillation candidate contract changed")
    ...
    if (
        ...
        or gate.get("action_score_mode") != HU_M43_ATTEMPT10_DISTILLED_ACTION_SCORE_MODE
    ):
        raise ValueError("Attempt10 distillation runtime gate changed")
    training = _mapping(config.get("training"), "training")
    if training.get("threshold_sweep_allowed") is not False:
        raise ValueError("Attempt10 distillation threshold sweep is forbidden")
    if _mapping(config.get("fit_authorization"), "fit_authorization") != {
        ...
        raise ValueError("Attempt10 distillation fit authorization changed")
    ...
    if isinstance(value, Mapping):
        for key, child in value.items():
            ...
            if lowered in {"opponent_private_discards", "opponent_discards"}:
                raise ValueError(f"Attempt10 distillation exposes hidden information at {path}")
...
def _mapping(value: Any, label: str) -> dict[str, Any]:
    if not isinstance(value, Mapping):
        raise ValueError(f"Attempt10 distillation {label} must be a mapping")
    return dict(value)


def _sequence(value: Any, label: str) -> list[Any]:
    if isinstance(value, (str, bytes, bytearray)) or not isinstance(value, Sequence):
        raise ValueError(f"Attempt10 distillation {label} must be a sequence")
    return list(value)


def _strict_int(value: Any, label: str) -> int:
    if isinstance(value, bool) or not isinstance(value, (int, np.integer)):
        raise ValueError(f"Attempt10 distillation {label} must be an integer")
    return int(value)
...
def _finite(value: Any, label: str) -> float:
    try:
        ...
        raise ValueError(f"Attempt10 distillation {label} must be numeric") from exc
    if not math.isfinite(result):
        raise ValueError(f"Attempt10 distillation {label} must be finite")
    return result
...
def _require_sha256(value: Any, label: str) -> str:
    if not isinstance(value, str):
        raise ValueError(f"Attempt10 distillation {label} must be a SHA-256 string")
    normalized = value.lower()
    if len(normalized) != 64 or any(
        character not in "0123456789abcdef" for character in normalized
    ):
        raise ValueError(f"Attempt10 distillation {label} is invalid")
    return normalized
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Fit frozen Attempt10 distillation heads")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    manifest = train_attempt10_distilled_artifact(
...
__all__ = [
    "ATTEMPT10_DISTILLATION_CONFIG_SHA256",
    "ATTEMPT10_DISTILLATION_FULL_ITERATIONS",
    "ATTEMPT10_DISTILLATION_FULL_STATES",
    "ATTEMPT10_DISTILLED_TRAINING_MANIFEST_SCHEMA",
    "Attempt10DistillationState",
    "assign_attempt10_identity_folds",
    "fit_attempt10_distilled_model",
    "prepare_attempt10_distillation_states",
    "read_attempt10_jsonl",
    "train_attempt10_distilled_artifact",
]
