"""Immutable source freeze for the Attempt08 distilled runtime.
...
ATTEMPT08_DISTILLED_RUNTIME_MANIFEST_SCHEMA = (
    "hu_m43_attempt08_distilled_runtime_source_manifest_v1"
)
ATTEMPT08_DISTILLED_RUNTIME_SOURCE_CLOSURE_SCHEMA = (
    "hu_m43_attempt08_distilled_runtime_source_closure_v1"
)
ATTEMPT08_DISTILLED_RUNTIME_SEMANTIC_CLOSURE_SCHEMA = (
    "hu_m43_attempt08_distilled_runtime_semantic_closure_v1"
)
_ARCHIVE_SCHEMA = "hu_m43_attempt08_distilled_runtime_source_zip_v1"
...
ATTEMPT08_DISTILLED_POPULATION_PLAN_PATH = (
...
ATTEMPT08_DISTILLED_SOURCE_MODEL_MANIFEST_SHA256 = (
    "e9b9d9748bb2b2f31a4baa0d928b60c05f254ab5d7915a7461fb8402e1e7ddb8"
)
ATTEMPT08_DISTILLED_SOURCE_NATIVE_MANIFEST_SHA256 = (
    "ec295d070ac7bb21a82aeb2f432b8f6f191e61b63027bc446e8bd5f39f51569f"
)
ATTEMPT08_DISTILLED_RUNTIME_DEPENDENCY_SCHEMA = (
    "hu_m43_attempt08_distilled_runtime_dependencies_v1"
)
...
_SELECTION = {
    ...
    "population_plan": ATTEMPT08_DISTILLED_POPULATION_PLAN_PATH,
    "seed_registry_configs": list(ATTEMPT08_DISTILLED_RUNTIME_REGISTRY_PATHS),
}
...
def build_distilled_runtime_source_freeze(
    *,
    ...
    if requested_root.is_symlink():
        raise ValueError("Attempt08 distilled source root must not be a symlink")
    root = requested_root.resolve(strict=True)
    if not root.is_dir():
        raise ValueError("Attempt08 distilled source root must be a real directory")
    archive_path = Path(output_archive)
    manifest_path = Path(output_manifest)
    if archive_path.resolve() == manifest_path.resolve():
        raise ValueError("Attempt08 archive and manifest paths must be distinct")
    if archive_path.exists() or manifest_path.exists():
        raise FileExistsError("Attempt08 distilled source freeze is immutable")

    rows = _source_rows(root)
    file_set_payload = {
        "schema": ATTEMPT08_DISTILLED_RUNTIME_SOURCE_CLOSURE_SCHEMA,
    ...
    manifest = {
        "schema": ATTEMPT08_DISTILLED_RUNTIME_MANIFEST_SCHEMA,
        "status": _STATUS,
        "file_set": {
            **file_set_payload,
            "file_count": len(rows),
...
def load_and_validate_distilled_runtime_manifest(
    ...
    if isinstance(source, Mapping):
        ...
        try:
            ...
            raise ValueError("Attempt08 distilled runtime manifest is invalid") from error
        if not isinstance(value, dict):
            raise ValueError("Attempt08 distilled runtime manifest must be a mapping")
        manifest = value
        if raw != canonical_json_bytes(manifest):
            raise ValueError("Attempt08 distilled runtime manifest is not canonical")
...
def validate_distilled_runtime_source_archive(
    *,
    ...
    if not archive.is_file() or archive.is_symlink():
        raise ValueError("Attempt08 distilled runtime archive is missing or unsafe")
    ...
    if (
        ...
        raise ValueError("Attempt08 distilled runtime archive bytes changed")
    ...
    try:
        with zipfile.ZipFile(archive, "r") as bundle:
            if bundle.comment != b"":
                raise ValueError("Attempt08 distilled runtime ZIP comment changed")
            infos = bundle.infolist()
            names = [info.filename for info in infos]
            for name in names:
                _safe_relative_path(name)
            if len(names) != len(set(names)):
                raise ValueError("Attempt08 distilled runtime ZIP has duplicate entries")
            if names != sorted(expected_rows) or set(names) != set(expected_rows):
                raise ValueError("Attempt08 distilled runtime ZIP file set changed")
            for info in infos:
                _validate_zip_metadata(info)
                row = expected_rows[info.filename]
                if info.file_size != row["bytes"]:
                    raise ValueError("Attempt08 distilled runtime ZIP size changed")
                encoded = bundle.read(info)
                if (
                    len(encoded) != row["bytes"]
                    or _sha256_bytes(encoded) != row["sha256"]
                ):
                    raise ValueError("Attempt08 distilled runtime ZIP content changed")
    except zipfile.BadZipFile as error:
        raise ValueError("Attempt08 distilled runtime archive is not a ZIP") from error
    return validated
...
def validate_distilled_runtime_extracted_tree(
    *,
    ...
    if not root.is_dir() or root.is_symlink():
        raise ValueError("Attempt08 extracted runtime root is missing or unsafe")
    expected_rows = {
        str(row["path"]): row
        for row in _sequence(validated["file_set"]["files"], "source files")
    }
    ...
    for path in root.rglob("*"):
        if path.is_symlink():
            raise ValueError("Attempt08 extracted runtime tree contains a symlink")
        ...
        if path.is_dir():
            ...
            raise ValueError("Attempt08 extracted runtime tree has a special file")
    ...
    if actual_files != set(expected_rows) or actual_directories != expected_directories:
        raise ValueError("Attempt08 extracted runtime tree file set changed")
    for name, row in expected_rows.items():
        target = root.joinpath(*PurePosixPath(name).parts)
        if target.stat().st_size != row["bytes"] or sha256_file(target) != row["sha256"]:
            raise ValueError("Attempt08 extracted runtime source bytes changed")
    requirements = root.joinpath(*PurePosixPath(_REQUIREMENTS_PATH).parts)
    if sha256_file(requirements) != ATTEMPT08_RUNTIME_REQUIREMENTS_SHA256:
        raise ValueError("Attempt08 extracted runtime requirements changed")
    return validated
...
def validate_distilled_runtime_dependencies(
    ...
    if not root.is_dir() or root.is_symlink():
        raise ValueError("Attempt08 runtime dependency root is missing or unsafe")
    ...
    for path, expected, label in (
        ...
        if not path.is_file() or path.is_symlink() or sha256_file(path) != expected:
            raise ValueError(f"Attempt08 pinned {label} bytes changed")
    ...
    if (
        ...
        raise ValueError("Attempt08 pinned source-model manifest semantics changed")
    if (
        ...
        raise ValueError("Attempt08 pinned source-native manifest semantics changed")
    ...
    closure_payload = {
        "schema": ATTEMPT08_DISTILLED_RUNTIME_DEPENDENCY_SCHEMA,
    ...
    *,
    ...
    for name in module_names:
        if not isinstance(name, str) or not name:
            raise ValueError("Attempt08 frozen execution module name is invalid")
        module = importlib.import_module(name)
        raw_file = getattr(module, "__file__", None)
        if not isinstance(raw_file, str):
            raise ValueError(f"Attempt08 execution module has no file: {name}")
        module_file = Path(raw_file).resolve(strict=True)
        try:
            relative = module_file.relative_to(root).as_posix()
        except ValueError as error:
            raise ValueError(
                f"Attempt08 execution module was imported outside frozen tree: {name}"
            ) from error
        row = expected_rows.get(relative)
        if row is None or sha256_file(module_file) != row["sha256"]:
            raise ValueError(
                f"Attempt08 execution module is not in the frozen manifest: {name}"
            )
...
def extract_distilled_runtime_source_archive(
    *,
    ...
    if destination.exists() or destination.is_symlink():
        raise FileExistsError("Attempt08 extracted runtime destination must be new")
...
def _source_rows(root: Path) -> list[dict[str, Any]]:
    ...
    if not pyproject.is_file() or not requirements.is_file() or not source.is_dir():
        raise ValueError("Attempt08 distilled runtime source boundary is incomplete")
    if (
        ...
        raise ValueError("Attempt08 distilled runtime source boundary has a symlink")
    if sha256_file(requirements) != ATTEMPT08_RUNTIME_REQUIREMENTS_SHA256:
        raise ValueError("Attempt08 distilled runtime requirements drifted")
    for path in source.rglob("*"):
        if path.is_symlink():
            raise ValueError("Attempt08 distilled runtime Python tree has a symlink")
    python_files = sorted(
        (path for path in source.rglob("*.py") if path.is_file()),
        key=lambda path: path.relative_to(root).as_posix(),
    )
    if not python_files:
        raise ValueError("Attempt08 distilled runtime Python source tree is empty")
    registry_files = [
        root.joinpath(*PurePosixPath(relative).parts)
        for relative in ATTEMPT08_DISTILLED_RUNTIME_REGISTRY_PATHS
    ]
    if any(not path.is_file() or path.is_symlink() for path in registry_files):
        raise ValueError("Attempt08 distilled seed-registry source boundary is incomplete")
    population_plan = root.joinpath(
        *PurePosixPath(ATTEMPT08_DISTILLED_POPULATION_PLAN_PATH).parts
    )
    if not population_plan.is_file() or population_plan.is_symlink():
        raise ValueError("Attempt08 distilled population plan source is missing")
    selected = [
        pyproject,
        requirements,
        population_plan,
    ...
    for path in selected:
        ...
        _safe_relative_path(relative)
        if relative in seen:
            raise ValueError("Attempt08 distilled runtime source selection is duplicated")
        seen.add(relative)
        rows.append(
            {
                "path": relative,
...
def _semantic_payload(*, source_closure_sha256: str) -> dict[str, Any]:
    return {
        "schema": ATTEMPT08_DISTILLED_RUNTIME_SEMANTIC_CLOSURE_SCHEMA,
...
def _validate_manifest(manifest: Mapping[str, Any]) -> None:
    if set(manifest) != {"schema", "status", "file_set", "semantic_closure", "archive"}:
        raise ValueError("Attempt08 distilled runtime manifest keys changed")
    if (
        manifest.get("schema") != ATTEMPT08_DISTILLED_RUNTIME_MANIFEST_SCHEMA
        or manifest.get("status") != _STATUS
    ):
        raise ValueError("Attempt08 distilled runtime manifest identity changed")
    file_set = _mapping(manifest.get("file_set"), "file_set")
    if set(file_set) != {"schema", "selection", "files", "file_count", "sha256"}:
        raise ValueError("Attempt08 distilled runtime file-set keys changed")
    if (
        file_set.get("schema")
        != ATTEMPT08_DISTILLED_RUNTIME_SOURCE_CLOSURE_SCHEMA
        or file_set.get("selection") != _SELECTION
    ):
        raise ValueError("Attempt08 distilled runtime source selection changed")
    ...
    for raw in rows:
        ...
        if set(row) != {"path", "bytes", "sha256"}:
            raise ValueError("Attempt08 distilled runtime source row keys changed")
        if not isinstance(row.get("path"), str):
            raise ValueError("Attempt08 distilled runtime source path is invalid")
        name = row["path"]
        _safe_relative_path(name)
        size = row.get("bytes")
        if isinstance(size, bool) or not isinstance(size, int) or size < 0:
            raise ValueError("Attempt08 distilled runtime source size is invalid")
    ...
    if (
        ...
        or ATTEMPT08_DISTILLED_POPULATION_PLAN_PATH not in names
        or any(name not in names for name in ATTEMPT08_DISTILLED_RUNTIME_REGISTRY_PATHS)
        or not any(name.startswith("src/") and name.endswith(".py") for name in names)
        or file_set.get("file_count") != len(rows)
    ):
        raise ValueError("Attempt08 distilled runtime source file set is invalid")
    closure_payload = {
        "schema": ATTEMPT08_DISTILLED_RUNTIME_SOURCE_CLOSURE_SCHEMA,
        "selection": dict(_SELECTION),
    ...
    if _require_sha256(file_set.get("sha256"), "source closure") != closure_sha:
        raise ValueError("Attempt08 distilled runtime source closure changed")
    ...
    if set(semantic) != set(expected_semantic) | {"sha256"} or any(
        ...
        raise ValueError("Attempt08 distilled runtime semantic binding changed")
    ...
    if _require_sha256(semantic.get("sha256"), "semantic closure") != semantic_sha:
        raise ValueError("Attempt08 distilled runtime semantic closure changed")
    ...
    if set(archive) != {
        ...
        raise ValueError("Attempt08 distilled runtime archive keys changed")
    if (
        ...
        raise ValueError("Attempt08 distilled runtime archive contract changed")
...
def _validate_zip_metadata(info: zipfile.ZipInfo) -> None:
    ...
    if (
        ...
        raise ValueError("Attempt08 distilled runtime ZIP metadata changed")
...
def _safe_relative_path(value: str) -> PurePosixPath:
    if (
        ...
        raise ValueError("Attempt08 distilled runtime path is unsafe")
...
def _require_sha256(value: Any, label: str) -> str:
    if (
        ...
        raise ValueError(f"Attempt08 {label} SHA-256 is invalid")
    return value
...
def _sequence(value: Any, label: str) -> list[Any]:
    if isinstance(value, (str, bytes)) or not isinstance(value, list):
        raise ValueError(f"Attempt08 {label} must be a list")
    return value
...
def _json_mapping(path: Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise ValueError(f"Attempt08 {label} is invalid") from error
    ...
    *,
    ...
    for raw in rows:
        ...
        if set(row) != expected_keys or not isinstance(row.get("path"), str):
            raise ValueError(f"Attempt08 pinned {label} row changed")
        relative = _safe_relative_path(row["path"])
        if row["path"] in names:
            raise ValueError(f"Attempt08 pinned {label} path is duplicated")
        names.add(row["path"])
        size = row.get("bytes")
        if isinstance(size, bool) or not isinstance(size, int) or size <= 0:
            raise ValueError(f"Attempt08 pinned {label} size is invalid")
        ...
        try:
            ...
            raise ValueError(
                f"Attempt08 pinned {label} path escapes its root: {row['path']}"
            ) from error
        if any(
            parent.is_symlink()
            ...
            raise ValueError(f"Attempt08 pinned {label} path contains a symlink")
        if (
            not target.is_file()
            ...
            raise ValueError(f"Attempt08 pinned {label} bytes changed: {row['path']}")
        normalized_row = {"path": row["path"], "bytes": size, "sha256": digest}
        if expect_platform:
            if row.get("platform") != "linux-x86_64":
                raise ValueError("Attempt08 pinned native binary platform changed")
