"""Fresh-identity boundary for the post-Step12e exact-pair canary.

Step12b, Step12c, Step12d, and Step12e consumed cloud identities and
audit roots are all terminal.  This module proves that a newly prepared
direct-v2 contract is disjoint from all four terminal executions before a
live backend can be constructed.
"""
...
SCHEMA = "hu_m31_t3_step6d_step12f_fresh_identity_v1"
STATUS = (
    "fresh_step12f_identity_disjoint_from_terminal_step12b_through_step12e"
)
TERMINAL_STEP12E_CLOSEOUT_RECEIPT_SHA256 = (
    "39020a11f21ac43aa37889c13311a9801f6a7905f8f859d0fff0aa5132da2f92"
)
TERMINAL_STEP12E_DEPLOYMENT_CONTRACT_SHA256 = (
    "af4e78c7f1b981e7894684141e7ae9d6a451e1c66a0daee0d60dca342cdfb429"
)
...
TERMINAL_STEP12E_OUTPUT_ROOT = ARTIFACT_PARENT / "step12e_pair_v1_actual"
EXPECTED_OUTPUT_ROOT = ARTIFACT_PARENT / "step12f_pair_v1_actual"
    ...
    for _, terminal_root in _TERMINAL_ROOTS:
        ...
        if root == terminal or terminal in root.parents:
            raise PermissionError(
                "only the exact fresh Step12f output root is allowed"
            )
    if root != expected:
        raise PermissionError(
            "only the exact fresh Step12f output root is allowed"
...
def exact_output_root(path: str | Path) -> Path:
    ...
    if root.exists() or root.is_symlink():
        raise FileExistsError(f"Step12f output root is not fresh: {root}")
    if not root.parent.is_dir() or root.parent.is_symlink():
        raise FileNotFoundError("Step12f output parent must be a real directory")
    return root
...
def _validate_terminal_closeouts() -> None:
    # Step12b, Step12c, and Step12d bindings are revalidated through the
    # Step12e boundary module so all four stay pinned by one call.
    step12e_identity._validate_terminal_closeouts()
    step12e_closeout = _validated_sealed_receipt(
        _read_json(
            TERMINAL_STEP12E_OUTPUT_ROOT
            / "sa_readback_failure_closeout_receipt.json"
        ),
        expected_sha256=TERMINAL_STEP12E_CLOSEOUT_RECEIPT_SHA256,
        label="terminal Step12e closeout",
    )
    if step12e_closeout.get("step12e_identity_terminal") is not True:
        raise ValueError("terminal Step12e closeout binding changed")
    step12e_deployment = _read_json(
        TERMINAL_STEP12E_OUTPUT_ROOT / "deployment_contract.json"
    )
    if (
        step12e_deployment.get("deployment_contract_sha256")
        != TERMINAL_STEP12E_DEPLOYMENT_CONTRACT_SHA256
    ):
        raise ValueError("terminal Step12e deployment binding changed")
...
def validate_fresh_identity(
    *,
    ...
    if (
        ...
        raise ValueError("Step12f exact-pair topology changed")
    ...
    for label, fresh in fresh_values.items():
        if not isinstance(fresh, str) or not fresh:
            raise ValueError(f"Step12f {label} is missing")
        for terminal in terminals:
            if fresh == terminal[label]:
                raise ValueError(
                    f"Step12f reused terminal {terminal['label']} {label}"
                )
    ...
    if len(set(new_instance_names)) != 2 or any(
        ...
        raise ValueError("Step12f reused a terminal VM/disk name")
