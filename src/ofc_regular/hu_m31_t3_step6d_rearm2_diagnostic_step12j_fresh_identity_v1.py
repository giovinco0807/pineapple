...
Step12b through Step12i consumed cloud identities and audit roots are all
...
SCHEMA = "hu_m31_t3_step6d_step12j_fresh_identity_v1"
STATUS = (
    "fresh_step12j_identity_disjoint_from_terminal_step12b_through_step12i"
)
TERMINAL_STEP12I_CLOSEOUT_RECEIPT_SHA256 = (
    "63f735ec7c9bb4cbf1c6f6c6f2b48166ff2f1cdbce2cfadf99ff2acca838acd3"
)
TERMINAL_STEP12I_DEPLOYMENT_CONTRACT_SHA256 = (
    "29010c0a02d92f6068fb1cda76b3ba7520cd179c64dfefaba6d7ed29a60e1960"
)
...
TERMINAL_STEP12I_OUTPUT_ROOT = ARTIFACT_PARENT / "step12i_pair_v1_actual"
EXPECTED_OUTPUT_ROOT = ARTIFACT_PARENT / "step12j_pair_v1_actual"
    ...
    for _, terminal_root in _TERMINAL_ROOTS:
        ...
        if root == terminal or terminal in root.parents:
            raise PermissionError(
                "only the exact fresh Step12j output root is allowed"
            )
    if root != expected:
        raise PermissionError(
            "only the exact fresh Step12j output root is allowed"
...
def exact_output_root(path: str | Path) -> Path:
    ...
    if root.exists() or root.is_symlink():
        raise FileExistsError(f"Step12j output root is not fresh: {root}")
    if not root.parent.is_dir() or root.parent.is_symlink():
        raise FileNotFoundError("Step12j output parent must be a real directory")
    return root
...
def _validate_terminal_closeouts() -> None:
    # Step12b through Step12h bindings are revalidated through the Step12i
    # boundary module so all eight stay pinned by one call.
    step12i_identity._validate_terminal_closeouts()
    step12i_closeout = _validated_sealed_receipt(
        _read_json(
            TERMINAL_STEP12I_OUTPUT_ROOT
            / "phase2_install_failure_closeout_receipt.json"
        ),
        expected_sha256=TERMINAL_STEP12I_CLOSEOUT_RECEIPT_SHA256,
        label="terminal Step12i closeout",
    )
    if step12i_closeout.get("step12i_identity_terminal") is not True:
        raise ValueError("terminal Step12i closeout binding changed")
    step12i_deployment = _read_json(
        TERMINAL_STEP12I_OUTPUT_ROOT / "deployment_contract.json"
    )
    if (
        step12i_deployment.get("deployment_contract_sha256")
        != TERMINAL_STEP12I_DEPLOYMENT_CONTRACT_SHA256
    ):
        raise ValueError("terminal Step12i deployment binding changed")
...
def validate_fresh_identity(
    *,
    ...
    if (
        ...
        raise ValueError("Step12j exact-pair topology changed")
    ...
    for label, fresh in fresh_values.items():
        if not isinstance(fresh, str) or not fresh:
            raise ValueError(f"Step12j {label} is missing")
        for terminal in terminals:
            if fresh == terminal[label]:
                raise ValueError(
                    f"Step12j reused terminal {terminal['label']} {label}"
                )
    ...
    if len(set(new_instance_names)) != 2 or any(
        ...
        raise ValueError("Step12j reused a terminal VM/disk name")
    ...
    return seal(
        {
            ...
            "terminal_step12h_closeout_receipt_sha256": (
                step12i_identity.TERMINAL_STEP12H_CLOSEOUT_RECEIPT_SHA256
            ),
            "terminal_step12i_closeout_receipt_sha256": (
                TERMINAL_STEP12I_CLOSEOUT_RECEIPT_SHA256
            ),
