"""Frozen pre-preflight contract for the M4.3 Attempt10 search arm.

Attempt10 is a development-only, profile-blind T1-second search contract.  It
...
ATTEMPT10_VETO_MIN_MEAN = 0.0
ATTEMPT10_VETO_MIN_P05 = -25.0
ATTEMPT10_VETO_MIN_P01 = -40.0
ATTEMPT10_VETO_MIN_VALUE = -50.0
ATTEMPT10_STRICT_MIN_P05 = -22.0
ATTEMPT10_STRICT_MIN_P01 = -36.0
ATTEMPT10_STRICT_MIN_VALUE = -45.0

ATTEMPT09_DECISION_SHA256 = (
    "48b5aa7af92a09bb3b0636278af6acbfd8d86de52d0242d5b7b0bacd41c216a6"
)
ATTEMPT09_DECISION_RECEIPT_SHA256 = (
...
_CANONICAL_PLAN_PATH = (
    _REPOSITORY_ROOT / "configs" / "hu_joint_policy_m43_attempt10.json"
)
...
def load_and_validate_attempt10_plan(path: str | Path) -> dict[str, Any]:
    """Load a byte-identical plan and fail closed on any drift."""

    target = Path(path)
    if hashlib.sha256(target.read_bytes()).hexdigest() != M43_ATTEMPT10_PLAN_SHA256:
        raise ValueError("Attempt10 plan SHA-256 changed")
    plan = json.loads(target.read_text(encoding="utf-8-sig"))
    if not isinstance(plan, dict):
        raise ValueError("Attempt10 plan must be a mapping")
    validate_attempt10_plan(plan)
    return plan


def validate_attempt10_plan(plan: Mapping[str, Any]) -> None:
    ...
    validate_attempt10_seed_freshness(plan)


def validate_attempt10_artifact_bindings(
    plan: Mapping[str, Any], *, repository_root: str | Path
) -> None:
    """Validate Attempt09 No-Go evidence and unchanged frozen dependencies."""

    validate_attempt10_plan(plan)
    root = Path(repository_root).resolve()
    boundary = _mapping(plan.get("attempt09_boundary"), "attempt09_boundary")
    decision = _mapping(boundary.get("decision"), "attempt09_boundary.decision")
    receipt = _mapping(
        boundary.get("decision_receipt"), "attempt09_boundary.decision_receipt"
    )
    ...
    bindings = (
        (decision["path"], decision["sha256"], "Attempt09 No-Go decision"),
        (
            receipt["path"],
            receipt["sha256"],
            "Attempt09 No-Go decision receipt",
        ),
        (
            search["candidate_generator_artifact_path"],
            search["candidate_generator_artifact_sha256"],
            "Attempt10 Lambda model",
        ),
        (
    ...
    decision_payload = json.loads(
        resolved["Attempt09 No-Go decision"].read_text(encoding="utf-8-sig")
    )
    if (
        decision_payload.get("decision") != "no_go"
        or decision_payload.get("status")
        != "no_go_close_attempt09_development"
        or decision_payload.get("search_freeze_authorized") is not False
        or decision_payload.get("selected_threshold") is not None
    ):
        raise ValueError("Attempt09 decision is not the closed No-Go boundary")
    receipt_payload = json.loads(
        resolved["Attempt09 No-Go decision receipt"].read_text(
            encoding="utf-8-sig"
        )
    )
    if (
        ...
        or receipt_payload.get("decision_sha256") != ATTEMPT09_DECISION_SHA256
        or receipt_payload.get("search_freeze_authorized") is not False
        or receipt_payload.get("future_audit_authorized") is not False
        or receipt_payload.get("runtime_policy_activated") is not False
    ):
        raise ValueError("Attempt09 decision receipt boundary changed")


def enumerate_attempt10_seed_schedules(
    plan: Mapping[str, Any], *, population: str
) -> dict[str, tuple[int, ...]]:
    """Derive all seven schedules for development, audit, or preflight."""

    if population == "preflight":
        return enumerate_attempt10_preflight_seed_schedules(plan)
...
def enumerate_attempt10_preflight_seed_schedules(
...
def enumerate_known_seed_schedules() -> dict[str, tuple[int, ...]]:
    """Materialize every known Attempt06/07/08/09 namespace in the proof."""
    ...
    for group, bases, indices in _KNOWN_SEED_GROUPS:
        for domain, base in bases.items():
            schedules[f"{group}.{domain}"] = tuple(
                base + M43_ATTEMPT10_SEED_STRIDE * index for index in indices
            )
    return schedules


def validate_attempt10_seed_freshness(plan: Mapping[str, Any]) -> None:
    ...
    for population, expected_count in (("development", 200), ("future_audit", 50)):
        for domain, values in enumerate_attempt10_seed_schedules(
            plan, population=population
        ).items():
            if len(values) != expected_count or len(set(values)) != expected_count:
                raise ValueError(f"Attempt10 {population}.{domain} is not unique")
            declared[f"{population}.{domain}"] = values
    for domain, values in enumerate_attempt10_preflight_seed_schedules(plan).items():
        if len(values) != 3 or len(set(values)) != 3:
            raise ValueError(f"Attempt10 preflight.{domain} is not unique")
        declared[f"preflight.{domain}"] = values
    _reject_pairwise_overlap(declared, label="Attempt10 declared seed schedules")
    ...
    for new_name, new_values in declared.items():
        ...
        for old_name, old_values in known.items():
            if new_set.intersection(old_values):
                raise ValueError(f"Attempt10 {new_name} overlaps {old_name}")


def _load_canonical_plan() -> dict[str, Any]:
    if not _CANONICAL_PLAN_PATH.is_file():
        raise ValueError("canonical Attempt10 plan is missing")
    raw = _CANONICAL_PLAN_PATH.read_bytes()
    if hashlib.sha256(raw).hexdigest() != M43_ATTEMPT10_PLAN_SHA256:
        raise ValueError("canonical Attempt10 plan SHA-256 changed")
    value = json.loads(raw.decode("utf-8-sig"))
    if not isinstance(value, dict):
        raise ValueError("canonical Attempt10 plan must be a mapping")
    return value


def _validate_frozen_semantics(plan: Mapping[str, Any]) -> None:
    _require_exact(plan.get("schema"), M43_ATTEMPT10_PLAN_SCHEMA, "plan.schema")
    _require_exact(plan.get("milestone"), "M4.3-attempt10", "plan.milestone")
    ...
    _require_exact(plan.get("profiles"), list(M43_ATTEMPT10_PROFILES), "profiles")
    ...
    _require_exact(decision.get("sha256"), ATTEMPT09_DECISION_SHA256, "decision.sha")
    _require_exact(
        receipt.get("sha256"),
        ATTEMPT09_DECISION_RECEIPT_SHA256,
    ...
    if scope.get("public_information_set_only") is not True:
        raise ValueError("Attempt10 must remain public-information-set only")
    ...
    _require_exact(search.get("learned_nonbaseline_top_k"), 12, "Top12")
    ...
    _require_exact(
        ...
        "rank12_plus_explicit_baseline",
        "rerank.action_scope",
    )
    shortlist = _mapping(search.get("shortlist"), "shortlist")
    _require_exact(shortlist.get("symbol"), "K8", "shortlist.symbol")
    _require_exact(shortlist.get("nonbaseline_actions"), 8, "shortlist.size")
    _require_exact(shortlist.get("rerank_top_actions"), 4, "shortlist.top")
    _require_exact(
        shortlist.get("selection"),
        "R_top4_plus_four_unique_lambda_predicted_risk_reserves_from_R_ranks5_through12",
        "shortlist.selection",
    )
    reserve = _mapping(shortlist.get("risk_reserve"), "shortlist.risk_reserve")
    _require_exact(
        reserve.get("candidate_R_positions_inclusive"),
        [5, 12],
        "shortlist.risk_reserve.positions",
    )
    _require_exact(reserve.get("count"), 4, "shortlist.risk_reserve.count")
    _require_exact(
    ...
    if not guards or any(value is not False for value in guards.values()):
        raise ValueError("all Attempt10 activation guards must remain false")
...
def _validate_profile_balance(plan: Mapping[str, Any]) -> None:
    ...
    for section, expected in (("development_population", 40), ("future_audit_population", 10)):
        ...
        if any(assigned.count(profile) != expected for profile in profiles):
            raise ValueError(f"Attempt10 {section} is not profile-balanced")
...
def _validate_compute_accounting(plan: Mapping[str, Any]) -> None:
    ...
    _require_exact(
        ...
        {
            "rerank_action_futures_per_root": 13 * r,
            "veto_action_futures_per_root": 9 * v,
            "fixed_prefilter_action_futures_per_root": 13 * r + 9 * v,
...
__all__ = [
    ...
    "ATTEMPT10_SHORTLIST_K",
    "ATTEMPT10_STRESS_SAMPLES",
    "ATTEMPT10_STRICT_MIN_P01",
    "ATTEMPT10_STRICT_MIN_P05",
    "ATTEMPT10_STRICT_MIN_VALUE",
    "ATTEMPT10_TOP_K",
    "ATTEMPT10_VETO_MIN_MEAN",
    "ATTEMPT10_VETO_MIN_P01",
    "ATTEMPT10_VETO_MIN_P05",
    "ATTEMPT10_VETO_MIN_VALUE",
    "ATTEMPT10_VETO_SAMPLES",
    "M43_ATTEMPT10_PLAN_SCHEMA",
    "M43_ATTEMPT10_PLAN_SHA256",
    "M43_ATTEMPT10_PROFILES",
    "M43_ATTEMPT10_SEED_STRIDE",
    "enumerate_attempt10_preflight_seed_schedules",
    ...
    "validate_attempt10_seed_freshness",
]
