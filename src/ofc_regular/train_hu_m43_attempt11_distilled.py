"""Train the fixed Attempt11 public-infoset distillation ensemble.
...
from .hu_m43_attempt11_contract import M43_ATTEMPT11_PLAN_SHA256
...
ATTEMPT11_DISTILLATION_CONFIG_SCHEMA = "hu_m43_attempt11_distillation_config_v1"
ATTEMPT11_DISTILLATION_CONFIG_SHA256 = (
    "7098f8a21a2edf56a5158683569c15412f59df083c8f56f630fe060227eff138"
)
ATTEMPT11_DISTILLATION_FULL_ITERATIONS = 180
ATTEMPT11_DISTILLATION_FULL_STATES = 200
ATTEMPT11_DEVELOPMENT_ROW_SCHEMA = "hu_m43_attempt11_search_root_v1"
ATTEMPT11_DISTILLED_TRAINING_MANIFEST_SCHEMA = (
    "hu_m43_attempt11_distilled_training_manifest_v1"
)
ATTEMPT11_DEVELOPMENT_DECISION_SCHEMA = (
    "hu_m43_attempt11_development_go_no_go_v1"
)
ATTEMPT11_DEVELOPMENT_SELECTOR_RECEIPT_SCHEMA = (
    "hu_m43_attempt11_development_selector_receipt_v1"
)
ATTEMPT11_DEVELOPMENT_PASS_FREEZE_SCHEMA = (
    "hu_m43_attempt11_development_go_freeze_v1"
)
ATTEMPT11_PROFILES = (
    "stage19_p0",
    "stage9f_p2",
...
ATTEMPT11_FOLDS = 5
ATTEMPT11_PROPOSAL_ROWS_MAX = ATTEMPT11_CANDIDATE_MAX + 1


@dataclass(frozen=True)
class Attempt11DistillationState:
    root_index: int
    root_profile: str
...
def read_attempt11_jsonl(path: str | Path) -> list[dict[str, Any]]:
    raw = Path(path).read_bytes()
    if not raw or not raw.endswith(b"\n"):
        raise ValueError("Attempt11 distillation JSONL must be non-empty and newline terminated")
    rows: list[dict[str, Any]] = []
    for line_number, line in enumerate(raw.splitlines(), start=1):
        try:
            value = json.loads(line)
        except json.JSONDecodeError as exc:
            raise ValueError(f"Attempt11 distillation JSONL line {line_number} is invalid") from exc
        if not isinstance(value, dict):
            raise ValueError("Attempt11 distillation JSONL rows must be mappings")
        rows.append(value)
    return rows


def prepare_attempt11_distillation_states(
    rows: Sequence[Mapping[str, Any]],
    *,
    ranker: FrozenAttempt11LambdaRanker,
    expected_roots: int | None = 200,
) -> tuple[Attempt11DistillationState, ...]:
    ...
    if expected_roots is not None and len(rows) != expected_roots:
        raise ValueError(f"Attempt11 distillation requires exactly {expected_roots} rows")
    by_root: dict[int, Mapping[str, Any]] = {}
    for raw in rows:
        root_index = _strict_int(raw.get("root_index"), "root_index")
        if root_index in by_root:
            raise ValueError("Attempt11 distillation rejects duplicate root indices")
        by_root[root_index] = raw
    if expected_roots is not None and set(by_root) != set(range(expected_roots)):
        raise ValueError("Attempt11 distillation root indices are not contiguous")
    ...
    for root_index in sorted(by_root):
        ...
        if row.get("schema") != ATTEMPT11_DEVELOPMENT_ROW_SCHEMA:
            raise ValueError("Attempt11 distillation requires canonical search rows")
        ...
        if (
            ...
            raise ValueError(
                "Attempt11 distillation accepts Development200 rows only"
            )
        profile = str(row.get("root_profile"))
        if profile not in ATTEMPT11_PROFILES:
            raise ValueError("Attempt11 distillation root profile is unknown")
        if expected_roots == 200 and profile != ATTEMPT11_PROFILES[root_index % 5]:
            raise ValueError("Attempt11 distillation profile schedule changed")
        ...
        if observation.to_dict() != observation_payload:
            raise ValueError("Attempt11 distillation observation is non-canonical")
        if observation.street != "T1" or observation.seat != "second":
            raise ValueError("Attempt11 distillation authorizes only T1-second")
        fingerprint = observation.fingerprint()
        if row.get("observation_fingerprint") not in (None, fingerprint):
            raise ValueError("Attempt11 distillation observation fingerprint changed")
        if fingerprint in seen_fingerprints:
            raise ValueError("Attempt11 distillation rejects duplicate information sets")
        ...
        if teacher.get("schema") != ATTEMPT11_TEACHER_SCHEMA:
            raise ValueError("Attempt11 distillation teacher schema changed")
        if teacher.get("policy_observation") != observation_payload:
            raise ValueError("Attempt11 distillation teacher observation changed")
        if teacher.get("baseline_action_key") != baseline_token:
            raise ValueError("Attempt11 distillation teacher baseline changed")
        if teacher.get("runtime_gate_allowed") is not False:
            raise ValueError("Attempt11 teacher data cannot be runtime authorization")
        _validate_attempt11_public_teacher(
            observation,
            baseline_action_key=baseline_token,
            teacher=teacher,
        )
        built = build_attempt11_distilled_features(
            sample,
        ...
        teacher_candidates = tuple(str(value) for value in _sequence(
            teacher.get("learned_nonbaseline_action_keys"),
            "learned_nonbaseline_action_keys",
        ))
        ...
        if teacher_candidates != built_candidates:
            raise ValueError("Attempt11 distillation variable candidate mapping changed")
        candidate_count = len(built.candidate_indices)
        if (
            teacher.get("candidate_nonbaseline_count") != candidate_count
            or not 0 <= candidate_count <= ATTEMPT11_CANDIDATE_MAX
            or len(set(teacher_candidates)) != candidate_count
            or baseline_token in teacher_candidates
        ):
            raise ValueError("Attempt11 distillation candidate count/uniqueness changed")
        ...
        if len(rerank_rows) != group_size:
            raise ValueError("Attempt11 distillation requires variable candidates plus baseline R128 rows")
        ...
        if tuple(str(_mapping(item, "rerank action").get("action_key")) for item in rerank_rows) != expected_tokens:
            raise ValueError("Attempt11 distillation R128 action mapping changed")
        ...
        if type(fired) is not bool or type(fallback) is not bool:
            raise ValueError("Attempt11 distillation decision flags are invalid")
        if fired == fallback or (not fired and selected_token != baseline_token):
            raise ValueError("Attempt11 distillation baseline fallback changed")
        if selected_token not in expected_tokens:
            raise ValueError("Attempt11 distillation selected action is outside proposals")
        ...
        for index, raw_rerank in enumerate(rerank_rows):
            ...
            if raw_delta.shape != (128,) or not np.isfinite(raw_delta).all():
                raise ValueError("Attempt11 distillation R128 vector changed")
            ...
            for name, expected in summary.items():
                if not math.isclose(
                    ...
                    raise ValueError("Attempt11 distillation R128 summary changed")
        ...
        if not np.allclose(delta[-1], 0.0, rtol=0.0, atol=1.0e-12) or not np.allclose(
            ...
            raise ValueError("Attempt11 distillation explicit baseline is not zero")
        ...
        if fired:
            if evaluation.get("opened") is not True or len(evaluation_rows) != 2:
                raise ValueError("Attempt11 distillation fired row lacks E256")
            ...
            if selected_evaluation.get("action_key") != selected_token:
                raise ValueError("Attempt11 distillation E256 action mapping changed")
            ...
            raise ValueError("Attempt11 distillation nonfire opened E256")
...
def _validate_attempt11_public_teacher(
    ...
    *,
    ...
    config = Attempt11TeacherConfig(
        frozen_model_sha256=ATTEMPT11_FROZEN_MODEL_SHA256,
    ...
    result = validate_attempt11_teacher_output(
        observation,
    ...
    if (
        ...
        raise ValueError("Attempt11 public teacher validation result changed")


def assign_attempt11_identity_folds(
    states: Sequence[Attempt11DistillationState], *, folds: int = ATTEMPT11_FOLDS
) -> np.ndarray:
    if folds != ATTEMPT11_FOLDS or not states:
        raise ValueError("Attempt11 distillation requires exactly five folds")
    if len({state.observation_fingerprint for state in states}) != len(states):
        raise ValueError("Attempt11 distillation fold identities repeat")
    result = np.full(len(states), -1, dtype=np.int8)
    for profile in ATTEMPT11_PROFILES:
    ...
    if np.any(result < 0):
        raise ValueError("Attempt11 distillation fold assignment found unknown profile")
    return result


def fit_attempt11_distilled_model(
    states: Sequence[Attempt11DistillationState],
    *,
    candidate_ranker: FrozenAttempt11LambdaRanker,
    config: Mapping[str, Any],
    smoke: bool = False,
    model_id: str = "hu-m43-attempt11-distilled-development200-oof",
) -> tuple[HuM43Attempt11DistilledModel, dict[str, Any]]:
    """Fit five train-four/hold-one models and freeze OOF tail cushions."""

    _validate_config(config)
    if len(states) < ATTEMPT11_FOLDS:
        raise ValueError("Attempt11 distillation has too few states")
    folds = assign_attempt11_identity_folds(states)
    training = _mapping(config.get("training"), "training")
    seed = _strict_int(training.get("random_seed"), "random_seed")
    predictors: list[Attempt11DistilledFoldPredictor] = []
    for fold_index in range(ATTEMPT11_FOLDS):
    ...
    model = HuM43Attempt11DistilledModel(
        ...
        manifest={
            ...
            "identity_grouped_folds": ATTEMPT11_FOLDS,
    ...
    diagnostics = {
        ...
        "candidate_count_histogram": {
            ...
            for candidate_count in range(ATTEMPT11_CANDIDATE_MAX + 1)
        },
        "fold_counts": {
            str(fold): int(np.sum(folds == fold)) for fold in range(ATTEMPT11_FOLDS)
        },
        "profile_counts": {
...
def train_attempt11_distilled_artifact(
    *,
    input_path: str | Path,
    ...
    _validate_config(config)
    if _file_sha256(config_path) != ATTEMPT11_DISTILLATION_CONFIG_SHA256:
        raise ValueError("Attempt11 full distillation config SHA changed")
    decision = _load_mapping(decision_path, "development decision")
    if (
        decision.get("schema") != ATTEMPT11_DEVELOPMENT_DECISION_SCHEMA
        or decision.get("decision") != "go"
        or decision.get("search_freeze_authorized") is not True
    ):
        raise ValueError("Attempt11 distillation requires development Go")
    ...
    if decision_source.get("input_jsonl_sha256") != input_sha:
        raise ValueError("Attempt11 development decision does not bind training JSONL")
    ...
    if (
        ...
        raise ValueError("Attempt11 development selector receipt is not fit authorization")
    if (
        pass_freeze.get("schema") != ATTEMPT11_DEVELOPMENT_PASS_FREEZE_SCHEMA
        or pass_freeze.get("status") != "go_freeze_attempt11_development"
        ...
        raise ValueError("Attempt11 immutable development-pass freeze is not bound")
    ...
    if (
        freeze_bindings["plan_sha256"] != M43_ATTEMPT11_PLAN_SHA256
        ...
        or decision_source.get("plan_sha256") != M43_ATTEMPT11_PLAN_SHA256
        ...
        raise ValueError("Attempt11 development source hash chain changed")
    ...
    if (
        ...
        or teacher_contract.get("plan_sha256") != M43_ATTEMPT11_PLAN_SHA256
        or teacher_contract.get("candidate_model_sha256")
        != ATTEMPT11_FROZEN_MODEL_SHA256
    ):
        raise ValueError("Attempt11 runtime source/teacher closure changed")
    ...
    ranker = FrozenAttempt11LambdaRanker.load(
        candidate_model_path, expected_sha256=ATTEMPT11_FROZEN_MODEL_SHA256
    )
    rows = read_attempt11_jsonl(input_path)
    states = prepare_attempt11_distillation_states(rows, ranker=ranker, expected_roots=200)
    model, diagnostics = fit_attempt11_distilled_model(
        states, candidate_ranker=ranker, config=config, smoke=smoke
    )
    model_sha = model.save(output_model_path)
    manifest = {
        "schema": ATTEMPT11_DISTILLED_TRAINING_MANIFEST_SCHEMA,
        ...
        "source": {
            ...
            "candidate_model_sha256": ATTEMPT11_FROZEN_MODEL_SHA256,
...
def _fit_fold(
    ...
    *,
    ...
    if not states:
        raise ValueError("Attempt11 distilled fold has no training states")
    ...
    if any(not 1 <= group <= ATTEMPT11_PROPOSAL_ROWS_MAX for group in groups):
        raise ValueError("Attempt11 distilled group size is outside 1..13")
    if sum(groups) != x.shape[0]:
        raise ValueError("Attempt11 distilled LightGBM groups do not match row total")
    ...
    return Attempt11DistilledFoldPredictor(
        ranker=ranker,
...
def _profile_state_weights(states: Sequence[Attempt11DistillationState]) -> np.ndarray:
    counts = {profile: sum(state.root_profile == profile for state in states) for profile in ATTEMPT11_PROFILES}
...
def _validate_config(config: Mapping[str, Any]) -> None:
    if config.get("schema") != ATTEMPT11_DISTILLATION_CONFIG_SCHEMA:
        raise ValueError("Attempt11 distillation config schema changed")
    schemas = _mapping(config.get("schemas"), "schemas")
    if schemas.get("model") != HU_M43_ATTEMPT11_DISTILLED_MODEL_SCHEMA:
        raise ValueError("Attempt11 distillation model schema changed")
    candidate = _mapping(config.get("candidate_generation"), "candidate_generation")
    if (
        candidate.get("source_model_sha256") != ATTEMPT11_FROZEN_MODEL_SHA256
        or candidate.get("nonbaseline_max") != ATTEMPT11_CANDIDATE_MAX
        or candidate.get("actual_count_range") != [0, ATTEMPT11_CANDIDATE_MAX]
        ...
        or candidate.get("lightgbm_group_size_range") != [1, ATTEMPT11_PROPOSAL_ROWS_MAX]
        or candidate.get("feature_dimension") != HU_M43_ATTEMPT11_DISTILLED_FEATURE_DIM
    ):
        raise ValueError("Attempt11 distillation candidate contract changed")
    ...
    if (
        ...
        or gate.get("action_score_mode") != HU_M43_ATTEMPT11_DISTILLED_ACTION_SCORE_MODE
    ):
        raise ValueError("Attempt11 distillation runtime gate changed")
    training = _mapping(config.get("training"), "training")
    if training.get("threshold_sweep_allowed") is not False:
        raise ValueError("Attempt11 distillation threshold sweep is forbidden")
    if _mapping(config.get("fit_authorization"), "fit_authorization") != {
        ...
        raise ValueError("Attempt11 distillation fit authorization changed")
    ...
    if isinstance(value, Mapping):
        for key, child in value.items():
            ...
            if lowered in {"opponent_private_discards", "opponent_discards"}:
                raise ValueError(f"Attempt11 distillation exposes hidden information at {path}")
...
def _mapping(value: Any, label: str) -> dict[str, Any]:
    if not isinstance(value, Mapping):
        raise ValueError(f"Attempt11 distillation {label} must be a mapping")
    return dict(value)


def _sequence(value: Any, label: str) -> list[Any]:
    if isinstance(value, (str, bytes, bytearray)) or not isinstance(value, Sequence):
        raise ValueError(f"Attempt11 distillation {label} must be a sequence")
    return list(value)


def _strict_int(value: Any, label: str) -> int:
    if isinstance(value, bool) or not isinstance(value, (int, np.integer)):
        raise ValueError(f"Attempt11 distillation {label} must be an integer")
    return int(value)
...
def _finite(value: Any, label: str) -> float:
    try:
        ...
        raise ValueError(f"Attempt11 distillation {label} must be numeric") from exc
    if not math.isfinite(result):
        raise ValueError(f"Attempt11 distillation {label} must be finite")
    return result
...
def _require_sha256(value: Any, label: str) -> str:
    if not isinstance(value, str):
        raise ValueError(f"Attempt11 distillation {label} must be a SHA-256 string")
    normalized = value.lower()
    if len(normalized) != 64 or any(
        character not in "0123456789abcdef" for character in normalized
    ):
        raise ValueError(f"Attempt11 distillation {label} is invalid")
    return normalized
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Fit frozen Attempt11 distillation heads")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    manifest = train_attempt11_distilled_artifact(
...
__all__ = [
    ...
    "prepare_attempt11_distillation_states",
    "read_attempt11_jsonl",
    "train_attempt11_distilled_artifact",
]
