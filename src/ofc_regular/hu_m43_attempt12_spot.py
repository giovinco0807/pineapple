"""Attempt12 binding for the immutable Attempt09 Spot lifecycle.
...
own.  This module binds that machinery to the frozen Attempt12 contract for
...
PACKAGE_SCHEMA = "hu_m43_attempt12_spot_package_v1"
LAUNCH_AUTHORIZATION_SCHEMA = "hu_m43_attempt12_spot_launch_authorization_v1"
DONE_SCHEMA = "hu_m43_attempt12_done_v1"
RECEIVE_SCHEMA = "hu_m43_attempt12_receive_v1"
SHARD_SCHEMA = "hu_m43_attempt12_spot_shard_v1"
LAUNCH_WAVE_SCHEMA = "hu_m43_attempt12_launch_wave_v1"
STATUS_SCHEMA = "hu_m43_attempt12_status_v1"
RECEIVED_SHARD_AUDIT_SCHEMA = "hu_m43_attempt12_received_shard_audit_v1"
PREFLIGHT_RESULT_SCHEMA = "hu_m43_attempt12_preflight_result_v1"
SOURCE_NAME = "ofc_regular_hu_m43_attempt12_source.zip"
SCHEDULE_NAME = _base.SCHEDULE_NAME
STARTUP_NAME = "startup_hu_m43_attempt12.sh"
...
PLAN_RELATIVE = "configs/hu_joint_policy_m43_attempt12.json"
GATE_RELATIVE = "artifacts/attempt12/preceding_gate.json"
OVERLAY_RELATIVES = (
    PLAN_RELATIVE,
    ...
    "src/ofc_regular/run_hu_m43_attempt12.py",
    # The generic root persistence runner and its eager import closure are
    # required by run_hu_m43_attempt12 inside the immutable VM package.
...
EXPECTED_GATES = {
    ...
    "future_audit": ("go_freeze_attempt12_development", "go"),
}

_BIND_LOCK = threading.RLock()


def _build_attempt12_schedule(mode: str, run_name: str) -> list[dict[str, Any]]:
    if not _base._SAFE_RUN.fullmatch(run_name):
        raise ValueError("Attempt12 run_name is not a safe GCP identity")
    if mode == "preflight":
        specs = [
        ...
        rows.append(
            {
                ...
                "root_profile": M43_ATTEMPT12_PROFILES[
                    root_index % len(M43_ATTEMPT12_PROFILES)
                ],
...
_ATTEMPT12_BINDINGS: dict[str, Any] = {
    ...
    "build_schedule": _build_attempt12_schedule,
}


@contextmanager
def _attempt12_bindings() -> Iterator[None]:
    """Install Attempt12 globals for one call, then restore Attempt09 exactly."""
...
def _validate_root4_variable_candidate_teacher(teacher: dict[str, Any]) -> int:
    ...
    if type(n) is not int or not 0 <= n <= 26:
        raise ValueError("Attempt12 root4 candidate count is outside 0..26")
    baseline = teacher.get("baseline_action_key")
    candidate_keys = teacher.get("all_legal_nonbaseline_action_keys")
    ...
    if not all(
        ...
        raise ValueError("Attempt12 root4 variable candidate mappings are missing")
    ...
    if (
        ...
        raise ValueError("Attempt12 root4 variable candidate mapping changed")
    return n


def _finalize_attempt12_preflight(
    *, received_dir: str | Path, output: str | Path
) -> dict[str, Any]:
    root = Path(received_dir).resolve()
    receipt_path = root / "merged" / "receive_receipt.json"
    receipt = _base._load_canonical(receipt_path, "Attempt12 preflight receipt")
    expected_root_indices = [0, 0, 0, 1, 2, 3, 4]
    expected_profiles = [
        M43_ATTEMPT12_PROFILES[index % len(M43_ATTEMPT12_PROFILES)]
    ...
    if (
        ...
        raise ValueError("Attempt12 preflight requires exact seven-slot full-profile proof")
    merged_path = root / "merged" / "teacher.jsonl"
    if _base.sha256_file(merged_path) != _base._require_sha256(
        receipt.get("merged_sha256"), "Attempt12 preflight merged"
    ):
        raise ValueError("Attempt12 preflight merged bytes changed")
    if {path.name for path in root.iterdir()} != {"shards", "audits", "merged"}:
        raise ValueError("Attempt12 preflight receive root changed")
    ...
    if [path.name for path in audit_paths] != expected_audits:
        raise ValueError("Attempt12 preflight audit exact set changed")
    audit_payloads = [
        _base._load_canonical(path, f"Attempt12 preflight audit {index}")
        for index, path in enumerate(audit_paths)
    ]
    if hashlib.sha256(_base._canonical_jsonl(audit_payloads)).hexdigest() != (
        _base._require_sha256(receipt.get("audit_sha256"), "Attempt12 audits")
    ):
        raise ValueError("Attempt12 preflight audit bytes changed")
    shard_rows: list[dict[str, Any]] = []
    for shard in range(7):
        directories = sorted((root / "shards").glob(f"shard-{shard:03d}-*"))
        if len(directories) != 1:
            raise ValueError(f"Attempt12 preflight shard {shard} is missing")
    ...
    if not deterministic:
        raise ValueError("Attempt12 repeated batch root is not byte-identical")
    ...
    if not scalar_batch:
        raise ValueError("Attempt12 scalar/batch teacher payloads differ")
    ...
    if not inherited.is_file() or _base.sha256_file(inherited) != inherited_hash:
        raise ValueError("Attempt12 inherited exact-engine parity evidence changed")
    ...
    if inherited_payload.get("status") != (
        ...
        raise ValueError("Attempt12 inherited exact-engine parity did not pass")
...
def finalize_preflight(
    *, received_dir: str | Path, output: str | Path
    ...
    with _attempt12_bindings():
        return _finalize_attempt12_preflight(
