"""Freeze, preflight, and accept the Attempt12 distilled population policy.
...
from .hu_m43_attempt12_distilled_model import (
    BoundHuM43Attempt12DistilledModel,
...
from .select_hu_m43_attempt12_audit50 import (
    ATTEMPT12_AUDIT50_DECISION_SCHEMA,
    ATTEMPT12_AUDIT50_RECEIPT_KEYS,
    ATTEMPT12_AUDIT50_RECEIPT_SCHEMA,
)
from .merge_hu_m4_population_shards import M4_POPULATION_MERGE_SCHEMA
from .train_hu_m43_attempt12_distilled import (
...
ATTEMPT12_POPULATION_PLAN_SCHEMA = (
    "hu_m43_population_acceptance_plan_v1"
)
ATTEMPT12_POPULATION_PREFLIGHT_SCHEMA = (
    "hu_m43_attempt12_population_launch_preflight_v1"
)
ATTEMPT12_RUNTIME_FREEZE_SCHEMA = "hu_m43_attempt12_distilled_runtime_freeze_v1"
ATTEMPT12_POPULATION_ACCEPTANCE_STATUS_SCHEMA = (
    "hu_m43_attempt12_population_acceptance_status_v1"
)
ATTEMPT12_AUDIT50_SELECTOR_RECEIPT_SCHEMA = ATTEMPT12_AUDIT50_RECEIPT_SCHEMA
ATTEMPT12_OPPONENTS = (
    "stage19_p0",
    "stage9f_p2",
    "stage7_m5_r10",
...
# This namespace is already reserved by the frozen Attempt12 search plan.  The
# final population plan remains a separately authored artifact and must supply
# its own file hash at preflight/acceptance time.
ATTEMPT12_POPULATION_SEED = 220_108_071_901
ATTEMPT12_POPULATION_SEED_STRIDE = 1_000_003
ATTEMPT12_POPULATION_SEEDS = 1000
ATTEMPT12_POPULATION_SHARDS = 20
ATTEMPT12_POPULATION_SEEDS_PER_SHARD = 50
ATTEMPT12_POPULATION_PLAN_SHA256 = (
    "66d9c3dc50bcd6ae2ce72e2a558dcfae5720d3cb2387a16ef810d1498f4db462"
)
ATTEMPT12_EXCLUDED_NAMESPACE_BASES = (
    200_108_071_901,
    201_108_071_901,
    202_108_071_901,
    203_108_071_901,
...
ATTEMPT12_BASELINE_PROFILE = "stage19_p0"
ATTEMPT12_FIXED_GATES = {
...
ATTEMPT12_SEED_REGISTRY_SOURCES = ATTEMPT12_DISTILLED_RUNTIME_REGISTRY_PATHS
ATTEMPT12_SEARCH_PLAN_REGISTRY_PATH = "configs/hu_joint_policy_m43_attempt12.json"
ATTEMPT11_POPULATION_REGISTRY_PATH = (
    "configs/hu_joint_policy_m43_attempt11_population.json"
)
_REQUIRED_REGISTRY_SOURCES = frozenset(
    {ATTEMPT12_SEARCH_PLAN_REGISTRY_PATH, ATTEMPT11_POPULATION_REGISTRY_PATH}
)


def load_and_validate_attempt12_population_plan(
    source: str | Path | Mapping[str, Any]
) -> dict[str, Any]:
    if isinstance(source, Mapping):
        plan = dict(source)
    else:
        if _file_sha256(source) != ATTEMPT12_POPULATION_PLAN_SHA256:
            raise ValueError("Attempt12 frozen population plan SHA changed")
        plan = _load_mapping(source, "plan")
    if (
        plan.get("schema") != ATTEMPT12_POPULATION_PLAN_SCHEMA
        or plan.get("milestone") != "M4.3-attempt12-final-population"
        or plan.get("status") != "frozen_before_population_evaluation"
        or plan.get("purpose")
        != "fresh_realized_match_ev_acceptance_for_attempt12_distilled_t1_second_selector"
        or plan.get("policy_attempt") != "attempt12_distilled_v1"
        or plan.get("fixed_baseline_profile") != ATTEMPT12_BASELINE_PROFILE
        or plan.get("opponents") != list(ATTEMPT12_OPPONENTS)
        or plan.get("paired_seat_swap") is not True
    ):
        raise ValueError("Attempt12 population plan identity changed")
    if plan.get("baseline_chain") != [
        ...
        raise ValueError("Attempt12 population baseline chain changed")
    runtime = _mapping(plan.get("runtime_contract"), "runtime_contract")
    expected_runtime = {
        "model_schema": HU_M43_ATTEMPT12_DISTILLED_MODEL_SCHEMA,
        "artifact_schema": HU_M43_ATTEMPT12_DISTILLED_ARTIFACT_SCHEMA,
    ...
    if runtime != expected_runtime:
        raise ValueError("Attempt12 population runtime contract changed")
    schedule = (
    ...
    if schedule != (
        ATTEMPT12_POPULATION_SEEDS,
        ATTEMPT12_POPULATION_SEED,
        ATTEMPT12_POPULATION_SEED_STRIDE,
        ATTEMPT12_POPULATION_SHARDS,
        ATTEMPT12_POPULATION_SEEDS_PER_SHARD,
    ):
        raise ValueError("Attempt12 population schedule changed")
    if (
        ...
        raise ValueError("Attempt12 population power contract changed")
    ...
    if sizing != {
        ...
        raise ValueError("Attempt12 population optional-stopping contract changed")
    ...
    if evaluation_contract != {
        ...
        raise ValueError("Attempt12 population counterfactual contract changed")
    ...
    if spot != {
        ...
        raise ValueError("Attempt12 population Spot execution contract changed")
    if _mapping(plan.get("fixed_acceptance_gates"), "gates") != ATTEMPT12_FIXED_GATES:
        raise ValueError("Attempt12 fixed acceptance gates changed")
    ...
    if set(guards) != expected_guard_names or any(
        ...
        raise ValueError("Attempt12 population activation guard changed")
    if _mapping(
        ...
        raise ValueError("Attempt12 post-acceptance activation contract changed")
    planned = {
        ATTEMPT12_POPULATION_SEED + index * ATTEMPT12_POPULATION_SEED_STRIDE
        for index in range(ATTEMPT12_POPULATION_SEEDS)
    }
    freshness = _mapping(plan.get("freshness"), "freshness")
    for name in (
        "exclude_all_m4_m41_m42_m43_teacher_hand_seeds",
        "exclude_attempt12_preflight_development_and_audit_namespaces",
        ...
        if freshness.get(name) is not True:
            raise ValueError("Attempt12 population freshness contract changed")
    if (
        freshness.get("planned_overlap_count_at_freeze") != 0
        or freshness.get("alternate_seed_after_result_allowed") is not False
    ):
        raise ValueError("Attempt12 population seed-selection contract changed")
    ...
    if registry_sources != ATTEMPT12_SEED_REGISTRY_SOURCES:
        raise ValueError("Attempt12 seed registry source list changed")
    if not _REQUIRED_REGISTRY_SOURCES.issubset(registry_sources):
        raise ValueError("Attempt12 required seed registry source is missing")
    registry_seeds, registry_rows = _load_seed_registry(registry_sources)
    registry_overlap = planned & registry_seeds
    if registry_overlap:
        raise ValueError("Attempt12 population seed grid overlaps hashed registry")
    registry_digest = hashlib.sha256(
        _canonical(registry_rows).encode("utf-8")
    ).hexdigest()
    if freshness.get("excluded_schedule_registry_sha256") != registry_digest:
        raise ValueError("Attempt12 hashed seed registry snapshot changed")
    plan["_freshness_registry"] = {
        "schema": "hu_m43_attempt12_seed_registry_snapshot_v1",
    ...
    raw_namespace_bases = _sequence(
        freshness.get("excluded_attempt12_namespace_bases"), "namespace bases"
    )
    if raw_namespace_bases != list(ATTEMPT12_EXCLUDED_NAMESPACE_BASES):
        raise ValueError("Attempt12 excluded namespace contract changed")
    teacher: set[int] = set()
    for raw_base in raw_namespace_bases:
        base = _integer(raw_base, "namespace base")
        teacher.update(
            base + index * ATTEMPT12_POPULATION_SEED_STRIDE for index in range(250)
        )
    ...
    if any(plan["_freshness_counts"].values()):
        raise ValueError("Attempt12 population seed grid overlaps prior evidence")
    if not any(
        isinstance(row, Mapping)
        and row.get("seed") == 190_108_071_901
        and row.get("seed_stride") == ATTEMPT12_POPULATION_SEED_STRIDE
        and row.get("paired_seeds") == 1000
        for row in freshness.get("excluded_population_schedules", [])
    ):
        raise ValueError("Attempt12 prior Attempt11 population exclusion is missing")
    return plan
...
def _load_seed_registry(
    ...
    for relative in relative_sources:
        ...
        if candidate.is_absolute() or ".." in candidate.parts:
            raise ValueError("Attempt12 seed registry path escapes repository")
        source = (root / candidate).resolve()
        try:
            source.relative_to(root)
        except ValueError as exc:
            raise ValueError("Attempt12 seed registry path escapes repository") from exc
        if not source.is_file():
            raise ValueError(f"Attempt12 seed registry source missing: {relative}")
        payload = _load_mapping(source, f"seed registry {relative}")
        source_seeds = _extract_declared_seed_schedules(payload)
        authorized_reservation_count = 0
        if relative == ATTEMPT12_SEARCH_PLAN_REGISTRY_PATH:
            search_plan = load_and_validate_attempt12_plan(source)
            seed_contract = _mapping(
                search_plan.get("seed_contract"), "Attempt12 search seed contract"
            )
            if (
                seed_contract.get("tentative_future_population_seed_base")
                != ATTEMPT12_POPULATION_SEED
                or seed_contract.get(
                    "tentative_future_population_overlap_count_at_freeze"
                )
                != 0
                or seed_contract.get("seed_stride")
                != ATTEMPT12_POPULATION_SEED_STRIDE
            ):
                raise ValueError("Attempt12 reserved population namespace changed")
            reservation = {
                ATTEMPT12_POPULATION_SEED
                + index * ATTEMPT12_POPULATION_SEED_STRIDE
                for index in range(ATTEMPT12_POPULATION_SEEDS)
            }
            if not reservation.issubset(source_seeds):
                raise ValueError("Attempt12 reserved population namespace was not indexed")
            source_seeds.difference_update(reservation)
            authorized_reservation_count = len(reservation)
        ...
        if authorized_reservation_count:
            row["authorized_self_reservation_seed_count"] = (
                authorized_reservation_count
            )
        rows.append(row)
    return seeds, rows
...
def _extract_declared_seed_schedules(payload: Mapping[str, Any]) -> set[int]:
    ...
    def visit(value: Any, inherited_stride: int = ATTEMPT12_POPULATION_SEED_STRIDE) -> None:
...
def freeze_attempt12_distilled_runtime(
    *,
    ...
    expected_runtime_bindings = {
        ...
        "runtime_fingerprint_sha256": ATTEMPT12_EXPECTED_RUNTIME_FINGERPRINT_SHA256,
    ...
    if any(
        ...
        raise ValueError("Attempt12 training/runtime source freeze mismatch")
    if (
        ...
        raise ValueError("Attempt12 external runtime identity changed")
    _validate_attempt12_audit50_go_authorization(
        audit,
        audit_receipt,
        audit_decision_sha256=audit_sha,
    )
    source_model = HuM43Attempt12DistilledModel.load(
        source_model_path, expected_sha256=source_sha
    )
    if source_model.safety_enabled or source_model.winner_frozen:
        raise ValueError("Attempt12 source model is already runtime frozen")
    ...
    if any(
        ...
        for name, value in {
            ...
            "fit_mode": "full",
            "effective_iterations": ATTEMPT12_DISTILLATION_FULL_ITERATIONS,
            "training_states": ATTEMPT12_DISTILLATION_FULL_STATES,
            "distillation_config_sha256": ATTEMPT12_DISTILLATION_CONFIG_SHA256,
        }.items()
    ):
        raise ValueError("Attempt12 source model is not the frozen full-fit artifact")
    ...
    freeze = {
        "schema": ATTEMPT12_RUNTIME_FREEZE_SCHEMA,
...
def _validate_attempt12_audit50_receipt_for_freeze(
    receipt: Mapping[str, Any], *, audit_decision_sha256: str
) -> None:
    if (
        set(receipt) != ATTEMPT12_AUDIT50_RECEIPT_KEYS
        or receipt.get("schema") != ATTEMPT12_AUDIT50_SELECTOR_RECEIPT_SCHEMA
        ...
        raise ValueError("Attempt12 audit50 selector receipt is not freeze authorization")


def _validate_attempt12_audit50_go_authorization(
    audit: Mapping[str, Any],
    receipt: Mapping[str, Any],
    *,
    audit_decision_sha256: str,
) -> None:
    """Require the one declared Audit50 gate evaluation and its exact receipt."""

    _sha256(audit_decision_sha256, "audit50 decision SHA")
    audit_science = _mapping(audit.get("science_boundary"), "audit50 science")
    if (
        audit.get("schema") != ATTEMPT12_AUDIT50_DECISION_SCHEMA
        or audit.get("status") != "go_attempt12_audit50_search_quality"
        ...
        raise ValueError("Attempt12 runtime freeze requires the one-shot audit50 Go")
    ...
    if (
        ...
        raise ValueError("Attempt12 audit50 Go contract changed")
    _validate_attempt12_audit50_receipt_for_freeze(
        receipt, audit_decision_sha256=audit_decision_sha256
    )


def load_bound_attempt12_distilled_model(
    path: str | Path,
    *,
    expected_sha256: str,
    ...
    execution_attestation = validate_frozen_execution_modules(
        ...
        manifest=runtime_source_manifest_path,
        module_names=ATTEMPT12_BOUND_EXECUTION_MODULES,
    )
    actual_sha = _file_sha256(path)
    if actual_sha != _sha256(expected_sha256, "model SHA"):
        raise ValueError("Attempt12 bound model SHA mismatch")
    freeze = (
        dict(runtime_freeze)
    ...
    _validate_runtime_dependency_semantics(runtime_semantic, runtime_dependencies)
    if (
        freeze.get("schema") != ATTEMPT12_RUNTIME_FREEZE_SCHEMA
        ...
        raise ValueError("Attempt12 runtime freeze contract changed")
    ...
    if training.get("model_id") != freeze.get("model_id"):
        raise ValueError("Attempt12 training manifest/runtime freeze mismatch")
    training_source = _mapping(training.get("source"), "training source")
    for name in (
        ...
        if training_source.get(name) != freeze.get(name):
            raise ValueError("Attempt12 runtime source hash chain changed")
    model = HuM43Attempt12DistilledModel.load(path, expected_sha256=actual_sha)
    ...
    if any(
        ...
        raise ValueError("Attempt12 frozen model full-fit contract changed")
    for name in (
        ...
        if embedded_manifest.get(name) != freeze.get(name):
            raise ValueError("Attempt12 frozen model semantic binding changed")
    if (
        ...
        raise ValueError("Attempt12 frozen model payload disagrees with freeze")
    return BoundHuM43Attempt12DistilledModel(model, execution_attestation)


def _validate_attempt12_population_provenance_bundle(
    *,
    ...
    """Re-open every Go artifact; hash-only orphan claims cannot package."""
    ...
    for name in (
        ...
        if training_source.get(name) != hashes[name]:
            raise ValueError("Attempt12 development provenance bytes changed")
    for name in ("audit50_decision_sha256", "audit50_selector_receipt_sha256"):
        if runtime_freeze.get(name) != hashes[name]:
            raise ValueError("Attempt12 audit50 provenance bytes changed")
    ...
    pass_freeze = _load_mapping(
        development_pass_freeze_path, "development pass freeze"
    )
    if (
        ...
        or receipt.get("schema") != ATTEMPT12_DEVELOPMENT_SELECTOR_RECEIPT_SCHEMA
        or receipt.get("status") != "single_frozen_gate_evaluation_complete"
        ...
        or receipt.get("current_profile_mutated") is not False
        or receipt.get("runtime_policy_activated") is not False
        or pass_freeze.get("schema") != ATTEMPT12_DEVELOPMENT_PASS_FREEZE_SCHEMA
        ...
        raise ValueError("Attempt12 development provenance is not a complete Go")
    bindings = _mapping(pass_freeze.get("bindings"), "development freeze bindings")
    if (
        bindings.get("plan_sha256") != M43_ATTEMPT12_PLAN_SHA256
        ...
        raise ValueError("Attempt12 development freeze binding changed")
...
def build_attempt12_population_preflight(
    *,
    model_path: str | Path,
    training_manifest_path: str | Path,
    ...
    plan = load_and_validate_attempt12_population_plan(population_plan_path)
    ...
    model = load_bound_attempt12_distilled_model(
        model_path,
    ...
    if all(path is not None for path in provenance_paths):
        provenance = _validate_attempt12_population_provenance_bundle(
        ...
        raise ValueError("Attempt12 provenance bundle must be complete")
    else:
        # Final acceptance reuses the launch-preflight hash chain.  The CLI
        # launch preflight itself requires and re-opens all five artifacts.
        provenance = {
    ...
    return {
        "schema": ATTEMPT12_POPULATION_PREFLIGHT_SCHEMA,
        "status": "pass",
        ...
        "baseline_profile": ATTEMPT12_BASELINE_PROFILE,
...
def validate_attempt12_population_acceptance(
    *,
    ...
    recomputed = summarize_hu_m4_population_records(
        records,
        opponents=ATTEMPT12_OPPONENTS,
        paired_seeds=ATTEMPT12_POPULATION_SEEDS,
        seed=ATTEMPT12_POPULATION_SEED,
        seed_stride=ATTEMPT12_POPULATION_SEED_STRIDE,
        baseline_profile=ATTEMPT12_BASELINE_PROFILE,
    )
    ...
    runtime_match = (
        ...
        and runtime.get("model_schema") == HU_M43_ATTEMPT12_DISTILLED_MODEL_SCHEMA
        and runtime.get("artifact_schema") == HU_M43_ATTEMPT12_DISTILLED_ARTIFACT_SCHEMA
        and runtime.get("feature_schema") == HU_M43_ATTEMPT12_DISTILLED_FEATURE_SCHEMA
        and runtime.get("head_schema") == HU_M43_ATTEMPT12_DISTILLED_HEAD_SCHEMA
        and runtime.get("model_id") == preflight["model_id"]
        and runtime.get("action_score_mode")
        == HU_M43_ATTEMPT12_DISTILLED_ACTION_SCORE_MODE
        and runtime.get("baseline_profile") == ATTEMPT12_BASELINE_PROFILE
        ...
        and runtime.get("shard_count") == ATTEMPT12_POPULATION_SHARDS
    ...
    merge_match = (
        ...
        and merge_manifest.get("seed") == ATTEMPT12_POPULATION_SEED
        and merge_manifest.get("seed_stride") == ATTEMPT12_POPULATION_SEED_STRIDE
        and merge_manifest.get("paired_seeds_per_opponent")
        == ATTEMPT12_POPULATION_SEEDS
        and merge_manifest.get("opponents") == list(ATTEMPT12_OPPONENTS)
        ...
        and len(merge_manifest.get("shards")) == ATTEMPT12_POPULATION_SHARDS
    )
    gates = [
        _gate(
    ...
    return {
        "schema": ATTEMPT12_POPULATION_ACCEPTANCE_STATUS_SCHEMA,
        "status": decision,
        "passed_gates": passed,
...
def _validate_training_manifest(
    ...
    if (
        source.get("development_plan_sha256") != M43_ATTEMPT12_PLAN_SHA256
        ...
        != {
            "fit_mode": "full",
            "effective_iterations": ATTEMPT12_DISTILLATION_FULL_ITERATIONS,
            "states": ATTEMPT12_DISTILLATION_FULL_STATES,
            "folds": 5,
        }
        or diagnostics.get("states") != ATTEMPT12_DISTILLATION_FULL_STATES
        or not _valid_variable_training_diagnostics(diagnostics)
        or diagnostics.get("fold_counts")
        != {str(index): 40 for index in range(5)}
    ):
        raise ValueError("Attempt12 training runtime semantic binding changed")
    if (
        manifest.get("schema") != ATTEMPT12_DISTILLED_TRAINING_MANIFEST_SCHEMA
        or manifest.get("status") != "fit_complete_runtime_disabled"
        or manifest.get("model_schema") != HU_M43_ATTEMPT12_DISTILLED_MODEL_SCHEMA
        or manifest.get("feature_schema") != HU_M43_ATTEMPT12_DISTILLED_FEATURE_SCHEMA
        or manifest.get("head_schema") != HU_M43_ATTEMPT12_DISTILLED_HEAD_SCHEMA
        ...
        raise ValueError("Attempt12 source training manifest contract changed")


def _valid_variable_training_diagnostics(diagnostics: Mapping[str, Any]) -> bool:
    """Bind every LightGBM state group to 0..26 candidates plus baseline."""

    raw_groups = diagnostics.get("group_sizes")
    if (
        not isinstance(raw_groups, list)
        or len(raw_groups) != ATTEMPT12_DISTILLATION_FULL_STATES
        or any(
            isinstance(value, bool)
            or not isinstance(value, int)
            or not 1 <= value <= ATTEMPT12_PROPOSAL_ROWS_MAX
    ...
    expected_histogram = {
        ...
        for candidate_count in range(ATTEMPT12_PROPOSAL_ROWS_MAX)
    }
    return (
...
def _validate_runtime_dependency_semantics(
    ...
    if teacher != {
        ...
        raise ValueError("Attempt12 runtime teacher semantic binding changed")
    expected = {
...
def _sequence(value: Any, label: str) -> list[Any]:
    if isinstance(value, (str, bytes, bytearray)) or not isinstance(value, Sequence):
        raise ValueError(f"Attempt12 {label} must be a sequence")
    return list(value)


def _integer(value: Any, label: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int):
        raise ValueError(f"Attempt12 {label} must be an integer")
    return value


def _sha256(value: Any, label: str) -> str:
    if not isinstance(value, str):
        raise ValueError(f"Attempt12 {label} must be a SHA-256 string")
    normalized = value.lower()
    if len(normalized) != 64 or any(c not in "0123456789abcdef" for c in normalized):
        raise ValueError(f"Attempt12 {label} is invalid")
    return normalized
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    preflight.add_argument("--development-decision", required=True, type=Path)
    preflight.add_argument(
        "--development-selector-receipt", required=True, type=Path
    )
    preflight.add_argument("--development-pass-freeze", required=True, type=Path)
    preflight.add_argument("--audit-decision", required=True, type=Path)
    preflight.add_argument("--audit-selector-receipt", required=True, type=Path)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "freeze":
        result = freeze_attempt12_distilled_runtime(
    ...
    if args.command == "preflight":
        result = build_attempt12_population_preflight(
    ...
    status = validate_attempt12_population_acceptance(
...
__all__ = [
    ...
    "ATTEMPT12_POPULATION_ACCEPTANCE_STATUS_SCHEMA",
    "ATTEMPT12_POPULATION_PREFLIGHT_SCHEMA",
    "ATTEMPT12_RUNTIME_FREEZE_SCHEMA",
    "build_attempt12_population_preflight",
    "freeze_attempt12_distilled_runtime",
    "load_and_validate_attempt12_population_plan",
    "load_bound_attempt12_distilled_model",
    "validate_attempt12_population_acceptance",
]
