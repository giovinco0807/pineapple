"""Train the fixed Attempt13 public-infoset distillation ensemble.
...
from .hu_m43_attempt13_distilled_model import (
    ...
    Attempt13DistilledFoldPredictor,
    HuM43Attempt13DistilledModel,
    build_attempt13_distilled_features,
)
...
from .freeze_hu_m43_attempt13_development import (
    ATTEMPT13_DEVELOPMENT_GO_FREEZE_SCHEMA as _DEVELOPMENT_GO_FREEZE_SCHEMA,
    ATTEMPT13_DEVELOPMENT_GO_FREEZE_STATUS as _DEVELOPMENT_GO_FREEZE_STATUS,
)
from .run_hu_m43_attempt13 import ATTEMPT13_ROW_SCHEMA
from .select_hu_m43_attempt13_audit50 import (
    ATTEMPT13_AUDIT50_DECISION_SCHEMA as _AUDIT50_DECISION_SCHEMA,
    ATTEMPT13_AUDIT50_RECEIPT_SCHEMA as _AUDIT50_RECEIPT_SCHEMA,
)
from .select_hu_m43_attempt13_development import (
    ATTEMPT13_DEVELOPMENT_DECISION_SCHEMA as _DEVELOPMENT_DECISION_SCHEMA,
    ATTEMPT13_DEVELOPMENT_RECEIPT_SCHEMA as _DEVELOPMENT_RECEIPT_SCHEMA,
)
from .hu_turn3_model import hu_policy_sample


ATTEMPT13_DISTILLATION_CONFIG_SCHEMA = "hu_m43_attempt13_distillation_config_v1"
ATTEMPT13_DISTILLATION_CONFIG_SHA256 = (
    "f4b7ff1efdf828a570954e17e394baa1f9e805fd46ba04f3c6324928617ecbd9"
)
ATTEMPT13_DISTILLATION_FULL_ITERATIONS = 180
ATTEMPT13_DISTILLATION_FULL_STATES = 200
ATTEMPT13_DEVELOPMENT_ROW_SCHEMA = ATTEMPT13_ROW_SCHEMA
ATTEMPT13_DISTILLED_TRAINING_MANIFEST_SCHEMA = (
    "hu_m43_attempt13_distilled_training_manifest_v1"
)
ATTEMPT13_DEVELOPMENT_DECISION_SCHEMA = _DEVELOPMENT_DECISION_SCHEMA
ATTEMPT13_DEVELOPMENT_SELECTOR_RECEIPT_SCHEMA = _DEVELOPMENT_RECEIPT_SCHEMA
ATTEMPT13_DEVELOPMENT_PASS_FREEZE_SCHEMA = _DEVELOPMENT_GO_FREEZE_SCHEMA
ATTEMPT13_AUDIT50_DECISION_SCHEMA = _AUDIT50_DECISION_SCHEMA
ATTEMPT13_AUDIT50_SELECTOR_RECEIPT_SCHEMA = _AUDIT50_RECEIPT_SCHEMA
ATTEMPT13_PROFILES = M43_ATTEMPT13_PROFILES
ATTEMPT13_FOLDS = 5
ATTEMPT13_PROPOSAL_ROWS_MAX = ATTEMPT13_CANDIDATE_MAX + 1


@dataclass(frozen=True)
class Attempt13DistillationState:
    root_index: int
    root_profile: str
...
def read_attempt13_jsonl(path: str | Path) -> list[dict[str, Any]]:
    raw = Path(path).read_bytes()
    if not raw or not raw.endswith(b"\n"):
        raise ValueError("Attempt13 distillation JSONL must be non-empty and newline terminated")
    rows: list[dict[str, Any]] = []
    for line_number, line in enumerate(raw.splitlines(), start=1):
        try:
            value = json.loads(line)
        except json.JSONDecodeError as exc:
            raise ValueError(f"Attempt13 distillation JSONL line {line_number} is invalid") from exc
        if not isinstance(value, dict):
            raise ValueError("Attempt13 distillation JSONL rows must be mappings")
        rows.append(value)
    return rows


def prepare_attempt13_distillation_states(
    rows: Sequence[Mapping[str, Any]],
    *,
    ranker: FrozenAttempt13LambdaRanker,
    expected_roots: int | None = 200,
) -> tuple[Attempt13DistillationState, ...]:
    ...
    if expected_roots is not None and len(rows) != expected_roots:
        raise ValueError(f"Attempt13 distillation requires exactly {expected_roots} rows")
    by_root: dict[int, Mapping[str, Any]] = {}
    for raw in rows:
        root_index = _strict_int(raw.get("root_index"), "root_index")
        if root_index in by_root:
            raise ValueError("Attempt13 distillation rejects duplicate root indices")
        by_root[root_index] = raw
    if expected_roots is not None and set(by_root) != set(range(expected_roots)):
        raise ValueError("Attempt13 distillation root indices are not contiguous")
    ...
    for root_index in sorted(by_root):
        ...
        if row.get("schema") != ATTEMPT13_DEVELOPMENT_ROW_SCHEMA:
            raise ValueError("Attempt13 distillation requires canonical search rows")
        ...
        if (
            ...
            raise ValueError(
                "Attempt13 distillation accepts Development200 rows only"
            )
        profile = str(row.get("root_profile"))
        if profile not in ATTEMPT13_PROFILES:
            raise ValueError("Attempt13 distillation root profile is unknown")
        if expected_roots == 200 and profile != ATTEMPT13_PROFILES[root_index % 5]:
            raise ValueError("Attempt13 distillation profile schedule changed")
        ...
        if observation.to_dict() != observation_payload:
            raise ValueError("Attempt13 distillation observation is non-canonical")
        if observation.street != "T1" or observation.seat != "second":
            raise ValueError("Attempt13 distillation authorizes only T1-second")
        fingerprint = observation.fingerprint()
        if row.get("observation_fingerprint") not in (None, fingerprint):
            raise ValueError("Attempt13 distillation observation fingerprint changed")
        if fingerprint in seen_fingerprints:
            raise ValueError("Attempt13 distillation rejects duplicate information sets")
        ...
        if teacher.get("schema") != ATTEMPT13_TEACHER_SCHEMA:
            raise ValueError("Attempt13 distillation teacher schema changed")
        if teacher.get("policy_observation") != observation_payload:
            raise ValueError("Attempt13 distillation teacher observation changed")
        if teacher.get("baseline_action_key") != baseline_token:
            raise ValueError("Attempt13 distillation teacher baseline changed")
        if teacher.get("runtime_gate_allowed") is not False:
            raise ValueError("Attempt13 teacher data cannot be runtime authorization")
        _validate_attempt13_public_teacher(
            observation,
            baseline_action_key=baseline_token,
            teacher=teacher,
        )
        built = build_attempt13_distilled_features(
            sample,
        ...
        if teacher_candidates != built_candidates:
            raise ValueError("Attempt13 distillation all-legal candidate mapping changed")
        candidate_count = len(built.candidate_indices)
        if (
            teacher.get("candidate_nonbaseline_count") != candidate_count
            or not 0 <= candidate_count <= ATTEMPT13_CANDIDATE_MAX
            or len(set(teacher_candidates)) != candidate_count
            or baseline_token in teacher_candidates
        ):
            raise ValueError("Attempt13 distillation candidate count/uniqueness changed")
        ...
        if len(rerank_rows) != group_size:
            raise ValueError("Attempt13 distillation requires all legal candidates plus baseline R128 rows")
        ...
        if tuple(str(_mapping(item, "rerank action").get("action_key")) for item in rerank_rows) != expected_tokens:
            raise ValueError("Attempt13 distillation R128 action mapping changed")
        ...
        if type(fired) is not bool or type(fallback) is not bool:
            raise ValueError("Attempt13 distillation decision flags are invalid")
        if fired == fallback or (not fired and selected_token != baseline_token):
            raise ValueError("Attempt13 distillation baseline fallback changed")
        if selected_token not in expected_tokens:
            raise ValueError("Attempt13 distillation selected action is outside proposals")
        ...
        for index, raw_rerank in enumerate(rerank_rows):
            ...
            if raw_delta.shape != (128,) or not np.isfinite(raw_delta).all():
                raise ValueError("Attempt13 distillation R128 vector changed")
            ...
            for name, expected in summary.items():
                if not math.isclose(
                    ...
                    raise ValueError("Attempt13 distillation R128 summary changed")
        ...
        if not np.allclose(delta[-1], 0.0, rtol=0.0, atol=1.0e-12) or not np.allclose(
            ...
            raise ValueError("Attempt13 distillation explicit baseline is not zero")
        ...
        if fired:
            if evaluation.get("opened") is not True or len(evaluation_rows) != 2:
                raise ValueError("Attempt13 distillation fired row lacks E512")
            ...
            if selected_evaluation.get("action_key") != selected_token:
                raise ValueError("Attempt13 distillation E512 action mapping changed")
            ...
            raise ValueError("Attempt13 distillation nonfire opened E512")
...
def _validate_attempt13_public_teacher(
    ...
    *,
    ...
    result = validate_attempt13_teacher_output(
        observation,
    ...
    if (
        ...
        raise ValueError("Attempt13 public teacher validation result changed")


def assign_attempt13_identity_folds(
    states: Sequence[Attempt13DistillationState], *, folds: int = ATTEMPT13_FOLDS
) -> np.ndarray:
    if folds != ATTEMPT13_FOLDS or not states:
        raise ValueError("Attempt13 distillation requires exactly five folds")
    if len({state.observation_fingerprint for state in states}) != len(states):
        raise ValueError("Attempt13 distillation fold identities repeat")
    result = np.full(len(states), -1, dtype=np.int8)
    for profile in ATTEMPT13_PROFILES:
    ...
    if np.any(result < 0):
        raise ValueError("Attempt13 distillation fold assignment found unknown profile")
    return result


def fit_attempt13_distilled_model(
    states: Sequence[Attempt13DistillationState],
    *,
    candidate_ranker: FrozenAttempt13LambdaRanker,
    config: Mapping[str, Any],
    smoke: bool = False,
    model_id: str = "hu-m43-attempt13-distilled-development200-oof",
) -> tuple[HuM43Attempt13DistilledModel, dict[str, Any]]:
    """Fit five train-four/hold-one models and freeze OOF tail cushions."""

    _validate_config(config)
    if len(states) < ATTEMPT13_FOLDS:
        raise ValueError("Attempt13 distillation has too few states")
    folds = assign_attempt13_identity_folds(states)
    training = _mapping(config.get("training"), "training")
    seed = _strict_int(training.get("random_seed"), "random_seed")
    predictors: list[Attempt13DistilledFoldPredictor] = []
    for fold_index in range(ATTEMPT13_FOLDS):
    ...
    model = HuM43Attempt13DistilledModel(
        ...
        manifest={
            ...
            "identity_grouped_folds": ATTEMPT13_FOLDS,
    ...
    diagnostics = {
        ...
        "candidate_count_histogram": {
            ...
            for candidate_count in range(ATTEMPT13_CANDIDATE_MAX + 1)
        },
        "fold_counts": {
            str(fold): int(np.sum(folds == fold)) for fold in range(ATTEMPT13_FOLDS)
        },
        "profile_counts": {
...
def train_attempt13_distilled_artifact(
    *,
    input_path: str | Path,
    ...
    _validate_config(config)
    if _file_sha256(config_path) != ATTEMPT13_DISTILLATION_CONFIG_SHA256:
        raise ValueError("Attempt13 full distillation config SHA changed")
    ...
    if (
        ...
        raise ValueError("Attempt13 distillation requires development Go")
    ...
    if decision_source.get("input_jsonl_sha256") != input_sha:
        raise ValueError("Attempt13 development decision does not bind training JSONL")
    ...
    if (
        ...
        != ATTEMPT13_DEVELOPMENT_SELECTOR_RECEIPT_SCHEMA
        ...
        raise ValueError("Attempt13 development selector receipt is not fit authorization")
    if (
        pass_freeze.get("schema") != ATTEMPT13_DEVELOPMENT_PASS_FREEZE_SCHEMA
        or pass_freeze.get("status") != _DEVELOPMENT_GO_FREEZE_STATUS
        ...
        raise ValueError("Attempt13 immutable development-pass freeze is not bound")
    ...
    if (
        freeze_bindings["plan_sha256"] != M43_ATTEMPT13_PLAN_SHA256
        ...
        or decision_source.get("plan_sha256") != M43_ATTEMPT13_PLAN_SHA256
        ...
        raise ValueError("Attempt13 development source hash chain changed")
    ...
    if (
        ...
        or teacher_contract.get("plan_sha256") != M43_ATTEMPT13_PLAN_SHA256
        or teacher_contract.get("candidate_model_sha256")
        != ATTEMPT13_FROZEN_MODEL_SHA256
    ):
        raise ValueError("Attempt13 runtime source/teacher closure changed")
    ...
    ranker = FrozenAttempt13LambdaRanker.load(
        candidate_model_path, expected_sha256=ATTEMPT13_FROZEN_MODEL_SHA256
    )
    rows = read_attempt13_jsonl(input_path)
    states = prepare_attempt13_distillation_states(rows, ranker=ranker, expected_roots=200)
    model, diagnostics = fit_attempt13_distilled_model(
        states, candidate_ranker=ranker, config=config, smoke=smoke
    )
    model_sha = model.save(output_model_path)
    manifest = {
        "schema": ATTEMPT13_DISTILLED_TRAINING_MANIFEST_SCHEMA,
        ...
        "source": {
            ...
            "candidate_model_sha256": ATTEMPT13_FROZEN_MODEL_SHA256,
...
def _fit_fold(
    ...
    *,
    ...
    if not states:
        raise ValueError("Attempt13 distilled fold has no training states")
    ...
    if any(not 1 <= group <= ATTEMPT13_PROPOSAL_ROWS_MAX for group in groups):
        raise ValueError("Attempt13 distilled group size is outside 1..27")
    if sum(groups) != x.shape[0]:
        raise ValueError("Attempt13 distilled LightGBM groups do not match row total")
    ...
    return Attempt13DistilledFoldPredictor(
        ranker=ranker,
...
def _profile_state_weights(states: Sequence[Attempt13DistillationState]) -> np.ndarray:
    counts = {profile: sum(state.root_profile == profile for state in states) for profile in ATTEMPT13_PROFILES}
...
def _validate_config(config: Mapping[str, Any]) -> None:
    if config.get("schema") != ATTEMPT13_DISTILLATION_CONFIG_SCHEMA:
        raise ValueError("Attempt13 distillation config schema changed")
    schemas = _mapping(config.get("schemas"), "schemas")
    if (
        schemas.get("model") != HU_M43_ATTEMPT13_DISTILLED_MODEL_SCHEMA
        or schemas.get("feature") != HU_M43_ATTEMPT13_DISTILLED_FEATURE_SCHEMA
        or schemas.get("heads") != HU_M43_ATTEMPT13_DISTILLED_HEAD_SCHEMA
        or schemas.get("training_manifest")
        != ATTEMPT13_DISTILLED_TRAINING_MANIFEST_SCHEMA
    ):
        raise ValueError("Attempt13 distillation model schema changed")
    data = _mapping(config.get("data"), "data")
    if (
        data.get("source_search_plan_sha256") != M43_ATTEMPT13_PLAN_SHA256
        or data.get("development_roots") != ATTEMPT13_DISTILLATION_FULL_STATES
        or data.get("audit50_may_fit_or_calibrate") is not False
        or data.get("attempt12_rows_may_fit_or_calibrate") is not False
    ):
        raise ValueError("Attempt13 distillation data boundary changed")
    candidate = _mapping(config.get("candidate_generation"), "candidate_generation")
    if (
        candidate.get("source_model_sha256") != ATTEMPT13_FROZEN_MODEL_SHA256
        or candidate.get("nonbaseline_max") != ATTEMPT13_CANDIDATE_MAX
        or candidate.get("actual_count_range") != [0, ATTEMPT13_CANDIDATE_MAX]
        ...
        or candidate.get("lightgbm_group_size_range") != [1, ATTEMPT13_PROPOSAL_ROWS_MAX]
        or candidate.get("feature_dimension") != HU_M43_ATTEMPT13_DISTILLED_FEATURE_DIM
    ):
        raise ValueError("Attempt13 distillation candidate contract changed")
    ...
    if (
        ...
        or gate.get("action_score_mode") != HU_M43_ATTEMPT13_DISTILLED_ACTION_SCORE_MODE
    ):
        raise ValueError("Attempt13 distillation runtime gate changed")
    training = _mapping(config.get("training"), "training")
    if training.get("threshold_sweep_allowed") is not False:
        raise ValueError("Attempt13 distillation threshold sweep is forbidden")
    ...
    required_authorization = {
        ...
        "attempt12_rows_used_for_fit": False,
        "development_decision_schema": ATTEMPT13_DEVELOPMENT_DECISION_SCHEMA,
        "development_receipt_schema": ATTEMPT13_DEVELOPMENT_SELECTOR_RECEIPT_SCHEMA,
        "development_freeze_schema": ATTEMPT13_DEVELOPMENT_PASS_FREEZE_SCHEMA,
        "audit50_decision_schema": ATTEMPT13_AUDIT50_DECISION_SCHEMA,
        "audit50_receipt_schema": ATTEMPT13_AUDIT50_SELECTOR_RECEIPT_SCHEMA,
    }
    if authorization != required_authorization:
        raise ValueError("Attempt13 distillation fit authorization changed")
    ...
    if isinstance(value, Mapping):
        for key, child in value.items():
            ...
            if lowered in {"opponent_private_discards", "opponent_discards"}:
                raise ValueError(f"Attempt13 distillation exposes hidden information at {path}")
...
def _mapping(value: Any, label: str) -> dict[str, Any]:
    if not isinstance(value, Mapping):
        raise ValueError(f"Attempt13 distillation {label} must be a mapping")
    return dict(value)


def _sequence(value: Any, label: str) -> list[Any]:
    if isinstance(value, (str, bytes, bytearray)) or not isinstance(value, Sequence):
        raise ValueError(f"Attempt13 distillation {label} must be a sequence")
    return list(value)


def _strict_int(value: Any, label: str) -> int:
    if isinstance(value, bool) or not isinstance(value, (int, np.integer)):
        raise ValueError(f"Attempt13 distillation {label} must be an integer")
    return int(value)
...
def _finite(value: Any, label: str) -> float:
    try:
        ...
        raise ValueError(f"Attempt13 distillation {label} must be numeric") from exc
    if not math.isfinite(result):
        raise ValueError(f"Attempt13 distillation {label} must be finite")
    return result
...
def _require_sha256(value: Any, label: str) -> str:
    if not isinstance(value, str):
        raise ValueError(f"Attempt13 distillation {label} must be a SHA-256 string")
    normalized = value.lower()
    if len(normalized) != 64 or any(
        character not in "0123456789abcdef" for character in normalized
    ):
        raise ValueError(f"Attempt13 distillation {label} is invalid")
    return normalized
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Fit frozen Attempt13 distillation heads")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    manifest = train_attempt13_distilled_artifact(
...
__all__ = [
    ...
    "read_attempt13_jsonl",
    "train_attempt13_distilled_artifact",
]
