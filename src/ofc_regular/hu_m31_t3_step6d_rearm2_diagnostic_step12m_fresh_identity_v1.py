...
Step12b through Step12l consumed cloud identities and audit roots are all
...
SCHEMA = "hu_m31_t3_step6d_step12m_fresh_identity_v1"
STATUS = (
    "fresh_step12m_identity_disjoint_from_terminal_step12b_through_step12l"
)
TERMINAL_STEP12L_CLOSEOUT_RECEIPT_SHA256 = (
    "cd18947f3bdc2a63ffdcc28e8707aed11961ecfab6b498c20837201ddb3131b3"
)
TERMINAL_STEP12L_DEPLOYMENT_CONTRACT_SHA256 = (
    "60d204cfd642fa91f22a52eed8874fe92d19c907851a9906b87719a8b4e3e742"
)
...
TERMINAL_STEP12K_OUTPUT_ROOT = step12l_identity.TERMINAL_STEP12K_OUTPUT_ROOT
TERMINAL_STEP12L_OUTPUT_ROOT = ARTIFACT_PARENT / "step12l_pair_v1_actual"
EXPECTED_OUTPUT_ROOT = ARTIFACT_PARENT / "step12m_pair_v1_actual"
...
_TERMINAL_ROOTS: tuple[tuple[str, Path], ...] = (
    ...
    ("step12l", TERMINAL_STEP12L_OUTPUT_ROOT),
)
    ...
    for _, terminal_root in _TERMINAL_ROOTS:
        ...
        if root == terminal or terminal in root.parents:
            raise PermissionError(
                "only the exact fresh Step12m output root is allowed"
            )
    if root != expected:
        raise PermissionError(
            "only the exact fresh Step12m output root is allowed"
...
def exact_output_root(path: str | Path) -> Path:
    ...
    if root.exists() or root.is_symlink():
        raise FileExistsError(f"Step12m output root is not fresh: {root}")
    if not root.parent.is_dir() or root.parent.is_symlink():
        raise FileNotFoundError("Step12m output parent must be a real directory")
    return root
...
def _validate_terminal_closeouts() -> None:
    # Step12b through Step12k bindings are revalidated through the Step12j
    # boundary module so all eleven stay pinned by one call.
    step12l_identity._validate_terminal_closeouts()
    step12l_closeout = _validated_sealed_receipt(
        _read_json(
            TERMINAL_STEP12L_OUTPUT_ROOT
            / "external_preflight_token_field_bug_closeout_receipt.json"
        ),
        expected_sha256=TERMINAL_STEP12L_CLOSEOUT_RECEIPT_SHA256,
        label="terminal Step12l closeout",
    )
    if step12l_closeout.get("step12l_identity_terminal") is not True:
        raise ValueError("terminal Step12l closeout binding changed")
    step12l_deployment = _read_json(
        TERMINAL_STEP12L_OUTPUT_ROOT / "deployment_contract.json"
    )
    if (
        step12l_deployment.get("deployment_contract_sha256")
        != TERMINAL_STEP12L_DEPLOYMENT_CONTRACT_SHA256
    ):
        raise ValueError("terminal Step12l deployment binding changed")
...
def validate_fresh_identity(
    *,
    ...
    if (
        ...
        raise ValueError("Step12m exact-pair topology changed")
    ...
    for label, fresh in fresh_values.items():
        if not isinstance(fresh, str) or not fresh:
            raise ValueError(f"Step12m {label} is missing")
        for terminal in terminals:
            if fresh == terminal[label]:
                raise ValueError(
                    f"Step12m reused terminal {terminal['label']} {label}"
                )
    ...
    if len(set(new_instance_names)) != 2 or any(
        ...
        raise ValueError("Step12m reused a terminal VM/disk name")
    ...
    return seal(
        {
            ...
            "terminal_step12k_closeout_receipt_sha256": (
                step12l_identity.TERMINAL_STEP12K_CLOSEOUT_RECEIPT_SHA256
            ),
            "terminal_step12l_closeout_receipt_sha256": (
                TERMINAL_STEP12L_CLOSEOUT_RECEIPT_SHA256
            ),
