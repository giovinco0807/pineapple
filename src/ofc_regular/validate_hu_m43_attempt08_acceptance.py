"""Freeze, preflight, and accept the Attempt08 distilled population policy.
...
from .hu_m43_attempt08_distilled_model import (
    BoundHuM43Attempt08DistilledModel,
...
from .hu_m43_attempt08_distilled_runtime import (
    ATTEMPT08_BOUND_EXECUTION_MODULES,
    ATTEMPT08_DISTILLED_RUNTIME_REGISTRY_PATHS,
...
from .train_hu_m43_attempt08_distilled import (
...
ATTEMPT08_POPULATION_PLAN_SCHEMA = (
    "hu_m43_population_acceptance_plan_v1"
)
ATTEMPT08_POPULATION_PREFLIGHT_SCHEMA = (
    "hu_m43_attempt08_population_launch_preflight_v1"
)
ATTEMPT08_RUNTIME_FREEZE_SCHEMA = "hu_m43_attempt08_distilled_runtime_freeze_v1"
ATTEMPT08_POPULATION_ACCEPTANCE_STATUS_SCHEMA = (
    "hu_m43_attempt08_population_acceptance_status_v1"
)
ATTEMPT08_AUDIT50_DECISION_SCHEMA = "hu_m43_attempt08_audit50_go_no_go_v1"
ATTEMPT08_AUDIT50_SELECTOR_RECEIPT_SCHEMA = (
    "hu_m43_attempt08_audit50_selector_receipt_v1"
)
ATTEMPT08_OPPONENTS = (
    "stage19_p0",
    "stage9f_p2",
...
ATTEMPT08_POPULATION_SEED = 160_108_071_901
ATTEMPT08_POPULATION_SEED_STRIDE = 1_000_003
ATTEMPT08_POPULATION_SEEDS = 1000
ATTEMPT08_POPULATION_SHARDS = 20
ATTEMPT08_POPULATION_SEEDS_PER_SHARD = 50
ATTEMPT08_BASELINE_PROFILE = "stage19_p0"
ATTEMPT08_FIXED_GATES = {
...
ATTEMPT08_SEED_REGISTRY_SOURCES = ATTEMPT08_DISTILLED_RUNTIME_REGISTRY_PATHS


def load_and_validate_attempt08_population_plan(
    source: str | Path | Mapping[str, Any]
) -> dict[str, Any]:
    plan = dict(source) if isinstance(source, Mapping) else _load_mapping(source, "plan")
    if (
        plan.get("schema") != ATTEMPT08_POPULATION_PLAN_SCHEMA
        or plan.get("status") != "frozen_before_population_evaluation"
        or plan.get("policy_attempt") != "attempt08_distilled_v1"
        or plan.get("fixed_baseline_profile") != ATTEMPT08_BASELINE_PROFILE
        or plan.get("opponents") != list(ATTEMPT08_OPPONENTS)
        or plan.get("paired_seat_swap") is not True
    ):
        raise ValueError("Attempt08 population plan identity changed")
    if plan.get("baseline_chain") != [
        ...
        raise ValueError("Attempt08 population baseline chain changed")
    runtime = _mapping(plan.get("runtime_contract"), "runtime_contract")
    expected_runtime = {
        "model_schema": HU_M43_ATTEMPT08_DISTILLED_MODEL_SCHEMA,
        "artifact_schema": HU_M43_ATTEMPT08_DISTILLED_ARTIFACT_SCHEMA,
    ...
    if runtime != expected_runtime:
        raise ValueError("Attempt08 population runtime contract changed")
    schedule = (
    ...
    if schedule != (
        ATTEMPT08_POPULATION_SEEDS,
        ATTEMPT08_POPULATION_SEED,
        ATTEMPT08_POPULATION_SEED_STRIDE,
        ATTEMPT08_POPULATION_SHARDS,
        ATTEMPT08_POPULATION_SEEDS_PER_SHARD,
    ):
        raise ValueError("Attempt08 population schedule changed")
    if (
        ...
        raise ValueError("Attempt08 population power contract changed")
    ...
    if sizing != {
        ...
        raise ValueError("Attempt08 population optional-stopping contract changed")
    ...
    if evaluation_contract != {
        ...
        raise ValueError("Attempt08 population counterfactual contract changed")
    ...
    if spot != {
        ...
        raise ValueError("Attempt08 population Spot execution contract changed")
    if _mapping(plan.get("fixed_acceptance_gates"), "gates") != ATTEMPT08_FIXED_GATES:
        raise ValueError("Attempt08 fixed acceptance gates changed")
    ...
    if set(guards) != expected_guard_names or any(
        ...
        raise ValueError("Attempt08 population activation guard changed")
    planned = {
        ATTEMPT08_POPULATION_SEED + index * ATTEMPT08_POPULATION_SEED_STRIDE
        for index in range(ATTEMPT08_POPULATION_SEEDS)
    }
    freshness = _mapping(plan.get("freshness"), "freshness")
    for name in (
        "exclude_all_m4_m41_m42_m43_teacher_hand_seeds",
        "exclude_attempt08_preflight_development_and_audit_namespaces",
        ...
        if freshness.get(name) is not True:
            raise ValueError("Attempt08 population freshness contract changed")
    if (
        freshness.get("planned_overlap_count_at_freeze") != 0
        or freshness.get("alternate_seed_after_result_allowed") is not False
    ):
        raise ValueError("Attempt08 population seed-selection contract changed")
    ...
    if registry_sources != ATTEMPT08_SEED_REGISTRY_SOURCES:
        raise ValueError("Attempt08 seed registry source list changed")
    registry_seeds, registry_rows = _load_seed_registry(registry_sources)
    registry_overlap = planned & registry_seeds
    if registry_overlap:
        raise ValueError("Attempt08 population seed grid overlaps hashed registry")
    registry_digest = hashlib.sha256(
        _canonical(registry_rows).encode("utf-8")
    ).hexdigest()
    if freshness.get("excluded_schedule_registry_sha256") != registry_digest:
        raise ValueError("Attempt08 hashed seed registry snapshot changed")
    plan["_freshness_registry"] = {
        "schema": "hu_m43_attempt08_seed_registry_snapshot_v1",
    ...
    for raw_base in _sequence(
        freshness.get("excluded_attempt08_namespace_bases"), "namespace bases"
    ):
        base = _integer(raw_base, "namespace base")
        teacher.update(
            base + index * ATTEMPT08_POPULATION_SEED_STRIDE for index in range(250)
        )
    ...
    if any(plan["_freshness_counts"].values()):
        raise ValueError("Attempt08 population seed grid overlaps prior evidence")
    return plan
...
def _load_seed_registry(
    ...
    for relative in relative_sources:
        ...
        if candidate.is_absolute() or ".." in candidate.parts:
            raise ValueError("Attempt08 seed registry path escapes repository")
        source = (root / candidate).resolve()
        try:
            source.relative_to(root)
        except ValueError as exc:
            raise ValueError("Attempt08 seed registry path escapes repository") from exc
        if not source.is_file():
            raise ValueError(f"Attempt08 seed registry source missing: {relative}")
...
def _extract_declared_seed_schedules(payload: Mapping[str, Any]) -> set[int]:
    ...
    def visit(value: Any, inherited_stride: int = ATTEMPT08_POPULATION_SEED_STRIDE) -> None:
...
def freeze_attempt08_distilled_runtime(
    *,
    ...
    if any(
        ...
        raise ValueError("Attempt08 training/runtime source freeze mismatch")
    if (
        ...
        raise ValueError("Attempt08 external runtime identity changed")
    if (
        audit.get("schema") != ATTEMPT08_AUDIT50_DECISION_SCHEMA
        or audit.get("status") != "go_write_separate_distillation_freeze_only"
        or audit.get("decision") != "go"
        or audit.get("fit_performed") is not False
        or audit.get("threshold_selected") is not False
        or audit.get("runtime_policy_activated") is not False
        or audit.get("current_profile_mutated") is not False
    ):
        raise ValueError("Attempt08 runtime freeze requires the one-shot audit50 Go")
    ...
    if (
        ...
        or audit_contract.get("threshold_search_on_audit_performed") is not False
        or audit_contract.get("fit_on_audit_performed") is not False
        or audit_contract.get("audit_rows_used_for_fit") is not False
        or audit_contract.get("realized_population_acceptance_still_required")
        is not True
    ):
        raise ValueError("Attempt08 audit50 Go contract changed")
    if (
        audit_receipt.get("schema") != ATTEMPT08_AUDIT50_SELECTOR_RECEIPT_SCHEMA
        or audit_receipt.get("status")
        != "single_frozen_audit_gate_evaluation_complete"
        or audit_receipt.get("decision_sha256") != audit_sha
        or audit_receipt.get("decision") != "go"
        or audit_receipt.get("go_action")
        != "authorize_distillation_from_development200_only"
        or audit_receipt.get("gate_evaluation_count") != 1
        or audit_receipt.get("selector_executed") is not True
        or audit_receipt.get("audit_rows_used_for_fit") is not False
        or audit_receipt.get("fit_performed") is not False
        or audit_receipt.get("threshold_selected") is not False
        or audit_receipt.get("current_profile_mutated") is not False
        or audit_receipt.get("runtime_policy_activated") is not False
    ):
        raise ValueError("Attempt08 audit50 selector receipt is not freeze authorization")
    source_model = HuM43Attempt08DistilledModel.load(
        source_model_path, expected_sha256=source_sha
    )
    if source_model.safety_enabled or source_model.winner_frozen:
        raise ValueError("Attempt08 source model is already runtime frozen")
    ...
    if any(
        ...
        for name, value in {
            ...
            "fit_mode": "full",
            "effective_iterations": ATTEMPT08_DISTILLATION_FULL_ITERATIONS,
            "training_states": ATTEMPT08_DISTILLATION_FULL_STATES,
            "distillation_config_sha256": ATTEMPT08_DISTILLATION_CONFIG_SHA256,
        }.items()
    ):
        raise ValueError("Attempt08 source model is not the frozen full-fit artifact")
    ...
    freeze = {
        "schema": ATTEMPT08_RUNTIME_FREEZE_SCHEMA,
...
def load_bound_attempt08_distilled_model(
    path: str | Path,
    *,
    expected_sha256: str,
    ...
    execution_attestation = validate_frozen_execution_modules(
        ...
        manifest=runtime_source_manifest_path,
        module_names=ATTEMPT08_BOUND_EXECUTION_MODULES,
    )
    actual_sha = _file_sha256(path)
    if actual_sha != _sha256(expected_sha256, "model SHA"):
        raise ValueError("Attempt08 bound model SHA mismatch")
    freeze = (
        dict(runtime_freeze)
    ...
    _validate_runtime_dependency_semantics(runtime_semantic, runtime_dependencies)
    if (
        freeze.get("schema") != ATTEMPT08_RUNTIME_FREEZE_SCHEMA
        ...
        raise ValueError("Attempt08 runtime freeze contract changed")
    _sha256(
    ...
    if training.get("model_id") != freeze.get("model_id"):
        raise ValueError("Attempt08 training manifest/runtime freeze mismatch")
    training_source = _mapping(training.get("source"), "training source")
    for name in (
        ...
        if training_source.get(name) != freeze.get(name):
            raise ValueError("Attempt08 runtime source hash chain changed")
    model = HuM43Attempt08DistilledModel.load(path, expected_sha256=actual_sha)
    ...
    if any(
        ...
        raise ValueError("Attempt08 frozen model full-fit contract changed")
    for name in (
        ...
        if embedded_manifest.get(name) != freeze.get(name):
            raise ValueError("Attempt08 frozen model semantic binding changed")
    if (
        ...
        raise ValueError("Attempt08 frozen model payload disagrees with freeze")
    return BoundHuM43Attempt08DistilledModel(model, execution_attestation)


def build_attempt08_population_preflight(
    *,
    model_path: str | Path,
    training_manifest_path: str | Path,
    ...
    plan = load_and_validate_attempt08_population_plan(population_plan_path)
    ...
    model = load_bound_attempt08_distilled_model(
        model_path,
    ...
    return {
        "schema": ATTEMPT08_POPULATION_PREFLIGHT_SCHEMA,
        "status": "pass",
...
def validate_attempt08_population_acceptance(
    *,
    ...
    recomputed = summarize_hu_m4_population_records(
        records,
        opponents=ATTEMPT08_OPPONENTS,
        paired_seeds=ATTEMPT08_POPULATION_SEEDS,
        seed=ATTEMPT08_POPULATION_SEED,
        seed_stride=ATTEMPT08_POPULATION_SEED_STRIDE,
        baseline_profile=ATTEMPT08_BASELINE_PROFILE,
    )
    ...
    runtime_match = (
        ...
        and runtime.get("model_schema") == HU_M43_ATTEMPT08_DISTILLED_MODEL_SCHEMA
        and runtime.get("artifact_schema") == HU_M43_ATTEMPT08_DISTILLED_ARTIFACT_SCHEMA
        and runtime.get("feature_schema") == HU_M43_ATTEMPT08_DISTILLED_FEATURE_SCHEMA
        and runtime.get("head_schema") == HU_M43_ATTEMPT08_DISTILLED_HEAD_SCHEMA
        and runtime.get("model_id") == preflight["model_id"]
        and runtime.get("action_score_mode")
        == HU_M43_ATTEMPT08_DISTILLED_ACTION_SCORE_MODE
        and runtime.get("baseline_profile") == ATTEMPT08_BASELINE_PROFILE
        ...
        and runtime.get("shard_count") == ATTEMPT08_POPULATION_SHARDS
    ...
    merge_match = (
        ...
        and merge_manifest.get("seed") == ATTEMPT08_POPULATION_SEED
        and merge_manifest.get("seed_stride") == ATTEMPT08_POPULATION_SEED_STRIDE
        and merge_manifest.get("paired_seeds_per_opponent")
        == ATTEMPT08_POPULATION_SEEDS
        and merge_manifest.get("opponents") == list(ATTEMPT08_OPPONENTS)
        ...
        and len(merge_manifest.get("shards")) == ATTEMPT08_POPULATION_SHARDS
    )
    gates = [
        _gate(
    ...
    return {
        "schema": ATTEMPT08_POPULATION_ACCEPTANCE_STATUS_SCHEMA,
        "status": decision,
        "passed_gates": passed,
...
def _validate_training_manifest(
    ...
    for name in (
        ...
        "development_runtime_source_closure_sha256",
        "development_runtime_semantic_anchor_sha256",
        "runtime_source_archive_sha256",
        "runtime_source_manifest_sha256",
        "runtime_source_closure_sha256",
        "runtime_semantic_closure_sha256",
    ...
    if (
        source.get("development_runtime_source_closure_sha256")
        != ATTEMPT08_RUNTIME_SOURCE_CLOSURE_SHA256
        or source.get("development_runtime_semantic_anchor_sha256")
        != ATTEMPT08_RUNTIME_SEMANTIC_ANCHOR_SHA256
        ...
        != {
            "fit_mode": "full",
            "effective_iterations": ATTEMPT08_DISTILLATION_FULL_ITERATIONS,
            "states": ATTEMPT08_DISTILLATION_FULL_STATES,
            "folds": 5,
        }
        or diagnostics.get("states") != ATTEMPT08_DISTILLATION_FULL_STATES
        or diagnostics.get("rows") != ATTEMPT08_DISTILLATION_FULL_STATES * 9
        or diagnostics.get("fold_counts")
        != {str(index): 40 for index in range(5)}
    ):
        raise ValueError("Attempt08 training runtime semantic binding changed")
    if (
        manifest.get("schema") != ATTEMPT08_DISTILLED_TRAINING_MANIFEST_SCHEMA
        or manifest.get("status") != "fit_complete_runtime_disabled"
        or manifest.get("model_schema") != HU_M43_ATTEMPT08_DISTILLED_MODEL_SCHEMA
        or manifest.get("feature_schema") != HU_M43_ATTEMPT08_DISTILLED_FEATURE_SCHEMA
        or manifest.get("head_schema") != HU_M43_ATTEMPT08_DISTILLED_HEAD_SCHEMA
        ...
        raise ValueError("Attempt08 source training manifest contract changed")
...
def _integer(value: Any, label: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int):
        raise ValueError(f"Attempt08 {label} must be an integer")
    return value


def _sha256(value: Any, label: str) -> str:
    if not isinstance(value, str):
        raise ValueError(f"Attempt08 {label} must be a SHA-256 string")
    normalized = value.lower()
    if len(normalized) != 64 or any(c not in "0123456789abcdef" for c in normalized):
        raise ValueError(f"Attempt08 {label} is invalid")
    return normalized
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if runtime_source_root is not None:
        validate_frozen_execution_modules(
            ...
            module_names=(
                ...
                "ofc_regular.train_hu_m43_attempt08_distilled",
                "ofc_regular.evaluate_hu_m4_population",
                "ofc_regular.merge_hu_m4_population_shards",
                "ofc_regular.validate_hu_m43_attempt08_acceptance",
            ),
        )
    if args.command == "freeze":
        result = freeze_attempt08_distilled_runtime(
    ...
    if args.command == "preflight":
        result = build_attempt08_population_preflight(
    ...
    status = validate_attempt08_population_acceptance(
...
__all__ = [
    "ATTEMPT08_POPULATION_ACCEPTANCE_STATUS_SCHEMA",
    "ATTEMPT08_POPULATION_PREFLIGHT_SCHEMA",
    "ATTEMPT08_RUNTIME_FREEZE_SCHEMA",
    "build_attempt08_population_preflight",
    "freeze_attempt08_distilled_runtime",
    "load_and_validate_attempt08_population_plan",
    "load_bound_attempt08_distilled_model",
    "validate_attempt08_population_acceptance",
]
