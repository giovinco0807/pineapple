"""Fresh-identity boundary for the post-Step12d exact-pair canary.

Step12b, Step12c, and Step12d consumed cloud identities and audit roots
are all terminal.  This module proves that a newly prepared direct-v2
contract is disjoint from all three terminal executions before a live
...
SCHEMA = "hu_m31_t3_step6d_step12e_fresh_identity_v1"
STATUS = (
    "fresh_step12e_identity_disjoint_from_terminal_step12b_step12c_step12d"
)
TERMINAL_STEP12D_CLOSEOUT_RECEIPT_SHA256 = (
    "dd33b2117b5bf92f5f75723f26951b665625a5677eb52e782e58b43eaf0e2a0e"
)
TERMINAL_STEP12D_DEPLOYMENT_CONTRACT_SHA256 = (
    "cb94bb46e98918e593f4348440aff264db0ad44d47f0047d2080b1102b1b4f83"
)
...
TERMINAL_STEP12D_OUTPUT_ROOT = ARTIFACT_PARENT / "step12d_pair_v1_actual"
EXPECTED_OUTPUT_ROOT = ARTIFACT_PARENT / "step12e_pair_v1_actual"
    ...
    for _, terminal_root in _TERMINAL_ROOTS:
        ...
        if root == terminal or terminal in root.parents:
            raise PermissionError(
                "only the exact fresh Step12e output root is allowed"
            )
    if root != expected:
        raise PermissionError(
            "only the exact fresh Step12e output root is allowed"
...
def exact_output_root(path: str | Path) -> Path:
    ...
    if root.exists() or root.is_symlink():
        raise FileExistsError(f"Step12e output root is not fresh: {root}")
    if not root.parent.is_dir() or root.parent.is_symlink():
        raise FileNotFoundError("Step12e output parent must be a real directory")
    return root
...
def _validate_terminal_closeouts() -> None:
    # Step12b and Step12c bindings are revalidated through the Step12d
    # boundary module so all three stay pinned by one call.
    step12d_identity._validate_terminal_closeouts()
    step12d_closeout = _validated_sealed_receipt(
        _read_json(
            TERMINAL_STEP12D_OUTPUT_ROOT
            / "transport_failure_closeout_receipt.json"
        ),
        expected_sha256=TERMINAL_STEP12D_CLOSEOUT_RECEIPT_SHA256,
        label="terminal Step12d closeout",
    )
    if step12d_closeout.get("step12d_identity_terminal") is not True:
        raise ValueError("terminal Step12d closeout binding changed")
    step12d_deployment = _read_json(
        TERMINAL_STEP12D_OUTPUT_ROOT / "deployment_contract.json"
    )
    if (
        step12d_deployment.get("deployment_contract_sha256")
        != TERMINAL_STEP12D_DEPLOYMENT_CONTRACT_SHA256
    ):
        raise ValueError("terminal Step12d deployment binding changed")
...
def validate_fresh_identity(
    *,
    ...
    if (
        ...
        raise ValueError("Step12e exact-pair topology changed")
    ...
    for label, fresh in fresh_values.items():
        if not isinstance(fresh, str) or not fresh:
            raise ValueError(f"Step12e {label} is missing")
        for terminal in terminals:
            if fresh == terminal[label]:
                raise ValueError(
                    f"Step12e reused terminal {terminal['label']} {label}"
                )
    ...
    if len(set(new_instance_names)) != 2 or any(
        ...
        raise ValueError("Step12e reused a terminal VM/disk name")
