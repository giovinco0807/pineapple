"""Freeze, preflight, and accept the Attempt10 distilled population policy.
...
from .hu_m43_attempt10_distilled_runtime import (
    ...
    ATTEMPT10_DISTILLED_RUNTIME_REGISTRY_PATHS,
    ATTEMPT10_EXPECTED_RUNTIME_FINGERPRINT_SHA256,
    ATTEMPT10_GCP_IMAGE_ID,
    ATTEMPT10_GCP_IMAGE_NAME,
    ATTEMPT10_GCP_IMAGE_SELF_LINK,
...
from .select_hu_m43_attempt10_audit50 import (
    ATTEMPT10_AUDIT50_DECISION_SCHEMA,
...
from .train_hu_m43_attempt10_distilled import (
...
ATTEMPT10_POPULATION_PLAN_SCHEMA = (
    "hu_m43_population_acceptance_plan_v1"
)
ATTEMPT10_POPULATION_PREFLIGHT_SCHEMA = (
    "hu_m43_attempt10_population_launch_preflight_v1"
)
ATTEMPT10_RUNTIME_FREEZE_SCHEMA = "hu_m43_attempt10_distilled_runtime_freeze_v1"
ATTEMPT10_POPULATION_ACCEPTANCE_STATUS_SCHEMA = (
    "hu_m43_attempt10_population_acceptance_status_v1"
)
ATTEMPT10_AUDIT50_SELECTOR_RECEIPT_SCHEMA = ATTEMPT10_AUDIT50_RECEIPT_SCHEMA
ATTEMPT10_OPPONENTS = (
    "stage19_p0",
    "stage9f_p2",
    "stage7_m5_r10",
...
ATTEMPT10_POPULATION_SEED = 180_108_071_901
ATTEMPT10_POPULATION_SEED_STRIDE = 1_000_003
ATTEMPT10_POPULATION_SEEDS = 1000
ATTEMPT10_POPULATION_SHARDS = 20
ATTEMPT10_POPULATION_SEEDS_PER_SHARD = 50
ATTEMPT10_BASELINE_PROFILE = "stage19_p0"
ATTEMPT10_FIXED_GATES = {
...
ATTEMPT10_SEED_REGISTRY_SOURCES = ATTEMPT10_DISTILLED_RUNTIME_REGISTRY_PATHS


def load_and_validate_attempt10_population_plan(
    source: str | Path | Mapping[str, Any]
) -> dict[str, Any]:
    plan = dict(source) if isinstance(source, Mapping) else _load_mapping(source, "plan")
    if (
        plan.get("schema") != ATTEMPT10_POPULATION_PLAN_SCHEMA
        or plan.get("status") != "frozen_before_population_evaluation"
        or plan.get("policy_attempt") != "attempt10_distilled_v1"
        or plan.get("fixed_baseline_profile") != ATTEMPT10_BASELINE_PROFILE
        or plan.get("opponents") != list(ATTEMPT10_OPPONENTS)
        or plan.get("paired_seat_swap") is not True
    ):
        raise ValueError("Attempt10 population plan identity changed")
    if plan.get("baseline_chain") != [
        ...
        raise ValueError("Attempt10 population baseline chain changed")
    ...
    if runtime != expected_runtime:
        raise ValueError("Attempt10 population runtime contract changed")
    schedule = (
    ...
    if schedule != (
        ATTEMPT10_POPULATION_SEEDS,
        ATTEMPT10_POPULATION_SEED,
        ATTEMPT10_POPULATION_SEED_STRIDE,
        ATTEMPT10_POPULATION_SHARDS,
        ATTEMPT10_POPULATION_SEEDS_PER_SHARD,
    ):
        raise ValueError("Attempt10 population schedule changed")
    if (
        ...
        raise ValueError("Attempt10 population power contract changed")
    ...
    if sizing != {
        ...
        raise ValueError("Attempt10 population optional-stopping contract changed")
    ...
    if evaluation_contract != {
        ...
        raise ValueError("Attempt10 population counterfactual contract changed")
    ...
    if spot != {
        ...
        raise ValueError("Attempt10 population Spot execution contract changed")
    if _mapping(plan.get("fixed_acceptance_gates"), "gates") != ATTEMPT10_FIXED_GATES:
        raise ValueError("Attempt10 fixed acceptance gates changed")
    ...
    if set(guards) != expected_guard_names or any(
        ...
        raise ValueError("Attempt10 population activation guard changed")
    if _mapping(
        ...
        raise ValueError("Attempt10 post-acceptance activation contract changed")
    planned = {
        ATTEMPT10_POPULATION_SEED + index * ATTEMPT10_POPULATION_SEED_STRIDE
        for index in range(ATTEMPT10_POPULATION_SEEDS)
    }
    freshness = _mapping(plan.get("freshness"), "freshness")
    for name in (
        "exclude_all_m4_m41_m42_m43_teacher_hand_seeds",
        "exclude_attempt10_preflight_development_and_audit_namespaces",
        ...
        if freshness.get(name) is not True:
            raise ValueError("Attempt10 population freshness contract changed")
    if (
        freshness.get("planned_overlap_count_at_freeze") != 0
        or freshness.get("alternate_seed_after_result_allowed") is not False
    ):
        raise ValueError("Attempt10 population seed-selection contract changed")
    ...
    if registry_sources != ATTEMPT10_SEED_REGISTRY_SOURCES:
        raise ValueError("Attempt10 seed registry source list changed")
    registry_seeds, registry_rows = _load_seed_registry(registry_sources)
    registry_overlap = planned & registry_seeds
    if registry_overlap:
        raise ValueError("Attempt10 population seed grid overlaps hashed registry")
    registry_digest = hashlib.sha256(
        _canonical(registry_rows).encode("utf-8")
    ).hexdigest()
    if freshness.get("excluded_schedule_registry_sha256") != registry_digest:
        raise ValueError("Attempt10 hashed seed registry snapshot changed")
    plan["_freshness_registry"] = {
        "schema": "hu_m43_attempt10_seed_registry_snapshot_v1",
    ...
    for raw_base in _sequence(
        freshness.get("excluded_attempt10_namespace_bases"), "namespace bases"
    ):
        base = _integer(raw_base, "namespace base")
        teacher.update(
            base + index * ATTEMPT10_POPULATION_SEED_STRIDE for index in range(250)
        )
    ...
    if any(plan["_freshness_counts"].values()):
        raise ValueError("Attempt10 population seed grid overlaps prior evidence")
    return plan
...
def _load_seed_registry(
    ...
    for relative in relative_sources:
        ...
        if candidate.is_absolute() or ".." in candidate.parts:
            raise ValueError("Attempt10 seed registry path escapes repository")
        source = (root / candidate).resolve()
        try:
            source.relative_to(root)
        except ValueError as exc:
            raise ValueError("Attempt10 seed registry path escapes repository") from exc
        if not source.is_file():
            raise ValueError(f"Attempt10 seed registry source missing: {relative}")
...
def _extract_declared_seed_schedules(payload: Mapping[str, Any]) -> set[int]:
    ...
    def visit(value: Any, inherited_stride: int = ATTEMPT10_POPULATION_SEED_STRIDE) -> None:
...
def freeze_attempt10_distilled_runtime(
    *,
    ...
    expected_runtime_bindings = {
        ...
        "runtime_fingerprint_sha256": ATTEMPT10_EXPECTED_RUNTIME_FINGERPRINT_SHA256,
    ...
    if any(
        ...
        raise ValueError("Attempt10 training/runtime source freeze mismatch")
    if (
        ...
        raise ValueError("Attempt10 external runtime identity changed")
    audit_science = _mapping(audit.get("science_boundary"), "audit50 science")
    if (
        audit.get("schema") != ATTEMPT10_AUDIT50_DECISION_SCHEMA
        or audit.get("status") != "go_attempt10_audit50_search_quality"
        ...
        raise ValueError("Attempt10 runtime freeze requires the one-shot audit50 Go")
    ...
    if (
        ...
        raise ValueError("Attempt10 audit50 Go contract changed")
    _validate_attempt10_audit50_receipt_for_freeze(
        audit_receipt, audit_decision_sha256=audit_sha
    )
    source_model = HuM43Attempt10DistilledModel.load(
        source_model_path, expected_sha256=source_sha
    )
    if source_model.safety_enabled or source_model.winner_frozen:
        raise ValueError("Attempt10 source model is already runtime frozen")
    ...
    if any(
        ...
        for name, value in {
            ...
            "fit_mode": "full",
            "effective_iterations": ATTEMPT10_DISTILLATION_FULL_ITERATIONS,
            "training_states": ATTEMPT10_DISTILLATION_FULL_STATES,
            "distillation_config_sha256": ATTEMPT10_DISTILLATION_CONFIG_SHA256,
        }.items()
    ):
        raise ValueError("Attempt10 source model is not the frozen full-fit artifact")
    ...
    freeze = {
        "schema": ATTEMPT10_RUNTIME_FREEZE_SCHEMA,
        ...
        **expected_runtime_bindings,
        "gcp_image": {
            "name": ATTEMPT10_GCP_IMAGE_NAME,
            "id": ATTEMPT10_GCP_IMAGE_ID,
            "self_link": ATTEMPT10_GCP_IMAGE_SELF_LINK,
        },
        "audit50_decision_sha256": audit_sha,
        "audit50_selector_receipt_sha256": audit_receipt_sha,
        "safety_enabled": True,
...
def _validate_attempt10_audit50_receipt_for_freeze(
    receipt: Mapping[str, Any], *, audit_decision_sha256: str
) -> None:
    if (
        set(receipt) != ATTEMPT10_AUDIT50_RECEIPT_KEYS
        or receipt.get("schema") != ATTEMPT10_AUDIT50_SELECTOR_RECEIPT_SCHEMA
        ...
        raise ValueError("Attempt10 audit50 selector receipt is not freeze authorization")


def load_bound_attempt10_distilled_model(
    path: str | Path,
    *,
    expected_sha256: str,
    ...
    execution_attestation = validate_frozen_execution_modules(
        ...
        manifest=runtime_source_manifest_path,
        module_names=ATTEMPT10_BOUND_EXECUTION_MODULES,
    )
    actual_sha = _file_sha256(path)
    if actual_sha != _sha256(expected_sha256, "model SHA"):
        raise ValueError("Attempt10 bound model SHA mismatch")
    freeze = (
        dict(runtime_freeze)
    ...
    _validate_runtime_dependency_semantics(runtime_semantic, runtime_dependencies)
    if (
        freeze.get("schema") != ATTEMPT10_RUNTIME_FREEZE_SCHEMA
        ...
        raise ValueError("Attempt10 runtime freeze contract changed")
    _sha256(
    ...
    if training.get("model_id") != freeze.get("model_id"):
        raise ValueError("Attempt10 training manifest/runtime freeze mismatch")
    training_source = _mapping(training.get("source"), "training source")
    for name in (
        ...
        if training_source.get(name) != freeze.get(name):
            raise ValueError("Attempt10 runtime source hash chain changed")
    model = HuM43Attempt10DistilledModel.load(path, expected_sha256=actual_sha)
    ...
    if any(
        ...
        raise ValueError("Attempt10 frozen model full-fit contract changed")
    for name in (
        ...
        if embedded_manifest.get(name) != freeze.get(name):
            raise ValueError("Attempt10 frozen model semantic binding changed")
    if (
        ...
        raise ValueError("Attempt10 frozen model payload disagrees with freeze")
    return BoundHuM43Attempt10DistilledModel(model, execution_attestation)


def build_attempt10_population_preflight(
    *,
    model_path: str | Path,
    training_manifest_path: str | Path,
    ...
    plan = load_and_validate_attempt10_population_plan(population_plan_path)
    ...
    model = load_bound_attempt10_distilled_model(
        model_path,
    ...
    return {
        "schema": ATTEMPT10_POPULATION_PREFLIGHT_SCHEMA,
        "status": "pass",
        ...
        "baseline_profile": ATTEMPT10_BASELINE_PROFILE,
...
def validate_attempt10_population_acceptance(
    *,
    ...
    recomputed = summarize_hu_m4_population_records(
        records,
        opponents=ATTEMPT10_OPPONENTS,
        paired_seeds=ATTEMPT10_POPULATION_SEEDS,
        seed=ATTEMPT10_POPULATION_SEED,
        seed_stride=ATTEMPT10_POPULATION_SEED_STRIDE,
        baseline_profile=ATTEMPT10_BASELINE_PROFILE,
    )
    ...
    runtime_match = (
        ...
        and runtime.get("model_schema") == HU_M43_ATTEMPT10_DISTILLED_MODEL_SCHEMA
        and runtime.get("artifact_schema") == HU_M43_ATTEMPT10_DISTILLED_ARTIFACT_SCHEMA
        and runtime.get("feature_schema") == HU_M43_ATTEMPT10_DISTILLED_FEATURE_SCHEMA
        and runtime.get("head_schema") == HU_M43_ATTEMPT10_DISTILLED_HEAD_SCHEMA
        and runtime.get("model_id") == preflight["model_id"]
        and runtime.get("action_score_mode")
        == HU_M43_ATTEMPT10_DISTILLED_ACTION_SCORE_MODE
        and runtime.get("baseline_profile") == ATTEMPT10_BASELINE_PROFILE
        ...
        and runtime.get("shard_count") == ATTEMPT10_POPULATION_SHARDS
    ...
    merge_match = (
        ...
        and merge_manifest.get("seed") == ATTEMPT10_POPULATION_SEED
        and merge_manifest.get("seed_stride") == ATTEMPT10_POPULATION_SEED_STRIDE
        and merge_manifest.get("paired_seeds_per_opponent")
        == ATTEMPT10_POPULATION_SEEDS
        and merge_manifest.get("opponents") == list(ATTEMPT10_OPPONENTS)
        ...
        and len(merge_manifest.get("shards")) == ATTEMPT10_POPULATION_SHARDS
    )
    gates = [
        _gate(
    ...
    return {
        "schema": ATTEMPT10_POPULATION_ACCEPTANCE_STATUS_SCHEMA,
        "status": decision,
        "passed_gates": passed,
...
def _validate_training_manifest(
    ...
    if (
        source.get("development_plan_sha256") != M43_ATTEMPT10_PLAN_SHA256
        ...
        != {
            "fit_mode": "full",
            "effective_iterations": ATTEMPT10_DISTILLATION_FULL_ITERATIONS,
            "states": ATTEMPT10_DISTILLATION_FULL_STATES,
            "folds": 5,
        }
        or diagnostics.get("states") != ATTEMPT10_DISTILLATION_FULL_STATES
        or diagnostics.get("rows") != ATTEMPT10_DISTILLATION_FULL_STATES * 13
        or diagnostics.get("fold_counts")
        != {str(index): 40 for index in range(5)}
    ):
        raise ValueError("Attempt10 training runtime semantic binding changed")
    if (
        manifest.get("schema") != ATTEMPT10_DISTILLED_TRAINING_MANIFEST_SCHEMA
        or manifest.get("status") != "fit_complete_runtime_disabled"
        or manifest.get("model_schema") != HU_M43_ATTEMPT10_DISTILLED_MODEL_SCHEMA
        or manifest.get("feature_schema") != HU_M43_ATTEMPT10_DISTILLED_FEATURE_SCHEMA
        or manifest.get("head_schema") != HU_M43_ATTEMPT10_DISTILLED_HEAD_SCHEMA
        ...
        raise ValueError("Attempt10 source training manifest contract changed")
...
def _validate_runtime_dependency_semantics(
    ...
    if teacher != {
        ...
        raise ValueError("Attempt10 runtime teacher semantic binding changed")
    expected = {
...
def _mapping(value: Any, label: str) -> dict[str, Any]:
    if not isinstance(value, Mapping):
        raise ValueError(f"Attempt10 {label} must be a mapping")
    return dict(value)


def _sequence(value: Any, label: str) -> list[Any]:
    if isinstance(value, (str, bytes, bytearray)) or not isinstance(value, Sequence):
        raise ValueError(f"Attempt10 {label} must be a sequence")
    return list(value)


def _integer(value: Any, label: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int):
        raise ValueError(f"Attempt10 {label} must be an integer")
    return value


def _sha256(value: Any, label: str) -> str:
    if not isinstance(value, str):
        raise ValueError(f"Attempt10 {label} must be a SHA-256 string")
    normalized = value.lower()
    if len(normalized) != 64 or any(c not in "0123456789abcdef" for c in normalized):
        raise ValueError(f"Attempt10 {label} is invalid")
    return normalized
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if runtime_source_root is not None:
        validate_frozen_execution_modules(
            ...
            module_names=(
                ...
                "ofc_regular.train_hu_m43_attempt10_distilled",
    ...
    if args.command == "freeze":
        result = freeze_attempt10_distilled_runtime(
    ...
    if args.command == "preflight":
        result = build_attempt10_population_preflight(
    ...
    status = validate_attempt10_population_acceptance(
...
__all__ = [
    "ATTEMPT10_POPULATION_ACCEPTANCE_STATUS_SCHEMA",
    "ATTEMPT10_POPULATION_PREFLIGHT_SCHEMA",
    "ATTEMPT10_RUNTIME_FREEZE_SCHEMA",
    "build_attempt10_population_preflight",
    "freeze_attempt10_distilled_runtime",
    "load_and_validate_attempt10_population_plan",
    "load_bound_attempt10_distilled_model",
    "validate_attempt10_population_acceptance",
]
