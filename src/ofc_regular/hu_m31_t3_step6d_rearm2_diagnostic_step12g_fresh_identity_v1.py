"""Fresh-identity boundary for the post-Step12f exact-pair canary.

Step12b through Step12f consumed cloud identities and audit roots are all
terminal.  This module proves that a newly prepared direct-v2 contract is
disjoint from all five terminal executions before a live backend can be
constructed.
"""
...
SCHEMA = "hu_m31_t3_step6d_step12g_fresh_identity_v1"
STATUS = (
    "fresh_step12g_identity_disjoint_from_terminal_step12b_through_step12f"
)
TERMINAL_STEP12F_CLOSEOUT_RECEIPT_SHA256 = (
    "db0aa1bd3f69e3318b2e85a1c871c60ea3c4738d0a4b080c50d0da223c43aecd"
)
TERMINAL_STEP12F_DEPLOYMENT_CONTRACT_SHA256 = (
    "62543ffa3a4e039c673c255a706d9e25c4b85a952e665094a8e6d9e4fc0b65fb"
)
...
TERMINAL_STEP12F_OUTPUT_ROOT = ARTIFACT_PARENT / "step12f_pair_v1_actual"
EXPECTED_OUTPUT_ROOT = ARTIFACT_PARENT / "step12g_pair_v1_actual"
    ...
    for _, terminal_root in _TERMINAL_ROOTS:
        ...
        if root == terminal or terminal in root.parents:
            raise PermissionError(
                "only the exact fresh Step12g output root is allowed"
            )
    if root != expected:
        raise PermissionError(
            "only the exact fresh Step12g output root is allowed"
...
def exact_output_root(path: str | Path) -> Path:
    ...
    if root.exists() or root.is_symlink():
        raise FileExistsError(f"Step12g output root is not fresh: {root}")
    if not root.parent.is_dir() or root.parent.is_symlink():
        raise FileNotFoundError("Step12g output parent must be a real directory")
    return root
...
def _validate_terminal_closeouts() -> None:
    # Step12b through Step12e bindings are revalidated through the Step12f
    # boundary module so all five stay pinned by one call.
    step12f_identity._validate_terminal_closeouts()
    step12f_closeout = _validated_sealed_receipt(
        _read_json(
            TERMINAL_STEP12F_OUTPUT_ROOT
            / "transport_burst_failure_closeout_receipt.json"
        ),
        expected_sha256=TERMINAL_STEP12F_CLOSEOUT_RECEIPT_SHA256,
        label="terminal Step12f closeout",
    )
    if step12f_closeout.get("step12f_identity_terminal") is not True:
        raise ValueError("terminal Step12f closeout binding changed")
    step12f_deployment = _read_json(
        TERMINAL_STEP12F_OUTPUT_ROOT / "deployment_contract.json"
    )
    if (
        step12f_deployment.get("deployment_contract_sha256")
        != TERMINAL_STEP12F_DEPLOYMENT_CONTRACT_SHA256
    ):
        raise ValueError("terminal Step12f deployment binding changed")
...
def validate_fresh_identity(
    *,
    ...
    if (
        ...
        raise ValueError("Step12g exact-pair topology changed")
    ...
    for label, fresh in fresh_values.items():
        if not isinstance(fresh, str) or not fresh:
            raise ValueError(f"Step12g {label} is missing")
        for terminal in terminals:
            if fresh == terminal[label]:
                raise ValueError(
                    f"Step12g reused terminal {terminal['label']} {label}"
                )
    ...
    if len(set(new_instance_names)) != 2 or any(
        ...
        raise ValueError("Step12g reused a terminal VM/disk name")
