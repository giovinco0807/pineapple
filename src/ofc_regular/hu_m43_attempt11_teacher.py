"""Bounded, profile-blind Attempt11 T1-second search teacher.

Attempt11 is a fresh one-shot architecture.  It shares only the frozen
candidate model and generic rollout primitives with earlier attempts:

* zero to twelve unique legal nonbaseline Lambda proposals plus the explicit
  baseline are reranked with R128 (there is never padding or duplication);
* K=min(8,n) is filled by the first min(4,n) actions in frozen R order and
  then the lowest-Lambda-risk actions from the remaining R positions;
...
ATTEMPT11_TEACHER_SCHEMA = (
    "hu_m43_attempt11_t1_second_variable_top12_r128_kmin8_v256_x1024_c512_e256_v1"
)
ATTEMPT11_SOLVER_ID = (
    "lambda_unique_0to12_r128_head4_riskfill_kmin8_v256_coarse_x1024_strict_"
    "c512_strict_tailrisk_lock_e256_m3_mc1_v1"
)
ATTEMPT11_FROZEN_MODEL_SHA256 = ATTEMPT08_FROZEN_MODEL_SHA256
ATTEMPT11_FROZEN_MODEL_ID = ATTEMPT08_FROZEN_MODEL_ID
ATTEMPT11_T2_POLICY_ID = ATTEMPT08_T2_POLICY_ID

ATTEMPT11_CANDIDATE_MAX = 12
ATTEMPT11_RERANK_SAMPLES = 128
ATTEMPT11_RERANK_HEAD_MAX = 4
...
ATTEMPT11_COARSE_MIN_MEAN = 0.0
ATTEMPT11_COARSE_MIN_P05 = -25.0
ATTEMPT11_COARSE_MIN_P01 = -40.0
ATTEMPT11_COARSE_MIN_VALUE = -50.0
ATTEMPT11_STRICT_MIN_MEAN = 0.0
ATTEMPT11_STRICT_MIN_P05 = -22.0
ATTEMPT11_STRICT_MIN_P01 = -36.0
ATTEMPT11_STRICT_MIN_VALUE = -45.0

ATTEMPT11_RNG_DOMAINS = (
    "hand_external",
    "rerank_r128",
    "veto_v256",
    "stress_x1024",
...
Attempt11RankScores = Attempt08RankScores
FrozenAttempt11LambdaRanker = FrozenAttempt08LambdaRanker


class Attempt11Ranker(Protocol):
    artifact_sha256: str
    model_id: str
...
class Attempt11TeacherConfig:
    """Hard-locked Attempt11 configuration for one canonical root."""
    ...
    run_id: str
    candidate_max: int = ATTEMPT11_CANDIDATE_MAX
    rerank_samples: int = ATTEMPT11_RERANK_SAMPLES
    rerank_head_max: int = ATTEMPT11_RERANK_HEAD_MAX
    shortlist_max: int = ATTEMPT11_SHORTLIST_MAX
    veto_samples: int = ATTEMPT11_VETO_SAMPLES
    stress_samples: int = ATTEMPT11_STRESS_SAMPLES
    confirmation_samples: int = ATTEMPT11_CONFIRMATION_SAMPLES
    evaluation_samples: int = ATTEMPT11_EVALUATION_SAMPLES
    coarse_min_mean: float = ATTEMPT11_COARSE_MIN_MEAN
    coarse_min_p05: float = ATTEMPT11_COARSE_MIN_P05
    coarse_min_p01: float = ATTEMPT11_COARSE_MIN_P01
    coarse_min_value: float = ATTEMPT11_COARSE_MIN_VALUE
    strict_min_mean: float = ATTEMPT11_STRICT_MIN_MEAN
    strict_min_p05: float = ATTEMPT11_STRICT_MIN_P05
    strict_min_p01: float = ATTEMPT11_STRICT_MIN_P01
    strict_min_value: float = ATTEMPT11_STRICT_MIN_VALUE
    t2_policy_id: str = ATTEMPT11_T2_POLICY_ID
    ...
    def __post_init__(self) -> None:
        ...
        if self.frozen_model_sha256 != ATTEMPT11_FROZEN_MODEL_SHA256:
            raise ValueError("Attempt11 requires the frozen candidate-only Lambda artifact")
        fixed_ints = {
            "candidate_max": ATTEMPT11_CANDIDATE_MAX,
            "rerank_samples": ATTEMPT11_RERANK_SAMPLES,
            "rerank_head_max": ATTEMPT11_RERANK_HEAD_MAX,
            "shortlist_max": ATTEMPT11_SHORTLIST_MAX,
            "veto_samples": ATTEMPT11_VETO_SAMPLES,
            "stress_samples": ATTEMPT11_STRESS_SAMPLES,
            "confirmation_samples": ATTEMPT11_CONFIRMATION_SAMPLES,
            "evaluation_samples": ATTEMPT11_EVALUATION_SAMPLES,
        ...
        for name, expected in fixed_ints.items():
            ...
            if isinstance(value, bool) or not isinstance(value, int) or value != expected:
                raise ValueError(f"Attempt11 {name} is fixed at {expected}")
        fixed_floats = {
            "coarse_min_mean": ATTEMPT11_COARSE_MIN_MEAN,
            "coarse_min_p05": ATTEMPT11_COARSE_MIN_P05,
            "coarse_min_p01": ATTEMPT11_COARSE_MIN_P01,
            "coarse_min_value": ATTEMPT11_COARSE_MIN_VALUE,
            "strict_min_mean": ATTEMPT11_STRICT_MIN_MEAN,
            "strict_min_p05": ATTEMPT11_STRICT_MIN_P05,
            "strict_min_p01": ATTEMPT11_STRICT_MIN_P01,
            "strict_min_value": ATTEMPT11_STRICT_MIN_VALUE,
        }
        ...
        for name in seed_names:
            ...
            if isinstance(value, bool) or not isinstance(value, int):
                raise TypeError(f"Attempt11 {name} must be an integer")
        if len({getattr(self, name) for name in seed_names}) != len(seed_names):
            raise ValueError("Attempt11 hand/R/V/X/C/E/child seeds must all be distinct")
        if self.t2_policy_id != ATTEMPT11_T2_POLICY_ID:
            raise ValueError("Attempt11 T2 policy is fixed at stage9f_p2")
        if not isinstance(self.run_id, str) or not self.run_id:
            raise ValueError("Attempt11 run_id must not be empty")
...
def _paired_row(candidate: _ActionScores, baseline: _ActionScores) -> dict[str, Any]:
    ...
    if not deltas or not all(math.isfinite(value) for value in deltas):
        raise ValueError("Attempt11 paired deltas must be finite and non-empty")
    return {
...
def _phase_actions(
    ...
    *,
    ...
    if len(actions) != len(scores) or not 0 <= baseline_position < len(actions):
        raise ValueError("Attempt11 phase action mapping is invalid")
...
def _lambda_risk_components(
    rank_scores: Attempt11RankScores, index: int
) -> dict[str, float]:
    return {
...
def _score_phase(
    ...
    *,
    ...
    if len(keys) != sample_count or len(set(keys)) != sample_count:
        raise ValueError(f"Attempt11 {phase} particle RNG keys are invalid")
    ...
    if len(scored) != len(actions):
        raise ValueError(f"Attempt11 {phase} scorer returned the wrong action count")
    for row in scored:
        if len(row.values) != sample_count or not all(
            math.isfinite(float(value)) for value in row.values
        ):
            raise ValueError(f"Attempt11 {phase} scorer returned invalid values")
...
def evaluate_attempt11_t1_second(
    observation: ActorObservation,
    *,
    baseline_action_key: str,
    ranker: Attempt11Ranker,
    t2_policies: Mapping[str, object],
    config: Attempt11TeacherConfig,
    library: Any | None = None,
) -> dict[str, Any]:
    """Evaluate the fixed Attempt11 search without activating a policy."""

    require_t1_second_root(observation)
    if ranker.artifact_sha256 != config.frozen_model_sha256:
        raise ValueError("Attempt11 ranker artifact hash disagrees with config")
    ...
    if len(baseline_matches) != 1:
        raise ValueError("Attempt11 explicit baseline ActionKey is not uniquely legal")
    ...
    if len(set(legal_tokens)) != len(legal_tokens):
        raise ValueError("Attempt11 complete legal action mapping is not unique")
    nonbaseline_indices = [
    ...
    top_indices = tuple(nonbaseline_indices[: config.candidate_max])
    ...
    if len({action_key(action) for action in proposal_actions}) != candidate_count + 1:
        raise AssertionError(
            "Attempt11 proposal set must contain unique candidates plus baseline once"
        )
    ...
    if len(shortlist_set) != shortlist_size:
        raise AssertionError("Attempt11 shortlist must contain K unique actions")
        ...
        legal_rows.append(
            {
                ...
                "in_learned_candidate_set": index in top_set,
    ...
    payload: dict[str, Any] = {
        "status": "ok",
        "schema": ATTEMPT11_TEACHER_SCHEMA,
        "solver_id": ATTEMPT11_SOLVER_ID,
        "street": "T1",
        "seat": "second",
        "to_act_order": "second",
        ...
        "learned_nonbaseline_original_legal_indices": list(top_indices),
        ...
        "seed_domain_provenance": {
            "domain_order": list(ATTEMPT11_RNG_DOMAINS),
        ...
        "root_selection_lock": (
            "unique_0to12_before_R128_Kmin8_before_V256_all_before_X1024_all_before_"
        ...
        "continuation_policy": {
            "t2_policy_id": ATTEMPT11_T2_POLICY_ID,
...
def _phase_raw_contract(
    ...
    *,
    ...
    if not isinstance(keys, list) or not isinstance(rows, list) or len(keys) != len(rows):
        raise ValueError("Attempt11 phase action mapping changed")
    if keys != list(expected_action_keys) or len(set(keys)) != len(keys):
        raise ValueError("Attempt11 phase action subset/order changed")
    try:
        mapped_actions = tuple(legal_by_token[token] for token in keys)
    except KeyError as exc:
        raise ValueError("Attempt11 phase contains a nonlegal ActionKey") from exc
    for name, value in _mapping_payload(mapped_actions).items():
        if phase.get(name) != value:
            raise ValueError("Attempt11 phase action mapping digest changed")
    if keys and keys[-1] != baseline_token:
        raise ValueError("Attempt11 explicit baseline must be last in each open phase")
    for position, row in enumerate(rows):
        if not isinstance(row, Mapping) or row.get("phase_position") != position:
            raise ValueError("Attempt11 phase position changed")
        if row.get("action_key") != keys[position]:
            raise ValueError("Attempt11 phase ActionKey mapping changed")
        raw = row.get("raw_paired_deltas_vs_baseline")
        if not isinstance(raw, list) or len(raw) != sample_count:
            raise ValueError("Attempt11 phase raw paired vector changed")
        if not all(
            isinstance(value, (int, float)) and math.isfinite(float(value))
            for value in raw
        ):
            raise ValueError("Attempt11 phase raw paired vector is invalid")
        if row.get("raw_paired_deltas_sha256") != _raw_digest(raw):
            raise ValueError("Attempt11 phase raw paired digest changed")
        summary = row.get("paired_delta_vs_baseline")
        if not isinstance(summary, Mapping):
            raise ValueError("Attempt11 phase paired summary changed")
        expected = {
    ...
    if rows and any(
        ...
        raise ValueError("Attempt11 explicit baseline paired vector changed")
...
def _open_phase_metadata(phase: Mapping[str, Any], *, sample_count: int) -> None:
    if (
        ...
        raise ValueError("Attempt11 open phase sample/count metadata changed")
...
def _closed_phase_contract(
    ...
    if (
        ...
        raise ValueError("Attempt11 closed phase mapping/count contract changed")


def validate_attempt11_teacher_output(
    observation: ActorObservation,
    *,
    baseline_action_key: str,
    ...
    """Recompute the bounded Attempt11 decision and fail closed on drift."""
    ...
    if any(
        ...
        raise ValueError("Attempt11 output contains hidden opponent information")
    if (
        payload.get("schema") != ATTEMPT11_TEACHER_SCHEMA
        or payload.get("solver_id") != ATTEMPT11_SOLVER_ID
    ...
    for index, (row, action) in enumerate(zip(legal_rows, legal_actions, strict=True)):
        if (
            ...
            raise ValueError("Attempt11 legal action index/order changed")
    ...
    if (
        ...
        or payload.get("learned_nonbaseline_original_legal_indices")
        != expected_top_indices
        or payload.get("learned_nonbaseline_action_keys") != expected_top_keys
    ):
        raise ValueError("Attempt11 learned candidate index/order changed")
    ...
    if payload.get("proposal_mapping") != _mapping_payload(proposal_actions):
        raise ValueError("Attempt11 proposal mapping changed")
    ...
    if not all(
        ...
        raise ValueError("Attempt11 phase payload changed")
    _phase_raw_contract(
        rerank,
        sample_count=128,
    ...
    if (
        ...
        raise ValueError("Attempt11 R128 sample/count metadata changed")
    rerank_order = sorted(
    ...
    if (
        ...
        raise ValueError("Attempt11 frozen R order changed")
    ...
    if (
        ...
        raise ValueError("Attempt11 variable shortlist subset/order changed")
    ...
    if (
        ...
        raise ValueError("Attempt11 V256 retained set changed")
    if stress.get("opened") is not bool(veto_expected):
        raise ValueError("Attempt11 X1024 conditional-open boundary changed")
    ...
    if stress.get("opened"):
        ...
        _open_phase_metadata(stress, sample_count=config.stress_samples)
        stress_expected = [
            position
        ...
        if (
            ...
            raise ValueError("Attempt11 X1024 retained set changed")
    else:
        _closed_phase_contract(
            stress, configured_sample_count=config.stress_samples
        )
    if confirmation.get("opened") is not bool(stress_expected):
        raise ValueError("Attempt11 C512 conditional-open boundary changed")
    ...
    if confirmation.get("opened"):
        ...
        _open_phase_metadata(
            confirmation, sample_count=config.confirmation_samples
        )
        ...
        if (
            ...
            raise ValueError("Attempt11 C512 risk-lock selection changed")
        ...
        if confirmation.get("selected_action_key") != (
            ...
            raise ValueError("Attempt11 C512 selected ActionKey changed")
    else:
        _closed_phase_contract(
    ...
    if decision.get("final_selected_action_key") != selected:
        raise ValueError("Attempt11 locked final action changed")
    ...
    if (
        ...
        raise ValueError("Attempt11 decision fallback contract changed")
    ...
    if (
        ...
        raise ValueError("Attempt11 E256 diagnostic lock changed")
    ...
    if payload.get("search_config") != expected_search_config:
        raise ValueError("Attempt11 variable search configuration changed")
    ...
    for left_index, left in enumerate(key_sets):
        for right in key_sets[left_index + 1 :]:
            if not left.isdisjoint(right):
                raise ValueError("Attempt11 phase RNG namespaces overlap")
    provenance = payload.get("seed_domain_provenance")
    if not isinstance(provenance, Mapping) or provenance.get("domain_order") != list(
        ATTEMPT11_RNG_DOMAINS
    ):
        raise ValueError("Attempt11 seven-domain seed provenance changed")
    expected_provenance = {
        "hand_external": config.hand_seed,
        "rerank_r128": config.rerank_seed,
        "veto_v256": config.veto_seed,
    ...
    if any(provenance.get(name) != value for name, value in expected_provenance.items()):
        raise ValueError("Attempt11 seed provenance changed")
    return {
        "schema": "hu_m43_attempt11_teacher_validation_v1",
...
__all__ = [
    "ATTEMPT11_COARSE_MIN_MEAN",
    "ATTEMPT11_COARSE_MIN_P01",
    "ATTEMPT11_COARSE_MIN_P05",
    "ATTEMPT11_COARSE_MIN_VALUE",
