...
Step12b through Step12j consumed cloud identities and audit roots are all
...
SCHEMA = "hu_m31_t3_step6d_step12k_fresh_identity_v1"
STATUS = (
    "fresh_step12k_identity_disjoint_from_terminal_step12b_through_step12j"
)
TERMINAL_STEP12J_CLOSEOUT_RECEIPT_SHA256 = (
    "22ea0459fade832e6068a1338f91cd33d25a5f1cc4a3d41b32afa399aa68813a"
)
TERMINAL_STEP12J_DEPLOYMENT_CONTRACT_SHA256 = (
    "a05772312e8f292199d8aa77c56d21cbff8e7d2d696a11bdffdb765079baa345"
)
...
TERMINAL_STEP12J_OUTPUT_ROOT = ARTIFACT_PARENT / "step12j_pair_v1_actual"
EXPECTED_OUTPUT_ROOT = ARTIFACT_PARENT / "step12k_pair_v1_actual"
    ...
    for _, terminal_root in _TERMINAL_ROOTS:
        ...
        if root == terminal or terminal in root.parents:
            raise PermissionError(
                "only the exact fresh Step12k output root is allowed"
            )
    if root != expected:
        raise PermissionError(
            "only the exact fresh Step12k output root is allowed"
...
def exact_output_root(path: str | Path) -> Path:
    ...
    if root.exists() or root.is_symlink():
        raise FileExistsError(f"Step12k output root is not fresh: {root}")
    if not root.parent.is_dir() or root.parent.is_symlink():
        raise FileNotFoundError("Step12k output parent must be a real directory")
    return root
...
def _validate_terminal_closeouts() -> None:
    # Step12b through Step12i bindings are revalidated through the Step12j
    # boundary module so all nine stay pinned by one call.
    step12j_identity._validate_terminal_closeouts()
    step12j_closeout = _validated_sealed_receipt(
        _read_json(
            TERMINAL_STEP12J_OUTPUT_ROOT
            / "mutation_transport_failure_closeout_receipt.json"
        ),
        expected_sha256=TERMINAL_STEP12J_CLOSEOUT_RECEIPT_SHA256,
        label="terminal Step12j closeout",
    )
    if step12j_closeout.get("step12j_identity_terminal") is not True:
        raise ValueError("terminal Step12j closeout binding changed")
    step12j_deployment = _read_json(
        TERMINAL_STEP12J_OUTPUT_ROOT / "deployment_contract.json"
    )
    if (
        step12j_deployment.get("deployment_contract_sha256")
        != TERMINAL_STEP12J_DEPLOYMENT_CONTRACT_SHA256
    ):
        raise ValueError("terminal Step12j deployment binding changed")
...
def validate_fresh_identity(
    *,
    ...
    if (
        ...
        raise ValueError("Step12k exact-pair topology changed")
    ...
    for label, fresh in fresh_values.items():
        if not isinstance(fresh, str) or not fresh:
            raise ValueError(f"Step12k {label} is missing")
        for terminal in terminals:
            if fresh == terminal[label]:
                raise ValueError(
                    f"Step12k reused terminal {terminal['label']} {label}"
                )
    ...
    if len(set(new_instance_names)) != 2 or any(
        ...
        raise ValueError("Step12k reused a terminal VM/disk name")
    ...
    return seal(
        {
            ...
            "terminal_step12i_closeout_receipt_sha256": (
                step12j_identity.TERMINAL_STEP12I_CLOSEOUT_RECEIPT_SHA256
            ),
            "terminal_step12j_closeout_receipt_sha256": (
                TERMINAL_STEP12J_CLOSEOUT_RECEIPT_SHA256
            ),
