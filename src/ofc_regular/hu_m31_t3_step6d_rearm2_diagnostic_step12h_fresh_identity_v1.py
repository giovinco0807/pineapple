"""Fresh-identity boundary for the post-Step12g exact-pair canary.

Step12b through Step12g consumed cloud identities and audit roots are all
terminal.  This module proves that a newly prepared direct-v2 contract is
disjoint from all six terminal executions before a live backend can be
constructed.
"""
...
SCHEMA = "hu_m31_t3_step6d_step12h_fresh_identity_v1"
STATUS = (
    "fresh_step12h_identity_disjoint_from_terminal_step12b_through_step12g"
)
TERMINAL_STEP12G_CLOSEOUT_RECEIPT_SHA256 = (
    "43aa9cd4400fa54c8f5500af2bebbb2638fee4da07ee753d85b79272476e7006"
)
TERMINAL_STEP12G_DEPLOYMENT_CONTRACT_SHA256 = (
    "9846d89548022f17040253cb23a40a6007fc4455738e93c0e60801c1d415d12d"
)
...
TERMINAL_STEP12G_OUTPUT_ROOT = ARTIFACT_PARENT / "step12g_pair_v1_actual"
EXPECTED_OUTPUT_ROOT = ARTIFACT_PARENT / "step12h_pair_v1_actual"
    ...
    for _, terminal_root in _TERMINAL_ROOTS:
        ...
        if root == terminal or terminal in root.parents:
            raise PermissionError(
                "only the exact fresh Step12h output root is allowed"
            )
    if root != expected:
        raise PermissionError(
            "only the exact fresh Step12h output root is allowed"
...
def exact_output_root(path: str | Path) -> Path:
    ...
    if root.exists() or root.is_symlink():
        raise FileExistsError(f"Step12h output root is not fresh: {root}")
    if not root.parent.is_dir() or root.parent.is_symlink():
        raise FileNotFoundError("Step12h output parent must be a real directory")
    return root
...
def _validate_terminal_closeouts() -> None:
    # Step12b through Step12f bindings are revalidated through the Step12g
    # boundary module so all six stay pinned by one call.
    step12g_identity._validate_terminal_closeouts()
    step12g_closeout = _validated_sealed_receipt(
        _read_json(
            TERMINAL_STEP12G_OUTPUT_ROOT
            / "transport_outage_failure_closeout_receipt.json"
        ),
        expected_sha256=TERMINAL_STEP12G_CLOSEOUT_RECEIPT_SHA256,
        label="terminal Step12g closeout",
    )
    if step12g_closeout.get("step12g_identity_terminal") is not True:
        raise ValueError("terminal Step12g closeout binding changed")
    step12g_deployment = _read_json(
        TERMINAL_STEP12G_OUTPUT_ROOT / "deployment_contract.json"
    )
    if (
        step12g_deployment.get("deployment_contract_sha256")
        != TERMINAL_STEP12G_DEPLOYMENT_CONTRACT_SHA256
    ):
        raise ValueError("terminal Step12g deployment binding changed")
...
def validate_fresh_identity(
    *,
    ...
    if (
        ...
        raise ValueError("Step12h exact-pair topology changed")
    ...
    for label, fresh in fresh_values.items():
        if not isinstance(fresh, str) or not fresh:
            raise ValueError(f"Step12h {label} is missing")
        for terminal in terminals:
            if fresh == terminal[label]:
                raise ValueError(
                    f"Step12h reused terminal {terminal['label']} {label}"
                )
    ...
    if len(set(new_instance_names)) != 2 or any(
        ...
        raise ValueError("Step12h reused a terminal VM/disk name")
