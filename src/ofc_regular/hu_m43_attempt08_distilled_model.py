"""Fail-closed distilled runtime selector for M4.3 Attempt08.

The expensive Attempt08 search remains a development teacher.  This module
distils that teacher into a five-fold public-information-set model while
retaining the frozen Attempt05 Lambda model solely as a top-eight candidate
generator.  Runtime never consumes teacher values, teacher lower bounds,
...
HU_M43_ATTEMPT08_DISTILLED_MODEL_SCHEMA = (
    "hu_m43_attempt08_t1_second_distilled_selector_v1"
)
HU_M43_ATTEMPT08_DISTILLED_ARTIFACT_SCHEMA = (
    "hu_m43_attempt08_t1_second_distilled_pickle_v1"
)
HU_M43_ATTEMPT08_DISTILLED_FEATURE_SCHEMA = (
    "hu_m43_attempt08_lambda_top8_public_infoset_features_v1"
)
HU_M43_ATTEMPT08_DISTILLED_HEAD_SCHEMA = (
    "hu_m43_attempt08_policy_delta_safe_tail_heads_v1"
)
HU_M43_ATTEMPT08_DISTILLED_ACTION_SCORE_MODE = (
    "attempt08_lambda_top8_distilled_safe_selector_v1"
)
HU_M43_ATTEMPT08_DISTILLED_CONFORMAL_SCHEMA = (
    "hu_m43_attempt08_identity_oof_upper_residual_conformal_v1"
)
HU_M43_ATTEMPT08_DISTILLED_RUNTIME_AUTHORIZATION = "T1-second-only"
HU_M43_ATTEMPT08_DISTILLED_EXTRA_FEATURES = 10
HU_M43_ATTEMPT08_DISTILLED_FEATURE_DIM = (
    4 * HU_FEATURE_DIM + HU_M43_ATTEMPT08_DISTILLED_EXTRA_FEATURES
)
HU_M43_ATTEMPT08_DISTILLED_REQUIRED_FOLDS = 5
HU_M43_ATTEMPT08_DISTILLED_TAIL_LIMITS = (25.0, 40.0, 50.0)
@dataclass(frozen=True)
class Attempt08DistilledFoldOutput:
...
@dataclass(frozen=True)
class Attempt08DistilledFoldPredictor:
    ...
    tail_max_head: Any
    fold_index: int
    feature_dim: int = HU_M43_ATTEMPT08_DISTILLED_FEATURE_DIM

    def __post_init__(self) -> None:
        if self.fold_index < 0:
            raise ValueError("Attempt08 distilled fold_index must be non-negative")
        if self.feature_dim != HU_M43_ATTEMPT08_DISTILLED_FEATURE_DIM:
            raise ValueError("Attempt08 distilled feature dimension changed")
        for name in (
            "ranker",
            "delta_head",
            "safe_head",
            ...
            if getattr(self, name) is None:
                raise ValueError(f"Attempt08 distilled {name} must not be None")

    def predict(self, features: np.ndarray) -> Attempt08DistilledFoldOutput:
        matrix = np.asarray(features, dtype=np.float32)
        if matrix.ndim != 2 or matrix.shape[1] != self.feature_dim:
            raise ValueError("Attempt08 distilled fold feature shape changed")
        ...
        return Attempt08DistilledFoldOutput(
            rank_score=rank,
...
@dataclass(frozen=True)
class Attempt08DistilledFeatures:
    runtime_sample: Mapping[str, Any]
    features: np.ndarray
    action_keys: tuple[ActionKey, ...]
    top8_indices: tuple[int, ...]
    baseline_index: int


@dataclass(frozen=True)
class Attempt08DistilledHeadPredictions:
...
@dataclass(frozen=True)
class HuM43Attempt08DistilledModel:
    """Hash-described, opt-in Attempt08 selector; fitting cannot activate it."""

    candidate_generator: HuM43Attempt05Model
    fold_predictors: tuple[Attempt08DistilledFoldPredictor, ...]
    conformal_cushions: tuple[float, float, float] = (0.0, 0.0, 0.0)
    conformal_quantile: float = 0.95
    safety_threshold: float = 0.5
    tail_limits: tuple[float, float, float] = HU_M43_ATTEMPT08_DISTILLED_TAIL_LIMITS
    minimum_fold_votes: int = 4
    safety_enabled: bool = False
    winner_frozen: bool = False
    model_id: str = "hu-m43-attempt08-distilled-unfrozen"
    source_candidate_sha256: str = ATTEMPT08_FROZEN_MODEL_SHA256
    manifest: Mapping[str, Any] = field(default_factory=dict)
    schema: str = HU_M43_ATTEMPT08_DISTILLED_MODEL_SCHEMA
    artifact_schema: str = HU_M43_ATTEMPT08_DISTILLED_ARTIFACT_SCHEMA
    feature_schema: str = HU_M43_ATTEMPT08_DISTILLED_FEATURE_SCHEMA
    head_schema: str = HU_M43_ATTEMPT08_DISTILLED_HEAD_SCHEMA
    action_score_mode: str = HU_M43_ATTEMPT08_DISTILLED_ACTION_SCORE_MODE
    _last_prediction_cache: Any = field(
        default=None, init=False, repr=False, compare=False
    )

    def __post_init__(self) -> None:
        if self.schema != HU_M43_ATTEMPT08_DISTILLED_MODEL_SCHEMA:
            raise ValueError("unsupported Attempt08 distilled model schema")
        if self.artifact_schema != HU_M43_ATTEMPT08_DISTILLED_ARTIFACT_SCHEMA:
            raise ValueError("Attempt08 distilled artifact schema changed")
        if self.feature_schema != HU_M43_ATTEMPT08_DISTILLED_FEATURE_SCHEMA:
            raise ValueError("Attempt08 distilled feature schema changed")
        if self.head_schema != HU_M43_ATTEMPT08_DISTILLED_HEAD_SCHEMA:
            raise ValueError("Attempt08 distilled head schema changed")
        if self.action_score_mode != HU_M43_ATTEMPT08_DISTILLED_ACTION_SCORE_MODE:
            raise ValueError("Attempt08 distilled action score mode changed")
        if len(self.fold_predictors) != HU_M43_ATTEMPT08_DISTILLED_REQUIRED_FOLDS:
            raise ValueError("Attempt08 distilled runtime requires exactly five folds")
        if sorted(int(item.fold_index) for item in self.fold_predictors) != list(
            range(HU_M43_ATTEMPT08_DISTILLED_REQUIRED_FOLDS)
        ):
            raise ValueError("Attempt08 distilled fold indices must be exactly 0..4")
        normalized_sha = _require_sha256(self.source_candidate_sha256)
        if normalized_sha != ATTEMPT08_FROZEN_MODEL_SHA256:
            raise ValueError("Attempt08 distilled source candidate SHA changed")
        ...
        if self.candidate_generator.model_id != ATTEMPT08_FROZEN_MODEL_ID:
            raise ValueError("Attempt08 distilled source candidate model_id changed")
        if self.safety_enabled and not self.winner_frozen:
            raise ValueError("Attempt08 distilled runtime cannot enable an unfrozen winner")
        if not self.model_id:
            raise ValueError("Attempt08 distilled model_id must be non-empty")
        if not 0.0 < float(self.conformal_quantile) < 1.0:
            raise ValueError("Attempt08 distilled conformal quantile must be in (0,1)")
        if not 0.0 <= float(self.safety_threshold) <= 1.0:
            raise ValueError("Attempt08 distilled safety threshold must be in [0,1]")
        if not 1 <= int(self.minimum_fold_votes) <= len(self.fold_predictors):
            raise ValueError("Attempt08 distilled minimum fold votes is invalid")
        if len(self.conformal_cushions) != 3 or any(
            not math.isfinite(float(value)) or float(value) < 0.0
            for value in self.conformal_cushions
        ):
            raise ValueError("Attempt08 distilled conformal cushions are invalid")
        if len(self.tail_limits) != 3 or any(
            not math.isfinite(float(value)) or float(value) <= 0.0
            for value in self.tail_limits
        ):
            raise ValueError("Attempt08 distilled tail limits are invalid")

    @property
    def runtime_contract(self) -> dict[str, Any]:
        return {
            "authorization": HU_M43_ATTEMPT08_DISTILLED_RUNTIME_AUTHORIZATION,
            "feature_schema": self.feature_schema,
            "head_schema": self.head_schema,
            "conformal_schema": HU_M43_ATTEMPT08_DISTILLED_CONFORMAL_SCHEMA,
    ...
    ) -> Attempt08DistilledFeatures:
        return build_attempt08_distilled_features(
            sample,
    ...
    def predict_heads_sample(
        self, sample: Mapping[str, Any], *, baseline_index: int | None = None
    ) -> Attempt08DistilledHeadPredictions:
    ...
    def _predict_heads_sample(
        ...
        *,
        ...
        candidate_mask[list(built.top8_indices)] = True
        ...
        runtime_authorized = bool(
            ...
            and _runtime_attestation.covers(ATTEMPT08_BOUND_EXECUTION_MODULES)
        )
        final_candidates = (
        ...
        return Attempt08DistilledHeadPredictions(
    ...
    def predict_safety_probability(
        ...
        *,
        ...
        if (
            ...
            raise IndexError("Attempt08 distilled candidate index is invalid")
    ...
    def load(
        ...
        if expected_sha256 is not None and digest != _require_sha256(expected_sha256):
            raise ValueError("Attempt08 distilled artifact SHA-256 mismatch")
        ...
        if not isinstance(envelope, dict) or set(envelope) != {
            ...
            raise ValueError("Attempt08 distilled artifact envelope is invalid")
        if (
            ...
            raise ValueError("Attempt08 distilled artifact schema mismatch")
        model = envelope["model"]
        if not isinstance(model, cls):
            raise TypeError("Attempt08 distilled artifact payload type mismatch")
        model.__post_init__()
        return model


class BoundHuM43Attempt08DistilledModel:
    ...
    def __init__(
        ...
        if (
            not _called_by_verified_attempt08_loader(model, attestation)
            or type(attestation) is not FrozenExecutionModulesAttestation
            or not attestation.covers(ATTEMPT08_BOUND_EXECUTION_MODULES)
        ):
            raise TypeError("Attempt08 bound runtime wrappers are loader-issued")
        if model.safety_enabled is not True or model.winner_frozen is not True:
            raise ValueError("Attempt08 bound runtime requires a frozen safe model")
        object.__setattr__(self, "_BoundHuM43Attempt08DistilledModel__model", model)
        object.__setattr__(
            self,
            "_BoundHuM43Attempt08DistilledModel__attestation",
            attestation,
        )
    ...
    def __require_capability(self) -> None:
        try:
            ...
            raise RuntimeError("Attempt08 runtime binding capability is absent") from error
        if (
            type(attestation) is not FrozenExecutionModulesAttestation
            or not attestation.covers(ATTEMPT08_BOUND_EXECUTION_MODULES)
        ):
            raise RuntimeError("Attempt08 runtime binding capability is invalid")
    ...
    def __reduce__(self) -> Any:
        raise TypeError("Attempt08 bound runtime wrappers cannot be serialized")


def is_bound_attempt08_distilled_model(value: object) -> bool:
    """Return True only for a wrapper carrying the loader-issued capability."""

    if not isinstance(value, BoundHuM43Attempt08DistilledModel):
        return False
    try:
...
def _called_by_verified_attempt08_loader(
    model: HuM43Attempt08DistilledModel,
    attestation: FrozenExecutionModulesAttestation,
) -> bool:
    ...
    try:
        ...
        loader = (
            ...
            else getattr(acceptance, "load_bound_attempt08_distilled_model", None)
        )
        return bool(
            caller is not None
...
def build_attempt08_distilled_features(
    sample: Mapping[str, Any],
    *,
    candidate_generator: HuM43Attempt05Model,
    ...
    if isinstance(raw_actions, (str, bytes)) or not isinstance(raw_actions, Sequence):
        raise ValueError("Attempt08 distilled runtime requires ordered actions")
    ...
    if observation.street != "T1" or observation.seat != "second":
        raise ValueError("Attempt08 distilled runtime authorizes only T1-second")
    ...
    frozen_ranker = FrozenAttempt08LambdaRanker(
    ...
    if paired.shape != (len(ordered_actions), 4 * HU_FEATURE_DIM):
        raise ValueError("Attempt08 distilled paired feature shape changed")
    ...
    if len(nonbaseline) < ATTEMPT08_TOP_K:
        raise ValueError("Attempt08 distilled root has fewer than eight proposals")
    nonbaseline.sort(
        key=lambda index: (-float(scores.rank_mean[index]), keys[index].sort_key())
    )
    top8 = tuple(nonbaseline[:ATTEMPT08_TOP_K])
    top8_rank = {index: rank + 1 for rank, index in enumerate(top8)}
    ...
    extra = np.column_stack(
        (
            ...
            normalized_risk,
            np.asarray(
                [top8_rank.get(index, ATTEMPT08_TOP_K + 1) / ATTEMPT08_TOP_K for index in range(len(ordered_actions))],
                dtype=np.float64,
            ),
            np.asarray(
    ...
    if (
        features.shape
        != (len(ordered_actions), HU_M43_ATTEMPT08_DISTILLED_FEATURE_DIM)
        or not np.isfinite(features).all()
    ):
        raise ValueError("Attempt08 distilled feature matrix is invalid")
    return Attempt08DistilledFeatures(
...
def _score_frozen_candidate_generator(
    ...
    *,
    ...
    if [int(item.fold_index) for item in predictors] != list(range(5)) or any(
        ...
        raise ValueError("Attempt08 distilled candidate fold identities changed")
...
def _resolve_baseline(
    ...
    if baseline_index is not None and declared is not None and int(declared) != int(
        ...
        raise ValueError("Attempt08 distilled baseline index disagrees with sample")
    if isinstance(value, (bool, np.bool_)) or not isinstance(value, (int, np.integer)):
        raise TypeError("Attempt08 distilled baseline index must be an integer")
    result = int(value)
    if not 0 <= result < action_count:
        raise IndexError("Attempt08 distilled baseline index is outside actions")
    declared_key = sample.get("baseline_action_key")
    if declared_key is not None:
        if not isinstance(declared_key, str):
            raise TypeError("Attempt08 distilled baseline ActionKey must be a string")
        if action_key_from_payload(sample["actions"][result]).to_token() != declared_key:
            raise ValueError("Attempt08 distilled baseline ActionKey disagrees with index")
    return result
...
def _prediction_cache_key(
    ...
    raw_actions = sample.get("actions")
    if isinstance(raw_actions, (str, bytes)) or not isinstance(raw_actions, Sequence):
        raise ValueError("Attempt08 distilled cache requires ordered actions")
    baseline = _resolve_baseline(sample, baseline_index, len(raw_actions))
...
def _canonical_argmax(
    ...
    if not candidates:
        raise ValueError("Attempt08 distilled argmax requires candidates")
...
def _validate_fold_output(
    output: Attempt08DistilledFoldOutput, action_count: int
) -> None:
    for name in (
        "rank_score",
        "predicted_delta",
        ...
        if values.shape != (action_count,) or not np.isfinite(values).all():
            raise ValueError(f"Attempt08 distilled fold {name} output is invalid")
    safe = np.asarray(output.safe_probability, dtype=np.float64)
    if np.any((safe < 0.0) | (safe > 1.0)):
        raise ValueError("Attempt08 distilled safe probability is outside [0,1]")
    if np.any(output.downside_p95 < 0.0) or np.any(
        ...
        raise ValueError("Attempt08 distilled downside heads are not monotone")
...
def _regression(estimator: Any, features: np.ndarray, label: str) -> np.ndarray:
    ...
    if result.shape != (features.shape[0],) or not np.isfinite(result).all():
        raise ValueError(f"Attempt08 distilled {label} prediction is invalid")
    return result
...
def _probability(estimator: Any, features: np.ndarray) -> np.ndarray:
    if hasattr(estimator, "booster_"):
        ...
        if raw.shape == (features.shape[0], 2):
            ...
            if classes.shape != (1,):
                raise ValueError("Attempt08 distilled safe classes are invalid")
            result = np.full(features.shape[0], float(classes[0] == 1))
        else:
            raise ValueError("Attempt08 distilled safe predict_proba shape is invalid")
    else:
        result = _regression(estimator, features, "safe")
    if not np.isfinite(result).all() or np.any((result < 0.0) | (result > 1.0)):
        raise ValueError("Attempt08 distilled safe probability is invalid")
    return result


def _require_sha256(value: Any) -> str:
    if not isinstance(value, str):
        raise ValueError("Attempt08 distilled SHA-256 must be a string")
    normalized = value.lower()
    if len(normalized) != 64 or any(
        character not in "0123456789abcdef" for character in normalized
    ):
        raise ValueError("Attempt08 distilled SHA-256 is invalid")
    return normalized


__all__ = [
    "Attempt08DistilledFeatures",
    "Attempt08DistilledFoldOutput",
