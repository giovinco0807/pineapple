"""Generate hidden-discard-safe T1 roots from explicit behavior profiles.

Mirrors `hu_m31_t2_behavior_roots` one street earlier: both players play T0
under one frozen profile, and the hand is exposed at the two sequential T1
...
def generate_behavior_t1_roots(
    *,
    hand_seed: int,
    behavior_seed: int,
    profile: str,
    ...
    """Play T0 with one explicit profile and expose both sequential T1 roots."""
    ...
    def deal(count: int) -> tuple[str, ...]:
        ...
        cursor += count
        if len(cards) != count:
            raise RuntimeError("T1 behavior-root deck was exhausted")
        return cards
    ...
    advance(0, first_dealt, "T1", serial)
    second = _observation(
        player=1,
        boards=boards,
        private_discards=private_discards,
...
__all__ = ["generate_behavior_t1_roots"]
