...
### M3 second seat, and the first whole-game verdict

The T2 second-seat evaluator, trained on 25,000 positions at 128 particles
generated by the cloud fleet in 1.3 hours: referee-judged +0.1121 against the
512-particle teacher's own +0.0593 floor (gap +0.0528, continuing the
per-street progression 0.0002 / 0.0202 / 0.0310 / 0.0528), and 5.8x better
than the production stage9f_p2 on the same referee (paired +0.534, interval
excluding zero). G2 met.

The number that matters more: the first whole-game measurement. A challenger
using the learned evaluators only where they exist -- T2 second, T3 both
seats, exact enumeration at T4 -- and production play everywhere else, against
the production chain end to end, paired seat-swapped decks:

    3,008 hands: challenger minus baseline +0.7114 per hand,
    95% CI [+0.4757, +0.9471].

Every label so far was generated under the production policies' behavior
distribution, so this was the test of whether per-decision gains survive both
integration and the distribution shift of a challenger playing its own
upstream streets. They do, by a margin roughly twelve times the +0.06 per
hand the turn-1 override program measured as its realized gain. The G3 bar --
beat the chain over 3,000+ hands with the interval excluding zero -- is
already cleared by a partial assembly; the formal G3 verdict still waits for
the complete policy after M4 and M5.

Engine additions behind this: a `decide` request (the assembled policy's
decision function; exact at T4 because three milliseconds is affordable at
the table, pinned models elsewhere, refusal names any missing model) and a
`score_final` request that takes two bare 13-card boards through the raw
request path, because a finished board is not a decision point and the
observation machinery rightly refuses to represent it.

Cloud accounting for the day: four fleets launched, two lost to defects
(subnet Private Google Access off; debian-12 shipping python3 without venv),
one to a checkpoint-name collision under create-only semantics (HTTP 412) on
every slow shard -- each diagnosed in minutes from the serial console mirror
and fixed with a local rehearsal before relaunch. Labels: 25,000 training
positions and a 904-position two-seed referee set, roughly twenty dollars
all in, including the failures.
