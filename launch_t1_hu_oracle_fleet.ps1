...
for ($i = 0; $i -lt $Count; $i++) {
    ...
    $name = "ofc-t1hu-$Position-$workerId"
    ...
    gcloud compute instances create $name `
        ...
        --metadata="worker-id=$workerId,max-seconds=$MaxSeconds,n-samples=$NSamples,chunk-size=$ChunkSize,states=$States,position=$Position,run-id=$RunId" `
        --metadata-from-file="startup-script=gcp_t1_hu_oracle_startup.sh" `
        --quiet
}

Write-Host ""
Write-Host "Monitor:"
Write-Host "  gcloud compute instances list --filter='name~ofc-t1hu' --project=$Project"
Write-Host "  gcloud storage ls -r gs://ofc-solver-485418/t1_hu_oracle/results/$RunId/$Position/"
