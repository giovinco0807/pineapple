...
apt-get update -y
DEBIAN_FRONTEND=noninteractive apt-get install -y curl build-essential pkg-config libssl-dev python3-numpy
if ! command -v /root/.cargo/bin/cargo >/dev/null 2>&1; then
...
p.write_text(s)
PY
/root/.cargo/bin/cargo build -p t3_generator --release
cd "$BASE"
